...
## What this is
**Arcanum** is a .NET 10, local-first AI assistant/inference hub. One Native AOT `arcanum` binary is both the HTTP host (`arcanum serve`) and thin CLI clients (`run`, `watch`, `session`, …) over the same API. macOS uses Apple's linker with the serviced .NET 10.0.12+ Native AOT pack; the isolated pre-10.0.12 compatibility path requires LLVM `lld` (`brew install lld`). Every shipping publish fails closed when its native AOT toolchain is unavailable, with no managed fallback. Linux is not a shipping RID: the hermetic SQLCipher targets map a filename only for an `osx-` or `win-` prefix, so `dotnet build` on a Linux host fails with ARCSQLC001 before any build output is produced (`ValidateArcanumNativeSqlCipher` runs `BeforeTargets=CopyFilesToOutputDirectory;ComputeFilesToPublish`, so compilation has already happened and nothing reaches the output directory or the publish set).
...
## Non-negotiable conventions (see the owning canonical documents for full rationale)
1. **Native AOT.** No reflection-based `JsonSerializer`, no `AIFunctionFactory.Create`, no anonymous DTOs. Every `/api` payload type gets a `[JsonSerializable]` entry on `ArcanumJsonContext`. Config POCOs under `Arcanum:…` must use `{ get; set; }`, **not `init`** (the config binding generator silently drops `init`-only members). In the Grimoire, EF Core is the compiled-model, change-tracking, `SaveChanges`, and transaction lane only. Every application-authored read/filter/order/aggregate/page and every set-based update/delete is parameterized direct SQLite on the same admitted connection and transaction; `EfNativeAotBoundaryTests` enforces the split in source and compiled assemblies.
...
4. **C# house style:** keep one blank line between ordinary statement/member lines, but never immediately after an opening or before a closing parenthesis, bracket, or brace; keep attached control clauses tight. Use file-scoped namespaces, positional records for DTOs, **no `[JsonPropertyName]`** on `/api` wire types (OpenAI `/v1` and MCP JSON-RPC types are the explicit exceptions), and primary constructors for DI.
5. **Thematic (D&D) naming.** New domain concepts must fit the established design terminology (`Campaign`, `Spell`, `Ward`, `Sanctum`, `Grimoire`, `Apprentice`, `The Weave`, …) unless they're genuinely universal terms like `Prompt`/`Workspace`. Propose new names before implementing.
6. **Docs travel with code.** Architecture/persistence/testing → `Arcanum.DESIGN.md`; API contracts → `Arcanum.API.md`; CLI surface → `Arcanum.Command.Reference.md`; config keys → `Compendium.README.md`. Update the owning doc in the same change set as the code. Update the root `README.md` only for an intentional public-front-page change.
...
./scripts/verify-shipping-publish.sh --rid osx-arm64  # exact Native AOT apphost + in-process provider-contract smoke
./scripts/verify-native-sqlcipher.sh --rid osx-arm64   # native provenance, hashes, symbols, compile options
```
Run everything from the repo root. The warning gates require ripgrep, the native provenance gate requires `jq`, and each fails closed naming a missing command — `brew install ripgrep jq` covers those gates; add `lld` only when using a pre-10.0.12 macOS SDK. Don't rely on `workspace_check` as a bootstrap verifier for untrusted repos — it executes repo-authored code and requires explicit feature enablement, trusted workspace bytes, and an eligible macOS containment/runtime chain. Its Ward record is informational.
