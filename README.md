...
**Grimoire MCP toggles (`Arcanum:Intelligence`):** **`EnableLoreSystem`** (default **`true`**) controls whether **`read_lore`**, **`scribe_lore`**, and **`delete_lore`** appear on **`tools/list`** and accept **`tools/call`**. **`EnableArchiveSearch`** (default **`true`**) controls **`search_archives`**. **`ArchiveSearchMaxResults`** (default **`5`**) is the per-call cap passed into the FTS query (clamped **1–500** in **`GrimoireRepository`**). **`CampaignLogThreshold`** (default **`25`**) is passed to **`IGrimoireRepository.GetConversationsNeedingSummarizationAsync`**: the **`CampaignLoggerBackgroundService`** enqueues any conversation whose count of **`ChatMessages`** with **`Timestamp`** strictly after **`LastSummarizedMessageAt`** (or all messages when that cursor is null) exceeds the threshold. Override via configuration or environment variables with the **`ARCANUM_`** prefix (for example **`ARCANUM_Arcanum__Intelligence__EnableArchiveSearch=false`**).
