...
| **Cli: ask gets local time (needs API :5001)** | One-shot **`ask`** with prompt **`local time`** (exercises multi-word args and the buffered tool loop, including **`GetLocalSystemTime`**). The same host registers MCP workspace tools (**`read_file_chunk`**, **`write_file`**, **`execute_command`**, etc.) for models that support tools. **`preLaunchTask`** runs **`build-cli`** then **`.vscode/wait-arcanum-5001.*`**, which polls **`http://127.0.0.1:5001/`** for up to **45s** so the CLI is not launched against a closed port (avoids refused-connection noise in the debugger). Start **Cli: serve** or **Api.DevHost** in another session first, or the wait step fails with a clear message. |
