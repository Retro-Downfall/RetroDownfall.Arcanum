...
**Workspace arsenal:** `POST /api/intelligence/arsenal` accepts **`PingRequest`** (**`workingDirectory`** scopes spell scan and MCP partition status). Response **`ApiResponse<WorkspaceArsenalDto>`** (**`activeSpells`**, **`nativeTools`** — currently **`get_local_system_time`** — **`mcpServers`** as **`McpServerStatusDto`** rows: **`serverName`**, **`status`** (**`Online`** / **`Failed`**), **`toolCount`**, **`providedTools`**, optional **`errorMessage`**). Shapes are registered on **`ArcanumJsonContext`** for Native AOT.
...
**API key:** Routes under **`/api`** (for example `GET /api/health`, `POST /api/intelligence/ping`, `POST /api/intelligence/ping-stream`, `POST /api/intelligence/human-response`, `POST /api/mcp/reload`, `POST /api/intelligence/arsenal`) require header **`X-Arcanum-Key`** with the current master key. Requests without a valid key receive **401** with a source-generated JSON body: `ApiResponse<string>` and `Error` code **`Unauthorized`** (serialized via [`ArcanumJsonContext`](src/RetroDownfall.Arcanum.Api/Serialization/ArcanumJsonContext.cs)).
...
|----------------|---------|
| **Full Stack: API + External Chat** (compound) | Starts **Api.DevHost** and **`arcanum chat`** together (`stopAll: true`). **Api.DevHost** is the slim API on **:5001**; **`chat`** runs in an **external** terminal (better Spectre REPL). The **`chat`** profile’s **`preLaunchTask`** is **`build-cli-wait-api-port`** so the CLI waits until **`http://127.0.0.1:5001/`** responds (same **`.vscode/wait-arcanum-5001.*`** scripts as before). **Api.DevHost** opens **`/scalar/v1`** in the browser when Kestrel logs **Now listening on** (`serverReadyAction` in [`.vscode/launch.json`](.vscode/launch.json)). |
| **Api.DevHost: slim API** | Used by the compound; you can also launch it alone. Same routes as **`serve`** without Spectre; do not run **`serve`** and **Api.DevHost** at the same time (both bind **:5001**). |
| **Cli: Interactive Chat (External Terminal)** | Used by the compound; **`chat`** with **`build-cli-wait-api-port`**. Alone, it waits for an API on **:5001** (start **`serve`** or DevHost first, or the wait fails with a clear message). |

**`ask` / `chat` and the API:** The CLI’s HTTP client targets **`http://localhost:5001/`** (see `Program.cs`). Inference runs in the **API process** (`serve` or DevHost), including the bounded tool loop; the CLI streams **`ping-stream`** and prints NDJSON. **`ask`** and **`chat`** need an API host already listening unless you use the **Full Stack** compound.

**Arbitrary CLI commands:** For **`serve`**, **`ask …`**, **`look`**, **`daemon`**, etc., run **Tasks: Run Task** → **Arcanum: CLI workspace shell** (or **`dotnet run --project src/RetroDownfall.Arcanum.Cli/RetroDownfall.Arcanum.Cli.csproj -- …`** from a terminal) and attach the debugger to the **`dotnet`** process if you need breakpoints.
