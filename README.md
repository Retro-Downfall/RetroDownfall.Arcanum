...
After confirmation, global/all apply re-plans and publishes an owner-only V1 active record in `Prepared` with `HostFactoryErasure` before any host mutation. Its `OperationId` is the normalized requested-operation replay key; the host creates a distinct durable data-operation ID and echoes both identities in its content-free result. The requested identity is never `LongRunningOperation.RootOperationId`, the server operation ID, a Covenant gate owner, or a lease owner. The CLI durably appends a monotonic completion proof — requested identity, distinct server identity, confirmed data-plan ID, and ordinary observed totals — before asking the host to stop. The order is exact: `prepare -> host apply/replay -> proof -> shutdown -> maintenance lock -> offline continuation`. The offline continuation trusts only that proof, performs daemon mutation, advances to `DataResetComplete`, and continues the frozen cleanup/credential/verification phases without invoking a lease-free factory route. A pre-effect `Data.PlanChanged` may retire the prepared handoff; cancellation, connection loss, timeout, termination, or any uncertain outcome preserves it for exact replay. Host startup remains blocked by every active reset except the sole recovery bridge: global/all `Prepared + HostFactoryErasure` with no durable completion proof may start `serve` so the named operation can finish or replay. A proof-bearing `Prepared` record and every later or legacy state remain startup-blocking.
