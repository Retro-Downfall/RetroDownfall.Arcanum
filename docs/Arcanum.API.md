...
**Request-side `toolPolicy`** (on `PingRequest`, and likewise on `SpellExecuteRequest` and `PromptExecuteRequest`) narrows what a turn advertises: `allTools`, `noTools`, `readOnlyTools`, or `noForbiddenArts` (semantics in DESIGN §10.2.1). Omitted is equivalent to `allTools`. Like the reasoning and tokenization enums (§8.8), it is a **strict string-only** AOT contract: a numeric enum — defined or undefined — and an unknown enum string both fail JSON binding before any semantic validation, surfacing as **400** `Validation.InvalidBody` on the native routes and **400** `invalid_json` on `/v1`. The strictness is load-bearing rather than stylistic, because every consumer treats an unrecognized policy as the permissive arm: an integer that bound to an undefined member would fail open, discarding the same request's `disableMcpTools` and skipping the advertisement filters. Note that `disableMcpTools` is honoured only when `toolPolicy` is omitted or `allTools`; `noTools` disables MCP tools outright, while `readOnlyTools` and `noForbiddenArts` supersede the flag and apply their own advertisement filter.
