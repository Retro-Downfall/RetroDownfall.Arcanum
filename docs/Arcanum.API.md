...
>
> **Issue #90 status.** Backup, restore, and selective Session transfer are local operations and add no route here. `GET /api/sessions/{id}/export` and `GET /api/campaigns/{id}/export` are **unchanged**: the typed sensitivity refusal before content and the typed Covenant/tainted exclusion counts are still owed. The import side is covered instead — with `Arcanum:Features:Covenant` on, a selective import runs through `IProtectedArtifactTransferStore`, which refuses any Covenant-derived source outright ([DESIGN §10.19.5](Arcanum.DESIGN.md#10195-selective-transfer-and-the-two-ports-this-slice-had-to-build-first)).
