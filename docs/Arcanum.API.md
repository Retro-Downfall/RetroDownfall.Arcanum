...
| POST | `/api/web/search` | First-class bounded web search (`WebSearchWorkflowRequest` → `ApiResponse<WebSearchWorkflowResult>`; citations and provider usage; DESIGN §11.27). Optional `attachToSessionId` stores the result Markdown; the provider call is already billed by then, so a failed attachment answers **200** with `attachmentId: null` and a non-null `attachmentError` rather than discarding the answer. |
| POST | `/api/web/browse` | First-class bounded static page read (`WebBrowseWorkflowRequest` → `ApiResponse<WebBrowseWorkflowResult>`; JavaScript mode degrades explicitly when no renderer is configured; DESIGN §11.27). `attachToSessionId` behaves as on `/api/web/search`: a failed attachment reports `attachmentError` and still returns the page. |
...
| GET | `/api/sessions/{id}/export` | Export JSON or Markdown (`ApiResponse<SessionExportResult>`). Required `format` is `json` or `markdown`, matched without regard to case; anything else is **400** `Session.InvalidFormat`. With `Arcanum:Features:Covenant` on, a Session carrying any Covenant-derived artifact is refused with **403** `Covenant.PlaintextExportRefused` **before** the transcript is read ([DESIGN §10.19.11](Arcanum.DESIGN.md#101911-refusing-tainted-plaintext-export-and-reporting-what-a-campaign-export-left-behind)). |
...
| POST | `/api/memory/search` | Case-insensitive persisted-memory inspection (`MemorySearchRequest` → `ApiResponse<MemorySearchResponse>`). `scope` is `session`, `attachments`, `workspace`, `saga`, `lexicon`, or `all`; default `all` is echoed. Optional `sessionId`/`workspaceId` narrow ownership. Every result includes scope, source id, provenance, and retention. No embeddings or promotion side effect. One request returns at most 10,000 results in total — a single budget shared by every scope, not one per scope — consumed in scope order (`session`, `attachments`, `workspace`, `saga`, `lexicon`). Optional `limit` narrows that budget (never widens it); a value outside `[1, 10000]` is refused **400** `Validation.InvalidBody` rather than clamped, so a full page is never mistaken for a complete one. The response carries `scopes` — one `{ scope, count, hasMore }` entry per consulted scope, in consultation order — and a top-level `hasMore` that is true when any of them was truncated. A scope the budget was already exhausted before reaching is reported as `count: 0, hasMore: true`, which is how a starved scope is told apart from an empty one. |
...
| POST | `/api/spells/import` | Import spell into workspace (`ApiResponse<SpellSummary>`; **400** `Spell.NameCollision`, `Spell.InvalidFrontmatter`, `Spell.InvalidSkillJson`, `Validation.InvalidBody` when `payload` is absent; §8.14). |
...
| POST | `/api/spells/{name}/versions/{version}/activate` | Activate a version, swapping its content into `SPELL.md` and preserving the prior active content as `SPELL.v{previousLabel}.md`. Re-activating the already-active label would alias that backup onto the version file being activated, so the backup is skipped and `previousVersion` comes back `null`; the version → `SPELL.md` copy and the `activeVersion` write still run, which is what applies an edit made through `PUT /api/spells/{name}/versions/{version}`. |
...
| PUT | `/api/campaigns/{id}` | Update campaign (`ApiResponse<CampaignDto>`; DESIGN §19). A supplied `settings` object replaces the stored one wholesale rather than merging member by member, so every member a partial body omits falls back to that member's own default. `requireWardForForbiddenArts` carries its default on the record itself: omitting it leaves the campaign warded, and opting out means sending `"requireWardForForbiddenArts": false` explicitly. |
...
| GET | `/api/campaigns/{id}/prompts` | Prompts scoped to a campaign (`ApiResponse<ListPageResult<PromptSummaryDto>>`; `?q=`, `?tag=`; **404** `Campaign.NotFound`; DESIGN §19). Reads one 10,000-row page and reports `hasMore`/`nextOffset` from it, the same way `GET /api/prompts` does; `?q=`/`?tag=` filter that page client-side and therefore always report `hasMore: false`. |
...
| POST | `/api/campaigns/{id}/export` | Export spells + prompts + settings (`ApiResponse<CampaignExportDto>`; DESIGN §19). Carries no Covenant content, version, receipt, hash, provenance, or tainted artifact. With `Arcanum:Features:Covenant` on it adds `exclusions` — the content-free `covenantEntryCount` and `taintedArtifactCount` this bundle left behind; the field is omitted entirely when the feature is off ([DESIGN §10.19.11](Arcanum.DESIGN.md#101911-refusing-tainted-plaintext-export-and-reporting-what-a-campaign-export-left-behind)). |
| POST | `/api/campaigns/{id}/import` | Import portable campaign bundle (`ApiResponse<CampaignImportResultDto>`; DESIGN §19). The bundle's shape is validated in full before anything is written, so a bundle missing `campaign`, carrying a null spell/prompt entry, or carrying spell metadata that is not valid JSON is refused with **400** `Campaign.ImportFailed` and `strategy: "replace"` has not deleted anything. Absent `spells`/`prompts` are treated as empty. |
...
| POST | `/api/prompts/{id}/test` | Assemble system prompt without LLM (`ApiResponse<PromptTestResultDto>`; an optional `codexPath` outside the campaign or working directory is refused with **400** `Prompt.CodexPathNotContained`, an absent file with `Prompt.CodexPathNotFound`, a file that exists but cannot be opened or read with `Prompt.CodexReadFailed`, and one over the effective codex cap with `Prompt.CodexPathTooLarge`; DESIGN §19). |
...
| POST | `/api/prompts/import` | Import prompt (`ApiResponse<PromptSummaryDto>`; DESIGN §19). A body whose `payload` is absent, or whose payload omits `name`, `version`, or `template`, is refused with **400** `Prompt.InvalidRequest`/`Prompt.InvalidName`/`Prompt.InvalidVersion`; an absent `tags` persists as an empty list. |
...
| POST | `/api/executions/{id}/cancel` | Cancel a running execution; returns updated `ApiResponse<DaemonExecutionSummary>` (**400** `Daemon.NotRunning` when not running). Cancellation is cooperative: the response records the execution terminal, but the job body may still be unwinding, and the daemon keeps its single-flight reservation until it actually returns — so a `POST /api/daemons/{id}/run` issued during that window still fails `Daemon.AlreadyRunning` rather than starting a second headless turn against the same target spell. |
...
**Request-side `toolPolicy`** (on `PingRequest`, and likewise on `SpellExecuteRequest` and `PromptExecuteRequest`) narrows what a turn advertises: `allTools`, `noTools`, `readOnlyTools`, or `noForbiddenArts` (semantics in DESIGN §10.2.1). Omitted is equivalent to `allTools`. Like the reasoning and tokenization enums (§8.8), it is a **strict string-only** AOT contract: a numeric enum — defined or undefined — and an unknown enum string both fail JSON binding before any semantic validation, surfacing as **400** `Validation.InvalidBody` on the native routes and **400** `invalid_json` on `/v1`. The strictness is load-bearing rather than stylistic: an integer that bound to an undefined member would carry a restriction nothing in the system recognizes. Every consumer now resolves an unrecognized policy to `noTools` — the advertisement path advertises nothing, and the invocation context reports `noTools`, so an unknown value can never be the arm that widens what a turn may do. `disableMcpTools` and `toolPolicy` are both narrowing instructions and compose as their intersection: `noTools` disables MCP tools outright, and `readOnlyTools`/`noForbiddenArts` apply their advertisement filter **and** still honour the flag rather than superseding it.
...
There is no hop counter. Passes continue while they discover new unique URLs and stop at an optional `sourceTarget`, deterministic source exhaustion/no-progress, caller cancellation, explicit token/cost policy, provider/context failure, or a safety denial. The fetch phase that follows is bounded even when the request names no `sourceTarget`: a code-owned ceiling of 50 citations applies, and fetching stops once the retained page text already covers the synthesis prompt's character budget, since everything past it would be held in memory and then dropped unread. An explicit `sourceTarget` is the operator's own authority and is honoured as written.

On live synthesis, supplied `attachedFiles` and `scryingFoci` enter the normal attachment pipeline: when Attachments are enabled they are persisted and bound to the synthesis Session before model inference; when Attachments are disabled they remain in-memory current-turn content. A context preview never persists them. `attachToSessionId` is separate: it stores the final research Markdown, not the supplied synthesis context. Attachment is an optional side effect on an answer that is already billed by the time it runs, so a failure there is never terminal: the stream emits an `attachment_failed` progress frame carrying the attachment error and still emits the `result` frame, with `attachmentId` null. `/api/web/search` and `/api/web/browse` have no progress channel, so they carry the same fact as an `attachmentError` field on the result itself.
...
Both write routes are UTF-8 text routes, and neither rewrites bytes it cannot represent. PATCH decodes the target strictly: a target that is not valid UTF-8, or that contains a NUL byte and is therefore binary, returns **403** `Workspace.PathNotAllowed` and the file is left byte-for-byte untouched. PUT carries an existing destination's UTF-8 BOM across the overwrite, matching PATCH, so a read-modify-write loop through `GET .../files/contents` does not strip the preamble; a `content` value that already begins with U+FEFF supplies its own BOM and is never doubled.

Registration creates the workspace `.arcanum` directory **before** the Campaign row is persisted, mirroring `POST /api/campaigns`. A directory that cannot be created returns `Workspace.AccessDenied` / `Workspace.WriteFailed` and leaves nothing registered, so a retry is never blocked by a half-completed first attempt. Recursive file listing skips a subdirectory that vanished or cannot be read and returns the rest of the tree instead of failing the whole page. The listing walk runs off the request thread and proves containment once per directory chain rather than re-walking every ancestor component for each child; a symbolic link is still validated individually, since it is the one entry that can break a chain its parent already proved.
...
**Covenant inventory.** `DataRetentionStatus` and `DataRetentionPlan` each carry an optional `covenant` object with `rows`, `managedFiles`, `localArtifacts`, `affectedSessions`, `possibleDisclosures`, and `disclosureCountKind` (`Exact` or `LowerBound`). It is counts only — no subject text, key, digest preimage, path, or Campaign identity. `possibleDisclosures` folds only **nonrevocable** buckets, because it sits beside a statement about what local removal cannot revoke, and one lower-bound bucket carries the whole total to `LowerBound`. The field is **omitted entirely** when no Covenant read capability could be taken, which is any installation with the feature off or the canonical tier unhealthy: a zero would read as a measurement where the honest answer is that nothing was measured. A `Covenant` status item appears in `items` under exactly the same condition, always with `policyEnabled: false` and a null `retentionDays`. The plan's `covenant` object is deliberately excluded from `planId`, so an unrelated Covenant write never expires a prune plan (DESIGN §10.20.1).
...
| `Campaign.InvalidPath` / `DuplicateName` / `DuplicatePath`; `Session.Archived` / `InvalidStatus` / `InvalidFormat` / `TooManyEntries` / `EntryTooLarge` / `MemoryManagementDisabled` / `EmptyContent`; `Attachment.InvalidRequest` / `InvalidContent` / `InvalidReference` / `SourceUnavailable`; `Apprentice.Disabled` / `PendingQueueFull` / `InvalidGuidance` / `InvalidPlan` / `InvalidGoal` / `InvalidWorkspace`; `Workspace.NameEmpty` / `SymbolicLinkEscape` / `PathTraversal` / `DirectoryNotEmpty` / `ReplacementAmbiguous` / `PathIsDirectory` / `PathIsFile`; `Spell.NoWorkspace` / `InvalidWorkspace` / `InvalidName` / `NameCollision` / `BuiltinReadOnly` / `DuplicateVersion` / `InvalidVersion`; `Prompt.CodexPathNotContained` / `CodexPathRequired` / `CodexPathInvalid` / `CodexPathNotFound` / `CodexReadFailed` / `CodexPathTooLarge` / `DuplicateVersion` / `InvalidName` / `InvalidVersion` / `InvalidRequest`; `Mcp.AmbiguousServer` / `MissingWorkspace` / `ServerNotRunning` / `AmbiguousTool` / `ToolError`; `Sending.TaskRejected` / `ModalityMismatch` / `SkillNotAdvertised` / `PushNotificationRejected` / `PushNotificationsDisabled`; `Security.BlockedOutboundUrl` / `IdempotencyKeyTooLong`; `Files.InvalidMimeType`; `Batches.InvalidEndpoint`; `Embeddings.ConfirmationRequired`; `ProvingGrounds.InvalidTrial` / `WorkspaceNotAllowed`; `Saga.NotEmpty`; `Scrying.VisionNotSupported` / `TooManyImages` / `UnsupportedMimeType` / `InvalidImageData`; `WebBrowsing.TooLarge` (emitted in the `browse_web` tool result when a page nests HTML past the extraction ceiling; oversized bodies still truncate) / `InvalidUrl`; `ClientTools.Disabled` / `TooMany` / `InvalidSchema`; `Guardrails.PiiDetected` / `Blocked`; `StructuredOutput.ValidationFailed` / `SchemaInvalid`; `Operation.InvalidState`; `Perception.InvalidPath`; `Codex.PathNotContained` / `ContentTooLarge`; `Validation.InvalidLore`; `Validation.InvalidKey` | 400 | Domain validation / policy refusal (non-auth) |
| `Campaign.PathNotAllowed`; `Workspace.PathNotAllowed` / `AccessDenied` / `FileWriteDisabled`; `Spell.PathNotAllowed`; `Sending.Disabled` / `AgentNotAllowed`; `Mcp.WorkspaceNotTrusted` / `DiagnosticBlocked`; `Scrying.FeatureDisabled`; `Attachment.Disabled`; `WebBrowsing.SsrfBlocked`; `Perception.PathNotAllowed` | 403 | Path/network/feature deny |
...
| `Covenant.ForbiddenAuthority`; `Covenant.SensitiveEgressRequiresApproval`; `Covenant.PlaintextExportRefused` | 403 | The caller holds no authority for this effect, a Covenant-bearing disclosure the operator did not approve, or a plaintext export of a Session that carries Covenant-derived content. The export refusal has no approval arm and no retry that succeeds: a plaintext file is nonrevocable the moment it exists |
...
Composes `IWeaveService` + tokenizer. `model` must match `Arcanum:Integrations:Embeddings:Model` or be omitted → else **404** `model_not_found`. Long inputs use code-owned chunking + mean-pool/L2. `encoding_format` is `float|base64` (`EmbeddingBlobCodec`). Idempotency-Key is supported, and (like `/v1/chat/completions`) its fingerprint is the raw request bytes. Embedding provider calls have no Arcanum whole-operation deadline; request-abort/caller cancellation propagates directly, while provider failures are sanitized as **503** — including a provider answer whose vectors are not all the same width, which is refused rather than mean-pooled into a silently truncated embedding. That width check is a property of the whole response rather than of one round trip: a request issues one provider call for its short inputs and one per long input, so the assembled `data[]` is checked for a single common width before it is written, and a pool that answers each call consistently but the request at two model widths is refused the same way. Errors use the OpenAI envelope (**400** invalid input/chars, **400** `invalid_json` for a body that does not parse, **415** `unsupported_media_type` for a non-JSON `Content-Type`; **503** when The Weave is unavailable). The body is read by the handler rather than bound as a route parameter, so a malformed body never produces a body-less framework 400/415.
...
The request and response shapes the Covenant surfaces will use are frozen. The internal maintenance, transition, and recovery graph is implemented and composed, but **no route in this section is registered yet**; issue #128 owns activation. It is documented here because four surfaces build against these shapes, and a contract that four of them each invent separately is four contracts.

> **Boundary status.** The authenticated boundary these routes will sit behind is now in place — see [§8.29](#829-x-arcanum-context-policy-and-the-covenant-pre-binding-boundary) — and `MemoryStatusDto` gained its content-free `covenant` block. The routes themselves are still unmapped.
>
> **Backup and restore status.** Backup, restore, and selective Session transfer are local operations and add no route here. The import side is covered by `IProtectedArtifactTransferStore`, which refuses any Covenant-derived source outright ([DESIGN §10.19.5](Arcanum.DESIGN.md#10195-selective-transfer-and-the-two-ports-it-had-to-build-first)).
>
> **Export status.** The export side is covered too. `GET /api/sessions/{id}/export` returns **403** `Covenant.PlaintextExportRefused` before a single content byte when the Session carries any Covenant-derived artifact, and `POST /api/campaigns/{id}/export` reports typed `exclusions` counts for the Covenant memory and tainted artifacts it did not carry. Both hold one conditional Covenant read lease from before the export graph through the last response byte, and both emit the protected header tuple of [§8.29](#829-x-arcanum-context-policy-and-the-covenant-pre-binding-boundary) while that arm is active. With the feature off neither takes a lease, the `exclusions` field is absent, and both behave exactly as they did ([DESIGN §10.19.11](Arcanum.DESIGN.md#101911-refusing-tainted-plaintext-export-and-reporting-what-a-campaign-export-left-behind)).
...
The authenticated boundary every Covenant route will sit behind applies today to every `/api` and `/v1` route, whether or not any Covenant route exists yet.
