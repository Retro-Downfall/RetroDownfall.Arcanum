...
>
> **Issue #90 status.** Backup, restore, and selective Session transfer are local operations and add no route here. The import side is covered by `IProtectedArtifactTransferStore`, which refuses any Covenant-derived source outright ([DESIGN §10.19.5](Arcanum.DESIGN.md#10195-selective-transfer-and-the-two-ports-this-slice-had-to-build-first)).
>
> **Issue #114 status.** The export side is covered too. `GET /api/sessions/{id}/export` returns **403** `Covenant.PlaintextExportRefused` before a single content byte when the Session carries any Covenant-derived artifact, and `POST /api/campaigns/{id}/export` reports typed `exclusions` counts for the Covenant memory and tainted artifacts it did not carry. Both hold one conditional Covenant read lease from before the export graph through the last response byte, and both emit the protected header tuple of [§8.29](#829-x-arcanum-context-policy-and-the-covenant-pre-binding-boundary) while that arm is active. With the feature off neither takes a lease, the `exclusions` field is absent, and both behave exactly as they did ([DESIGN §10.19.11](Arcanum.DESIGN.md#101911-refusing-tainted-plaintext-export-and-reporting-what-a-campaign-export-left-behind)).
