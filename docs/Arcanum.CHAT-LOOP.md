...
Installation factory-reset apply first asks the running authenticated host to publish and prove its V2 reset handoff under the exact lifetime maintenance lock, then requests loopback shutdown and acquires the lock for offline continuation. No chat, watcher, indexing, or Tapestry writer remains active across that boundary. A noncompleted, ambiguous, or unreported-completed reset record blocks normal startup. The only recovery-host exception is the owning operation's authenticated V2, or exact eligible V1 migrated before its next effect, at global/all `Prepared + HostFactoryErasure + no proof`; that host admits only health, quit, and the exact factory replay while unrelated chat/API/background work stays closed. An active offline transition blocks startup on its own account, and that exception does not cover it: after the reset record is read and before the database bootstrapper runs, a journal that is standalone, bound as this reset's nested arm, or already carrying the stored completion receipt with only its retirement left keeps readiness closed and reports that manual recovery is required, because an active journal means the database is part way through a transformation. Startup refuses there rather than resuming the transition. Reading the two records as a pair adds its own refusals — a parent-bound journal with no reset record, a reset holding a claim whose journal is gone, a binding the two cannot reproduce together — and each is the same content-free answer, which deliberately does not say which arm it was. After a verified global or all reset, `arcanum run` enters the existing setup wizard on a fresh interactive invocation instead of recreating Grimoire state implicitly.
