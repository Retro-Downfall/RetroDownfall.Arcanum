...
Installation factory-reset apply requests authenticated loopback host shutdown, then acquires the maintenance lock before accepted deletion. No chat, watcher, indexing, or Tapestry writer remains active across that boundary. A noncompleted or unreported completed reset record blocks the next host startup. After a verified global or all reset, `arcanum run` enters the existing setup wizard on a fresh interactive invocation instead of recreating Grimoire state implicitly.
