...
| `1` | API, execution, or unexpected stream-disconnect failure; a lookup that found nothing; a parse failure a helper rather than the handler reported; a completed Trial that did not pass. |
| `2` | Command-line/configuration failure, including every check the handler itself performs on what it was given: a required option that is absent, a value outside its enumeration, and a value the handler could not parse. Also a confirmation that cannot be obtained non-interactively, a non-positive watch-health interval, an unrecognized `doctor` diagnostic or repair id, and an operation reconciliation that still requires operator attention. |
