...
| `Covenant.ManualRecoveryRequired` | A `data-retention-mutation` or `data-retention-factory-reset` row is an interrupted Covenant erasure. Its checkpoint is intact and still names the exact phase it reached, but this build ships only half the storage owner the erasure coordinator drives — the canonical transaction exists, the file scrub and the authority publication do not — so it is deliberately left in place. | Do not `retry`: restarting it would run a second erasure over a family whose canonical arm may already be gone. Covenant admission stays closed while the row exists. `arcanum operation show <id>` reports the recorded phase. |
...
| `arcanum data reset-memory --scope <scope>` | Reset exactly one named store: `entry`, `attachments`, `workspace`, `saga`, `lexicon`, or `covenant`. There is no ambiguous generic memory delete, and numeric enum spellings are rejected before any API request. `covenant` is planned and inventoried but refused on apply with `Data.CovenantResetRequiresErasureCoordinator` — erasing that family needs the exclusive erasure coordinator this build does not yet wire to a route, and nothing is changed. | `--scope <scope>` — Required explicit memory scope.<br>Mutation confirmation; use global `--yes` for automation. |
