...
| Command | Explanation | Additional command options |
|---|---|---|
| `arcanum saga list` | Paginated listing of Saga memories. A `Scope` column names the Campaign each memory belongs to, or reports it as global, unresolved, or not yet classified; naming `--session` narrows the listing to what that session's turns could rank, while omitting it lists the whole store. | `--query <query>` — Free-text query.<br>`--session <session>` — Filter by session GUID.<br>`--limit <limit>` — Maximum number of memories to return.<br>`--offset <offset>` — Pagination offset. |
