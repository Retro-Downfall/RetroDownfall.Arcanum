...
| `1` | API, execution, or unexpected stream-disconnect failure; a completed Trial that did not pass; and the content a command was handed rather than the option that named it — an `@file` value that could not be read, a `key=value` pair that is not one, or an inline JSON body that would not deserialize. |
| `2` | Command-line/configuration failure, including every check performed on an option's own value: a required option that is absent, a value outside its enumeration, and a value that would not parse as the type the option declares. Also a configured thing the command needs and cannot find — a master API key that was never stored, for one — plus a confirmation that cannot be obtained non-interactively, a non-positive watch-health interval, an unrecognized `doctor` diagnostic or repair id, and an operation reconciliation that still requires operator attention. |
...
Interactive auto-start uses short connection/readiness observation only: two seconds per nonce-bound local presence proof, three seconds for an already-listening host whose proof is not ready, and 20 seconds after spawn. It validates the responder before reading a credential, tries the encrypted local mirror before the OS credential store, and shares one short-lived process capability plus fixed-size key digest across the launcher and API clients; it never caches or sends the reusable master key. Capability renewal is single-flight before expiry, and the shared authenticated sender performs at most one refresh-and-replay after a `401` for requests whose bodies are explicitly replayable. A second `401` evicts only that rejected capability without another request or credential-store read; the next call renews it through the same single-flight path. A launcher timeout never kills the spawned host; retry, run `arcanum doctor`, verify `arcanum key show`, or inspect `~/.config/arcanum/logs/auto-serve-bootstrap.log`.
...
Setup is the recommended first-run path. After a successful setup on a fresh installation, it explains that the first server start will create Arcanum's private server-authentication key in the operating-system credential store. Setup does not create that key early: the foreground `run`, Command Center, or direct `serve` command gives the immediate explanation before it starts the host.
...
A launch committed by a process that died before publishing its journal is finished too, after the database is bootstrapped and before readiness is published, rather than by the ordinary reconciliation pass that runs afterwards. Exactly one such row may be adopted; a second, a legacy shape, a malformed one, and a resumption that does not reach a durable outcome each keep readiness closed.
...
Startup resolves the installation-reset record and the offline-transition journal as a pair, under the same held installation maintenance lock and before any database bootstrap. The exception therefore admits a recovery-mode `serve` only while no journal is active: the reset either never launched a nested database transition, or the one it launched completed and its journal is already retired. An active journal is finished before the database is opened for ordinary use, on the lock the host already holds — standalone transition work, this reset's exact nested arm, and a nested arm whose completion receipt is already stored and whose retirement alone remains, all alike. Startup opens the existing catalog through an opener that can validate it and nothing else, reads back the minimum persisted Covenant authority and refuses unless it agrees with the journal that names it, spends a one-use handoff that closes the operation gate around the reconstructed owner, closes that opener physically, and then runs the registered recovery handler for the exact operation the journal names. Only a durable outcome lets the start continue into ordinary bootstrap and readiness. A transition that parks, a catalog this installation can no longer open, an authority that does not agree with the journal, and a lease that could not be adopted each keep readiness closed and report that manual recovery is required. A pair that cannot be read forward together is the same refusal under `Covenant.ManualRecoveryRequired`: a claimed nested transition whose journal is absent, a journal that names a parent no record claims, a parent binding that does not recompute from the two records, a journal earlier than the receipt stored against it, and two records naming different terminal winners. Which of those it was is deliberately not in the message, because every one of them has the same remedy.
...
| `storage.file_encryption` | Storage | Explicit deep encrypted-blob inventory and file-encryption secret status. Unlike the host's constant-time `FileEncryption` health snapshot, this check may open secure storage and scan managed ciphertext. Unreadable or linked inventory is never treated as empty. | `arcanum data encryption status` |
...
| `arcanum key provider delete <provider>` | Delete a provider credential from local secure stores, after confirmation. | `--kind <kind>` — `inference` or `web-research`. |
...
| `arcanum lore delete <key>` | Delete a lore entry, after confirmation. | None beyond global or inherited family options. |
...
| `arcanum campaign delete [<id>]` | Remove a campaign, after confirmation. | None beyond global or inherited family options. |
...
| `arcanum campaign codex delete [<id>]` | Delete CODEX.md, after confirmation. | None beyond global or inherited family options. |
...
| Command | Explanation | Additional command options |
|---|---|---|
| `arcanum saga list` | Paginated listing of Saga memories. A `Scope` column names the Campaign each memory belongs to, or reports it as global, unresolved, or not yet classified; naming `--session` narrows the listing to the memories that session itself wrote, and omitting it lists the whole store. That is not the candidate set a turn in that session ranks, in either direction: a turn also ranks every installation-scoped memory and every memory of its own Campaign whoever wrote them, and it ranks no retired memory at all. A retired memory is listed here exactly as a live one is, and this table has no column that marks it: the listing selects from the memory rows themselves rather than through the embeddings retrieval ranks, so retirement — which removes those embeddings — changes what a turn can recall without changing what this command prints. `arcanum memory saga show <id>` reports one memory's retirement and pin state. | `--query <query>` — Free-text query.<br>`--session <session>` — Filter by session GUID.<br>`--limit <limit>` — Maximum number of memories to return.<br>`--offset <offset>` — Pagination offset. |
| `arcanum saga divine <query>` | Semantic search over Saga memories, with the same `Scope` column. `--session` searches *as* that session: the Campaign comes from that session's immutable binding, never from a Campaign named on the command line. With `features.campaignScopedMemory` enabled and no `--session`, the search sees installation-scoped memories only. | `--limit <limit>` — Maximum number of results to return.<br>`--session <session>` — Search as this session, honoring the Campaign scope its turns draw from. |
| `arcanum saga delete <id>` | Delete a single Saga memory, after confirmation. | None beyond global or inherited family options. |
...
| Command | Explanation | Additional command options |
|---|---|---|
| `arcanum memory status [<session>]` | Show feature gates and counts by memory store, including the `Tapestry` row (published hierarchical nodes only; gated by `Arcanum:Features:Tapestry`). When the Covenant arm is composed it adds a block of its own: whether the feature is enabled, whether the canonical tier is available with its degradation code when it is not, one row per scope, lane, and lifecycle with its count, and the rendered byte totals for the three prompt sections beside the per-section ceiling. The Global figure is that section's whole cost; the two Campaign figures are the largest single Campaign's, not an installation-wide sum, because the ceiling beside them bounds one Campaign's rendered section on one turn. One ceiling is printed for all three sections because the three placement ceilings hold the same value, and a domain-contract test pins them equal so a section that grew a ceiling of its own could not be reported under another section's. A census that was not taken — never on an installation with the feature off, and refused on an enabled one whenever an exclusive operation is closing the scope, which happens with the canonical tier reporting itself healthy — prints `Covenant entries were not counted` with the reason and no byte line at all, because every count and total is zero in that case and only a taken census may be rendered as emptiness. An installation with no Covenant arm prints no block at all rather than a row of zeroes, and an installation whose arm holds nothing and was counted prints `No Covenant entries.` — a zero is a measurement, and the honest rendering of something never measured is absence. | None beyond global or inherited family options. |
...
| `arcanum memory saga correct <id> --expected-content-hash <hex>` | Replace the text of one Saga memory, naming the exact content being corrected. Replacement text comes from `--file` or piped standard input — never argv, so a corrected memory never lands in shell history. Content that is empty or whitespace-only is refused from either source before any route is reached, because a file path can lie about what it holds and nothing in the command can tell a mistyped path apart from a deliberate blank. That is a property of this verb, not of the store: the correct route accepts the content it is given, so a memory really can be set to blank text through the API, and the refusal names that route for an operator who meant it. Correcting a retired memory is refused with `Saga.AlreadyRetired`: a retired memory is reinstated before it is corrected. Correcting to the text already stored succeeds, reporting `Unchanged`. | `--file <path>` — read the replacement text from this file; omit to read from piped standard input. |
...
**Nine of the dedicated commands below exist.** `arcanum memory covenant set|correct|list|show|retire|pin|unpin|mask|unmask` are registered and appear in the committed command map, including `list --lifecycle` and `show --history`. The Campaign path, Session-binding, and `doctor` branches are not, and neither is free-text query: no such option is registered on any verb and no route is mapped for it, the service method behind it keeping a typed `Covenant.Unavailable` refusal for a stale caller, because an unbuilt search reporting no matches would be indistinguishable from a Covenant that holds none. The `explain` route is mapped and the API client can reach it, but no verb is registered over it. It evaluates the Campaign the request names, resolved through the same canonical policy a live turn resolves through, so a Campaign explanation reports that Campaign's own sections — including the Proposed Section an agent's suggestion is rendered in — rather than Global sections under the Campaign's name. The schema-repair, index-rebuild, and family-reinitialize surfaces have no route and no verb either — the port behind them is registered nowhere, and two of its four methods still refuse with `Covenant.ManualRecoveryRequired` — so `memory status` reporting `RebuildRequired` names a state rather than a command. Their structured `--json` payloads are frozen (`CovenantEntryPayload`, `CovenantListPayload`, `CovenantShowPayload`, `CovenantMutationPlanPayload`, `CovenantMutationResultPayload`, `CovenantDoctorPayload`, `CampaignPathStatusPayload`, `CampaignPathPlanPayload`, `CampaignPathResultPayload`, `SessionBindingStatusPayload`, `SessionBindingResultPayload`) in `CliJsonContext`, together with the HTTP shapes they project ([API §8.28](Arcanum.API.md#828-covenant-public-contract-mutation-and-inspection-routed)). They are documented here so the surfaces that implement them have one contract to implement rather than one each. Five of them are what a registered verb writes today — `CovenantEntryPayload` inside `CovenantListPayload`, `CovenantShowPayload`, and the `CovenantMutationPlanPayload`/`CovenantMutationResultPayload` pair the three mutating verbs emit. The four curation verbs write the API's own `CovenantCurationResultDto` rather than a CLI payload, and the remaining names belong to the branches no verb registers, so nothing constructs them.

| Command | Input source and confirmation | Output and exit |
|---|---|---|
| `arcanum memory covenant set <key> [--campaign <id>] [--file <path>] --expected-revision <n> [--reactivate]` | Content from `--file` or piped standard input — never argv, so a preference never lands in shell history. There is no interactive console path: an unredirected terminal is a typed `Validation.InvalidBody` refusal naming the two sources, not a prompt. Omitting `--campaign` writes the installation-wide Global scope. Calls the set-preflight route first and shows the server's compiled hash, framed byte cost, and resolution effects, then confirms. Redirected stdin and every `--json` mutation require `--yes`; a confirmation that cannot be asked for is a typed non-interactive refusal naming `--yes`, never a cancellation reported as success. An expectation the preflight reports as stale is refused before the question is put, naming the revision the lane is actually at, because approving a screen the commit will then reject spends the operator's decision on a write that cannot happen. | `CovenantMutationPlanPayload` on the diagnostic stream, then `CovenantMutationResultPayload` as the one document on stdout; standard exit codes. |
| `arcanum memory covenant list [--campaign <id>] [--all-scopes] [--lane <lane>] [--lifecycle <lifecycle>]` | Read-only. Global is the default scope, `--campaign` narrows to one Campaign, and `--all-scopes` widens to Global and every Campaign together; no default sweeps every Campaign. `--lane` and `--lifecycle` refuse an unrecognized value with the valid names rather than falling back to no filter. Every page is followed to exhaustion: the continuation is an AEAD-sealed cursor the server mints, so there is no value an operator could supply, and a listing that stopped after one page would announce entries nothing could reach. A server that hands back the cursor it was given stops the walk and says the listing is incomplete. | `CovenantListPayload`. |
| `arcanum memory covenant show <key> [--campaign <id>] [--history]` | Read-only. Omitting `--campaign` reads the Global scope. `--history` adds each written lane's version page, newest revision first, printing revision, operation, origin, framed cost, mutation identity, and timestamp — never authored content. Only lanes with a head are asked about, and without the flag no version page is read at all, so a lookup's cost does not grow with how often the key was written. | `CovenantShowPayload`. |
| `arcanum memory covenant retire <key> [--campaign <id>] --lane <lane> --expected-revision <n>` | Omitting `--campaign` retires from the Global scope. Calls the retire-preflight route and shows the server's resolved lane, the exact targeted revision, dependent-head digest, Global and Proposed eligibility effects, full affected-Campaign count, and any truncated examples, then confirms through `IConfirmationPrompt`. An unrecognized `--lane` fails the command; it used to become absence, which this verb read as the operator-authored Confirmed lane. `--expected-revision` is required here rather than defaulted, because a live head is never at revision zero and an omitted flag would send the one value guaranteed to be refused. `--json` requires `--yes` independently of stdout redirection. | `CovenantMutationPlanPayload` on the diagnostic stream, then `CovenantMutationResultPayload` as the one document on stdout. |
| `arcanum memory covenant correct <key> [--campaign <id>] [--file <path>] --target-version <id> --target-hash <hex> --expected-revision <n>` | Replaces the text of one live preference while naming exactly the version it replaces. Corrected content arrives through `--file` or piped standard input, never argv, on the same terms as `set`. The three target flags come off `show` and `show --history`, which is where an operator reads the preference they decided was wrong; all three are required, because a correction that named only a revision would be a statement about whatever is current when the request lands rather than about something they looked at. A key with no live Confirmed head, a tombstone, a version or revision that is not the head's, and a hash naming content the head does not hold are each refused before the confirmation is put. Correcting the Proposed lane is not offered: that lane belongs to the agent, and promoting a proposal is a write of the operator's own. | `CovenantMutationPlanPayload` on the diagnostic stream, then `CovenantMutationResultPayload` as the one document on stdout. |
| `arcanum memory covenant pin <key> [--campaign <id>] [--lane <lane>] [--expected-revision <n>]` | Marks one scoped lane head durable, so the agent may not write over it or retire it. The operator's own `set`, `correct` and `retire` still work. `--expected-revision` defaults to zero, which is the curation revision of a subject nobody has curated — the common case, and the one an omitted flag should mean. | `CovenantCurationResultDto`. |
| `arcanum memory covenant unpin <key> [--campaign <id>] [--lane <lane>] [--expected-revision <n>]` | Releases a pin, so the agent may propose over the preference again. | `CovenantCurationResultDto`. |
| `arcanum memory covenant mask <key> --campaign <id> [--lane <lane>] [--expected-revision <n>]` | Stops a Global preference applying inside one Campaign, with nothing in its place. `--campaign` is required: Global content has no broader scope to be falling through from, so a Global mask is unrepresentable and requiring the flag says so on the command line rather than after a confirmation. The screen states which of the two outcomes is about to happen — the Global entry stops applying here, or it starts applying again — read off the server's measurement rather than off the flags this process parsed. A Campaign that already holds its own value for the key is told the mask suppresses nothing observable there. | `CovenantCurationResultDto`. |
| `arcanum memory covenant unmask <key> --campaign <id> [--lane <lane>] [--expected-revision <n>]` | Lets a masked Global preference apply in this Campaign again. | `CovenantCurationResultDto`. |
| **(not registered)** `arcanum memory covenant doctor [--repair-schema \| --rebuild-index \| --reinitialize-family]` | `--reinitialize-family` displays every server-authoritative loss, local-erasure, nonrevocable-disclosure, free-space, and preserved-core count and requires explicit destructive confirmation; redirected, plain, and JSON modes require `--yes`. It never touches the database or filesystem from the CLI process. | `CovenantDoctorPayload`; rebuild and reinitialize return the existing long-operation descriptor and use the established watch path. |
| **(not registered)** `arcanum campaign path status (--all \| --campaign <id>)` | Read-only. `--all` is the bulk legacy-upgrade inventory; each unresolved row carries the exact next verb. | `CampaignPathStatusPayload`. |
| **(not registered)** `arcanum campaign path register\|update\|repair\|deregister\|takeover <campaign-id> [<path>]` | The server opens and identifies the path; the CLI never inspects or writes a marker. Displays the opened path, old and new identity digests, marker effect, and drained-turn impact before confirmation. Redirected, plain, and JSON modes require `--yes`. | `CampaignPathPlanPayload` then `CampaignPathResultPayload`. |
| **(not registered)** `arcanum session campaign-binding status [--all \| --session <id>]` | Read-only. | `SessionBindingStatusPayload`. |
| **(not registered)** `arcanum session campaign-binding resolve <session-id> (--global \| --campaign <id>)` | Operates only on `LegacyUnresolved`; a final binding cannot be changed. Displays the immutable choice and the affected Session, then confirms. | `SessionBindingResultPayload`. |
| **(not registered)** `arcanum security host-process-tools enable --yes` | The contract for the one offline Covenant maintenance exception, held for whatever verb finally drives the transition port: it would acquire the installation lock and never start or call the HTTP host, where every other Covenant command is HTTP-thin and reaches Infrastructure through `ArcanumApiClient` only. No verb registers this spelling, and nothing in the product names it: a host blocked on the escape hatch tells the operator to clear `ARCANUM_ALLOW_HOST_PROCESS_TOOLS` instead. | One `--json` document, plain mode, confirmation, cancellation, and the standard exit codes, when it exists. |
...
| `arcanum spell delete <name>` | Delete a spell, after confirmation. | `--workspace <workspace>` — Selects the workspace by ID, name, or server-host path. |
...
| `arcanum prompt delete [<id>]` | Delete a prompt, after confirmation. | None beyond global or inherited family options. |
...
| `arcanum apprentice delete [<id>]` | Delete a terminal Apprentice, after confirmation. | None beyond global or inherited family options. |
...
| `operation.recovery_handler_missing` | No handler is registered for the kind. This is a build defect, not a runtime state. | Report it. Recovery never guesses success: every policy but `AbandonSafely` stops at `ReconciliationRequired`, and an `AbandonSafely` kind is closed as `Abandoned`, which is what that policy already says may be done to an instance nothing can drive forward. |
...
| `Covenant.ManualRecoveryRequired` | A Covenant erasure has a content-free manual blocker or malformed recovery evidence that automatic recovery cannot safely interpret. Valid current reset and factory-erasure launches are adopted under their exact owner and resumed by the production coordinator, which reads how far the run got from its authenticated journal rather than from the operation row. A launched factory erasure reruns ordinary cleanup when `ManagedArtifactsProcessed` is durable and skips it at `HandlesClosed` or later; a reset has no ordinary continuation, and legacy factory version 0 is unchanged. A factory erasure that is a reset's nested arm is adopted under the one nested identity that reset's own record claimed, so a resumed reset replays that transition rather than starting a second one; the journal remains the authority for how far the database work got, and the record supplies only the parent binding the journal has to reproduce. A record pair that disagrees resolves to this code — a claim whose journal is absent, a journal naming a parent no record claims, a binding that does not recompute, a journal earlier than the receipt stored against it, or two records naming different terminal winners — without saying which. | Keep admission closed, inspect `arcanum operation show <id>`, and follow the reported restore, Covenant-family reinitialize, or full-installation-reset remedy. Do not start a second erasure. |
...
| Command | Explanation | Additional command options |
|---|---|---|
| `arcanum data status` | Show rows, files, estimated bytes, effective policy, store, and provenance for every typed retention class, plus aggregate totals and categories preserved outside the selected root. The Annals class reports the bitemporal claim rows held over Saga and Lexicon memories; like the Covenant family it is inventoried and never aged out on its own timer, because a claim's lifecycle is its subject's. | None beyond global or inherited family options. |
...
| `arcanum data reset-memory --scope <scope> [--campaign <guid>]` | Reset exactly one named store: `entry`, `attachments`, `workspace`, `saga`, `lexicon`, or `covenant`. `--campaign` narrows the reset to one Campaign's memories and is accepted for `saga` and `lexicon` only — those are the two stores that record an owning Campaign — and is refused with `Data.InvalidRequest` for any other scope rather than silently widened. Every other Campaign's memories, and every installation-scoped memory, are left exactly where they were, and the confirmation prompt names the Campaign it is about to clear. Whichever store a reset names, the bitemporal claims held over the memories it clears go with them and the other store's claims are left standing. A whole-store `saga` reset additionally clears the evidence retirement leaves behind — the keyed suppression digests and the installation key that binds them — because digests left without their key read as evidence while matching nothing; naming a Campaign clears that Campaign's digests alone and keeps the key, which every other scope's digests are still bound by. There is no ambiguous generic memory delete, and numeric enum spellings are rejected before any API request. Every scope first calls the dedicated read-only reset-plan route and later sends that preview id as `expectedPlanId`. For `covenant`, the CLI additionally writes the nonrevocable external boundary, the receipt-backed possible-attempt count, and resolved provider help targets before confirmation, then enters the durable ten-phase protected-erasure coordinator. Plan drift is refused before effects. Success keeps the confirmed plan id, reports reconciliation, and does not invent row, file, byte, or derived-record deletion totals from retained disclosure evidence. | `--scope <scope>` — Required explicit memory scope.<br>`--campaign <guid>` — Optional Campaign; `saga` and `lexicon` only.<br>Mutation confirmation; use global `--yes` for automation. Covenant scope writes its disclosure to diagnostics before either path. |
| `arcanum data factory-reset (--workspace\|--global\|--all) (--dry-run\|--apply)` | Preview or apply the closed installation-reset plan. `--workspace` targets the most-specific registered Campaign containing the current directory, its exact-root derived Grimoire rows, and its `.arcanum` tree. `--global` targets installation-wide state, configured credential identities, and daemon registration. `--all` captures the current Campaign before running the global phase. Global/all rebind the authenticated online plan before any output or confirmation; missing host/key/inventory fails before shutdown. Apply sends a typed in-memory handoff to the running host; under its exact installation lock the host publishes owner-only authenticated V2 `Prepared + HostFactoryErasure`, uses its requested-operation identity only as the replay key for a distinct server operation, and durably records the content-free completion proof before responding. The CLI then shuts down the host, acquires the exact maintenance lock, and passes it through offline continuation. Exact order remains `prepare -> host apply/replay -> proof -> shutdown -> lock -> offline continuation`. The data phase requires a measurable current inventory and exclusive lifecycle, then runs protected erasure first and ordinary factory cleanup between `ManagedArtifactsProcessed` and `HandlesClosed`; missing proof fails closed rather than running ordinary cleanup alone, and recovery repeats that cleanup only before `HandlesClosed` is durable. Cancellation or an uncertain host outcome preserves authenticated evidence for replay; only a proven pre-effect `Data.PlanChanged` may close and retire it. Recognized `.arcbackup` files and nested registered Campaign roots are preserved. Ordinary global/all apply rereads and requires a clean host-tools marker pair before client coordination, host-handoff creation, the online host factory call, shutdown, lock acquisition, or any reset effect; missing pair-reader composition fails closed. The sole external-remediation spelling is `arcanum data factory-reset --all --apply --external-remediation-attestation <file>`: before configuration bootstrap the CLI requires that exact mode, then securely reads one owner-controlled file of at most 64 KiB and strictly decodes its source-generated version-1 JSON contract. The P-256/SHA-256 signature, fixed issuer, 24-hour lifetime, signed operation, installation identity, exact live `TaintedMatched` marker evidence, action, and nonce must all verify through the code-pinned public root. The signed operation ID becomes the authenticated active-record identity; this authorization creates no host handoff, online replay, or reset effect. Fresh acceptance must occur inside the statement's signed issue/expiry window. The exact authenticated pre-effect claim may resume after expiry only while the supplied statement and a final authoritative live installation/marker snapshot remain exact. Startup projects no ordinary host handoff from that claim; a missing retry attestation fails before the apply boundary, and the ordinary exact-lock seam rejects the claim even if the pair later reads clean. Acceptance records only the minimal encrypted, authenticated remediation claim fields and keeps the installation fail-closed. Under that same held lock the command then compare-deletes the exact host-tools marker pair, journaling the pair before the first deletion so a crash between the two can never publish a clean installation, and drives every authenticated Campaign cleanup child to a terminal outcome — deleted through the root that proved it, or left in place as a typed orphan — with deleted and orphaned counts that add up to the journaled intent count. It then reconciles every managed workspace file the Grimoire still records outside itself: each unfinished write is driven to a proven-clean or manual-orphan terminal by compare-deleting the exact created-child identity its producer recorded, each adopted file is routed through the one shared erasure state machine that the live retention path uses, and the operation refuses to continue unless the safe and orphaned counts add up against the source and work-item inventories it fixed first. Only once that inventory is verified does the locked service continue into the same ending an ordinary factory reset takes — accepted data plan, the sweep that deletes the Grimoire, accepted credentials — and then, between that sweep and verification, observe the database file absent, prove the profile's restore history terminal, and compare-remove five profile-namespaced credentials: the three restore accounts in anchor, journal-key, installation-identity order, then the offline-transition slot's anchor and key in that order, each against a digest projected while every account was still present. The pair is gated by its own proof, taken under the same held lock: the canonical journal file and its three siblings have to be proved absent through the same durable primitive retirement uses, and the slot's anchor has to read `Closed` for this installation with both of its accounts present, or anchor, key and file have to be absent together. Every other shape — an active anchor, a surviving journal file, a key beside an absent anchor — mints no proof and removes nothing, and a nested database transition this reset claimed but never saw report its completion blocks the removal, stopping the reset short of retirement with recovery required. Both projections are persisted into the same authenticated record before the first removal and each completed phase after it, so a crash mid-cleanup resumes against the proof rather than against whatever the accounts now hold; a slot that was never opened persists no projection and removes nothing. A fresh read proving all five accounts gone is the terminal of the cleanup, and only then is the active record retired. Short of a verified managed-file inventory the command reports the operation admitted and recovery required exactly as before; on Linux the operating-system marker capability still refuses outright, so a full reset cannot complete there at all. No statement plaintext, signature, nonce, issuer text, file path, trust-root material, or private-key material is written to command output, confirmation, diagnostics, logs, or an HTTP request; the signed operation ID remains intentionally visible as the reset operation identity. | Exactly one scope and mode are required. Interactive apply requires the case-sensitive text `RESET`; global and all scopes write any Covenant disclosure first. Noninteractive apply requires both global `--yes` and command-local `--force`; `--force` is invalid with dry-run, and global/all automation still emits any disclosure diagnostics.<br>`--external-remediation-attestation <file>` — Valid only with exact `--all --apply`; supplies the independently signed remediation statement. It is never an alternative to `--yes`, `--force`, or interactive confirmation, and no other command or mode accepts it. |
...
- [`Arcanum.Engineering.md`](Arcanum.Engineering.md) — contributor and agent orientation.
