...
| Command | Explanation | Additional command options |
|---|---|---|
| `arcanum memory status [<session>]` | Show feature gates and counts by memory store, including the `Tapestry` row (published hierarchical nodes only; gated by `Arcanum:Features:Tapestry`). When the Covenant arm is composed it adds a block of its own: whether the feature is enabled, whether the canonical tier is available with its degradation code when it is not, one row per scope, lane, and lifecycle with its count, and the rendered byte totals for the three prompt sections beside the per-section ceiling. The Global figure is that section's whole cost; the two Campaign figures are the largest single Campaign's, not an installation-wide sum, because the ceiling beside them bounds one Campaign's rendered section on one turn. A census that could not be taken — the installation read capability is refused whenever an exclusive operation is closing the scope, which happens with the canonical tier reporting itself healthy — prints `Covenant entries were not counted` with the reason and no byte line at all, because every count and total is zero in that case and only a taken census may be rendered as emptiness. An installation with no Covenant arm prints no block at all rather than a row of zeroes, and an installation whose arm holds nothing and was counted prints `No Covenant entries.` — a zero is a measurement, and the honest rendering of something never measured is absence. | None beyond global or inherited family options. |
