...
| `Covenant.ManualRecoveryRequired` | A `data-retention-mutation` or `data-retention-factory-reset` row is an interrupted Covenant erasure. Its checkpoint is intact and still names the exact phase it reached, but this build ships no storage owner for the erasure coordinator to drive, so it is deliberately left in place. | Do not `retry`: restarting it would run a second erasure over a family whose canonical arm may already be gone. Covenant admission stays closed while the row exists. `arcanum operation show <id>` reports the recorded phase. |
