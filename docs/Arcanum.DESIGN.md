...
| `Arcanum:Security:AllowUnsandboxedToolChildren` | `bool` | `false` | — | When `false` (default), `execute_command` / `run_spell_script` require an OS filesystem sandbox (Landlock on Linux, `sandbox-exec` on macOS). Setup failure refuses the tool rather than running unbounded. When `true`, logs a warning and runs without the FS jail (resource limits still apply where available). **Filesystem-only** — does not isolate network. On Windows, Sanctum `Enabled` + `EnforcePathBoundary` still denies these tools regardless of this flag. |
...
**MVP OS filesystem jail:** The same **`CappedChildProcessRunner`** composes env scrub → resource limits → **filesystem jail** → cwd / output caps / cancellation. This MVP is a **filesystem sandbox only** — it does **not** prevent network use by network-capable binaries. Sanctum network policy still applies at known tool boundaries (`browse_web`, Comm Link); `execute_command` network behavior is **not** solved by the FS jail.
- **Linux:** parent launches `Environment.ProcessPath __sandbox-exec --config <owner-only.json>` (hidden helper; not in CLI help). The helper applies **Landlock** path-beneath rules then `execv`s the target with argv (never shell-concatenated). Access classes: workspace / Sanctum `AllowedPaths` / spell script roots → read+write (+ execute as needed); minimal system runtime (`/usr`, `/bin`, …) → read+execute, **no write**; outside user-data → deny.
- **macOS:** wraps the child with deprecated **`sandbox-exec`** and an owner-only Seatbelt profile (deny-default + explicit allows). Documented as MVP-only — Apple may remove the tool; absence fail-closes unless `Arcanum:Security:AllowUnsandboxedToolChildren=true`.
- **Windows:** no FS jail. When the campaign Sanctum is **`Enabled` and `EnforcePathBoundary`**, `execute_command` / `run_spell_script` return the expected denial (*"Child process filesystem sandbox is unavailable on Windows while Sanctum path-boundary enforcement is enabled…"*). Otherwise Job Object resource limits only.
- **Fail-closed:** when the jail cannot be applied and the escape hatch is false, the model-visible result is *"Child process filesystem sandbox is unavailable on this host; refusing to run tool unbounded."* (not a Hub generic internal error).
- **Escape hatch:** `Arcanum:Security:AllowUnsandboxedToolChildren` (default `false`) logs a warning and runs without FS confinement.
