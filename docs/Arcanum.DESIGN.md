...
| `ask_human` | Prompt the operator for input (available even without a workspace). Shares `IHumanPromptRegistry` with standard MCP `elicitation/create`. Waiters prefer linked inference/stream cancellation; a **30-minute hard ceiling** is a leak guard only (default inference timeout is typically shorter). Timeout/cap surface as expected tool/elicitation results, not generic infrastructure faults. Soft cap: 64 concurrent waiters. |
