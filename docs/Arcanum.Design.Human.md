...
Unified retention sits above the existing stores instead of pretending they are one database. `arcanum data status` reports typed classes across SQLCipher rows, encrypted file trees, dated JSONL logs, and derived indexes. `Arcanum:Retention` gives each class an explicit enabled/day rule, keeps automatic sweeps and session-bearing deletion off by default, bounds each sweep, checkpoints durable progress, applies an accounting minimum, and supports explicit protected-session holds. Audit writers rotate and append their current dated files but never perform an unbounded cleanup scan. Historical inference and guardrail logs can be removed only by the same bounded, durable retention plan/apply boundary as the other typed classes. Two typed classes deliberately carry no rule at all: `Tapestry`, because a tree is derived data the weaving sweep already rebuilds and prunes, and `Covenant`, because its versions, tombstones, and disclosure receipts are the evidence that makes an erasure claim checkable and a schedule that quietly aged them out would destroy the only record of what was destroyed. `Covenant` is inventoried by `data status` as five content-free counts and removed only by an explicit reset; asking to give it a rule is refused with a message that says why rather than one that blames the operator's spelling. That reset does not run yet, but the durable record it will keep is: ten ordered, literally coded storage phases named once and shared by Covenant reset and healthy-catalog factory erasure, carried in a version-3 retention checkpoint alongside the immutable server operation identity, the 32-byte effect digest, and the exact operation code — which is all a recovery pass may rebuild an exclusive owner from. An unknown, zero, skipped, or regressed phase fails closed, the earlier checkpoint versions are unchanged and still recover as before, nothing may close admission until the first checkpoint has actually committed, and an interrupted erasure is reconciled ahead of every ordinary durable writer at the next start. The component that drives those ten steps now exists: it closes admission under one exclusive lease, quiesces the warm disclosure writer before touching anything, hands every artifact to the two shared erasure kernels under one borrowed authority rather than deleting anything itself, and takes exactly one reopening decision — reopening only when nothing was destroyed at all, and otherwise leaving the gate closed and the operation resumable. The storage owner it drives every irreversible step through is written up to the point of publication, so it is still registered nowhere and reached by nothing. The one transaction that removes the family's rows exists — it refuses to start until the database confirms in that moment that it overwrites the space it frees, it clears every pooled and directly held connection before taking its exclusive lock, and it leaves the disclosure record, the Campaign path markers, and the host-tools taint evidence exactly where it found them. The part that scrubs the file afterwards exists too, and it measures rather than assumes: it reads the engine's answer to a cleanup instead of discarding it and refuses if the cleanup was blocked or left frames behind; it shrinks the file and then checks whether what remains really is only the pages the file declares, writing and verifying a fresh copy where it cannot prove that, and leaving the original untouched when the copy cannot be verified; it looks for every kind of leftover file a database can leave beside itself and reports which kind survived without naming where it lives; and it reopens the result read-only through a handle that is incapable of creating any of those leftovers, re-reading the identity the erasure stamped and re-counting the rows it emptied. What is still missing is the step that publishes the result. Every direct deletion route — one Entry, session compaction, a Saga fact, all Saga facts, a Lexicon entity, an embeddings reset — now dispatches through one shared boundary that resolves each artifact's sensitivity label and hands the labelled ones to the single erasure kernel, so a governed artifact cannot leave through a legacy raw delete; the bulk paths walk bounded identity pages rather than issuing a set-based statement over rows nothing examined, and the three legacy repository deletes refuse a labelled artifact outright.
