...
Arcanum is a **.NET 10, Native AOT, local-first AI assistant and inference hub** delivered as one
self-contained executable. `arcanum serve` runs the HTTP host; CLI commands and desktop applications
consume that API. The host exposes native `/api` contracts and an honest OpenAI Chat Completions
compatibility subset under `/v1`.

```mermaid
flowchart LR
    Users[CLI / Command Center / The Forge / OpenAI clients] --> HTTP[ASP.NET Core Minimal API]
    HTTP --> Engine[TurnExecutionCoordinator + TurnEngine]
    Engine --> Models[OpenAI-compatible providers]
    Engine --> Tools[Built-ins + MCP tools]
    Engine --> Memory[Grimoire + Lexicon + Saga + The Weave]
    Tools --> Policy[Ward + Sanctum + OS boundaries]
    Config[arcanum.json / Compendium] --> HTTP
    Config --> Engine
```
...
```text
Cli ───────────────► Api ───────────────► Infrastructure ───────────────► Core
 │                    │                         │                            │
 │                    │                         └─ OS, SQLCipher, MCP, RAG   └─ contracts/domain
 │                    └─ endpoints, engine composition, /v1
 └─ terminal clients and the shipping executable

Compendium.Ux ─────────────────────────────────────────────────────────► Core
```

- **Core** owns portable contracts, settings, primitives, and domain models.
- **Infrastructure** owns OS-adjacent implementations and the encrypted Grimoire.
- **Api** owns route composition and intelligence orchestration, but not process lifetime.
- **Cli** is the production host executable and thin terminal client.
- **Compendium** edits `arcanum.json` only; it never runs inference or opens the Grimoire.
...
|---|---|---|
| One binary, hybrid process model | Minimize deployment and version skew | A CLI verb selects short-lived client or long-running host behavior |
| API-first product boundary | Keep every UI/client replaceable | Business behavior is exposed through `/api` or `/v1`, not embedded in clients |
| Native AOT is a hard constraint | Zero runtime prerequisite and reduced reflection surface | Source-generated JSON/RDG/config binding and hand-authored tool schemas are mandatory |
| Strict `Cli → Api → Infrastructure → Core` dependencies | Keep ownership and compile-time boundaries honest | Lower layers cannot depend on web or UI types |
| OpenAI-compatible HTTP providers only | One native provider abstraction without managed model lifecycle | Ollama is supported through its `/v1` endpoint; no local weight manager exists |
| One semantic turn engine | Prevent buffered/streaming behavior drift | Projections differ, but gates, context, tools, accounting, and finalization are shared |
| `IModelCallExecutor` is the sole model-I/O boundary | Centralize admission, usage, cache, reasoning, and accounting rules | Main calls, continuations, retries, routing, and extraction follow the same controls |
| Local encrypted Grimoire | Durable local-first state with no cloud dependency | SQLCipher, PBKDF2 key derivation, compiled EF model, and raw-SQL extensions coexist |
| Stable error codes and `Result<T>` | Avoid exception-message contracts | HTTP status mapping is centralized in `ArcanumErrorMapper` |
| Wards and Sanctum are orthogonal | Separate human approval from policy enforcement | A Ward-approved action may still be blocked by Sanctum |
| Reasoning is ephemeral and answer-separated | Prevent hidden/sensitive chain-of-thought persistence | Only client-safe projections and token counts leave the in-memory boundary |
| Usage is recorded per completed provider call | Preserve billable work across retries and later failures | Daily spend uses billable operations plus outstanding reservations, not session totals |
| Prompt caching is not response caching | Preserve provider semantics and tool correctness | Arcanum always invokes the model and emits only verified provider cache controls |
| RAG features degrade gracefully | Optional embeddings must not break ordinary chat | Managed cosine search replaces unavailable `vec0`; retrieval failures skip context |
| Workspace MCP requires byte-level trust | Prevent unapproved local code execution | Trust is bound to the current `mcp.json` digest, not path or timestamp |
| Compendium is configuration-only | Keep desktop UI low-risk and decoupled | It references Core, preserves unknown metadata, and never touches inference/persistence |
| Documentation changes with code | Keep architectural intent reviewable | Architecture, contract, persistence, config, and CLI changes update docs in the same PR |

---

## Request and response mental model

1. HTTP binding/authentication validates the request and establishes idempotency behavior.
2. `WizardIntelligenceProvider` creates a `TurnExecutionRequest` and delegates to the coordinator.
3. Shared preflight validates guardrails, attachments, request bounds, vision capability, prompt,
   and budget before model I/O.
4. Context is assembled once from the Session, Codex, routed Spell, RAG, Saga, Lexicon,
   attachments, Campaign, MCP catalog, Ward policy, and Sanctum policy.
5. Before every provider call, context tokens, dollar reservations, and per-turn call limits are checked.
6. The model either returns a final answer or tool proposals. Server tools pass Ward then Sanctum,
   execute, append call/result messages, and cause another model round.
7. Structured output, output guardrails, Grimoire finalization, usage projection, Saga enqueue,
   metrics, and audit complete the turn.
8. Exactly one projection materializes the semantic events as buffered native JSON, native NDJSON,
   or production OpenAI SSE/JSON.

---

## Configuration mental model

- Operator settings bind below the top-level `Arcanum` object in `arcanum.json`.
- Environment variables use `ARCANUM_` plus `__` nesting and override file values.
- Numeric values are clamped at each use site; startup validation rejects semantic conflicts.
- `Compendium` mirrors settings through `SettingDescriptor` metadata and polished section editors.
- Secrets are Data-Protection encrypted as `dp:v1:` values and redacted as `***` over the API.
- Campaign Sanctum settings are stored in the Grimoire, not in `arcanum.json`.

---

## Consolidated source parts

The following parts preserve the complete detailed design. Their local table of contents, links,
examples, defaults, ranges, and caveats are part of this handbook.
