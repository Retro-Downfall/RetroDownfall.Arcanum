...
That claim authorizes later work; it does not say the full reset has finished. The marker-pair and Campaign compare-delete checkpoint is a separate stage, and remaining managed-file reconciliation, retained restore credentials, and installation-identity rotation are a further one. Authorization therefore leaves the tainted installation closed and reports no deletion it has not yet proved.
