...
After confirmation, global/all writes an owner-only V1 `Prepared + HostFactoryErasure` record. Its operation identity is only the replay name sent to the host; the server creates and returns a different durable operation identity. The command stores a monotonic, content-free proof of that exact operation's completion before it asks the host to stop, then takes the maintenance lock and continues offline: `prepare -> host apply/replay -> proof -> shutdown -> lock -> offline continuation`. A plan change proven before effects may retire the handoff; uncertain outcomes keep it for replay. Active resets normally block `serve`; only a proof-absent global/all prepared handoff may temporarily start the host to finish that replay, and proof-bearing or later states block again. Requested identity never becomes a root operation, server identity, gate owner, or lease owner.
