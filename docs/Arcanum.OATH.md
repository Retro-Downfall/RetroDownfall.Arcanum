...
**Document status:** current as of **2026-08-23**, reconciled through GitHub issue #128 and issue #123's full-reset managed-file reconciliation and restore terminal-state proof.

**Branch status.** This #122 update is intentionally branch-local on `long-term-memory` until the normal project mirror reaches `main`; delivery of this issue does not merge or rewrite `main`. Read this copy together with the implementation branch. The implementation and the `Arcanum.DESIGN.md` sections it cites (§10.10–§10.20.13) live on `long-term-memory`; §22 marks those links.
...
| **#85** | Landed | `ProviderToolCallBuffer`, `CovenantToolInvocationContext` and `CovenantToolCapabilityRegistry`, `CovenantEgressWardPolicy`, `CovenantToolEgressGuard`, and the hand-authored `propose_covenant` / `retire_covenant` tools — registered inert. |
...
| **#123** | Partially landed | Full-reset managed-file reconciliation, wired and reachable: the four-phase authenticated checkpoint nested in the marker-pair checkpoint and null until the Campaign receipt is terminal, the source and work-item inventory vectors, the terminal classification and content-free per-arm blocker digests, the write-intent recovery that terminalizes every unfinished managed write by created-child physical identity, two stopped-host overloads on the existing erasure kernel adding no second opener or delete algorithm, and a journal proof and one-shot authority reasserted before every transaction and filesystem effect. The two-armed restore terminal-state proof and the ordered anchor, journal-key, installation-identity compare-removal are built and proven but deliberately unreached until something deletes the Grimoire. Grimoire deletion, joined nonrevocable disclosure-evidence removal, and identity rotation remain unbuilt. |
...
| Issue | Size | Role |
|---|---|---|
| **#94** | XL | Covenant family erasure and data-lifecycle activation are implemented through the #128 contract; #120–#122 secure installation-reset recovery, external authorization, and the exact marker-pair and Campaign-cleanup effects, and #123 adds managed-file reconciliation plus the restore terminal-state proof and credential-removal components. What remains of #123 is the terminal half: Grimoire deletion, joined nonrevocable disclosure-evidence removal, and installation, path-identity, recovery-envelope, and authority identity rotation. |
...
| **#123** | XL | Grimoire deletion, joined nonrevocable disclosure-evidence removal, and installation, path-identity, recovery-envelope, and authority identity rotation, gated on the landed `TerminalInventoryVerified` and `VerifiedAbsent` proofs. |
...
2. [`Arcanum.DESIGN.md`](Arcanum.DESIGN.md) describes the shipped architectural contract — §10.10 through §10.20.13 own the Covenant slices.
...
| **Profile namespace** | Digest derived from a profile root's retained no-follow parent handle, carrying no path text, used to namespace restore-journal and reset-active credentials. |
...
- [`Arcanum.DESIGN.md`](Arcanum.DESIGN.md): shipped architecture, persistence, runtime, security, testing, and implementation evidence. Covenant slices are §10.10 through §10.20.13 **(branch)**:
  ...
  - §10.20.1–§10.20.13 Retention, reset, and full erasure
