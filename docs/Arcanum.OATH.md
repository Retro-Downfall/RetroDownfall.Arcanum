...
**Branch status.** This #122 update is intentionally branch-local on `long-term-memory` until the normal project mirror reaches `main`; delivery of this issue does not merge or rewrite `main`. Read this copy together with the implementation branch. The implementation and the `Arcanum.DESIGN.md` sections it cites (§10.10–§10.22.4) live on `long-term-memory`; §22 marks those links.
...
2. [`Arcanum.DESIGN.md`](Arcanum.DESIGN.md) describes the shipped architectural contract — §10.10 through §10.22.4 own the Covenant slices.
...
- [`Arcanum.DESIGN.md`](Arcanum.DESIGN.md): shipped architecture, persistence, runtime, security, testing, and implementation evidence. Covenant slices are §10.10 through §10.22.4 **(branch)**:
  ...
  - §10.22.1–§10.22.4 The operator's write path
