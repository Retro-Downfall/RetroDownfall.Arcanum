...
- **The operator's ability to curate is now partial, and where it stops is the boundary rather than the whole of it.** The Covenant carries exact-version correction, pinning, unpinning, and scope masks over its own prepare-and-commit protocol, and Saga carries correction, retirement, reinstatement, and pinning over one memory at a time — each proved against the exact content the operator read, and each naming the single store it changes. The Lexicon still offers only deletion of one named entity, and no store offers a review queue, a bulk action, or a way to act on a search hit directly.
