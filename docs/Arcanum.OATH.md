...
| Issue | Size | Role |
|---|---|---|
| **#92** | XL, partly delivered | Performance, Native AOT, documentation, and release qualification — the #74 acceptance gate. The reproducible workload, benchmark host, statistics, baseline, and absolute gates are built and run against the production services (`scripts/benchmark-covenant.sh`, DESIGN §10.24). What remains is the evidence nobody on a Mac can produce: `win-x64` and `win-arm64` have no checked-in hermetic SQLCipher asset, so their Native AOT and runtime evidence is bound to the Windows verification workflow, and the five independent review passes the slice requires have not been run. |
