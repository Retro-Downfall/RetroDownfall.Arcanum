...
| **#117** | Landed | `ICovenantSensitiveArtifactPurger` and its coordinator over the shared erasure kernels, `RequireConditionalSensitivityRetentionPurge` with its write-once request-scoped authority, all six deletion routes dispatching through it, page-walking bulk Saga and embeddings-reset paths, `ICovenantLabeledArtifactGuard` on the three legacy raw deletes, and the destructive-disclosure ordering on `data reset-memory`. The guard now sits on the retention paths as well — the prune's Entry, Saga and Lexicon candidates, `DeleteSessionAsync`, and the untargeted arm of `ApplyMemoryResetAsync` — where nothing dispatches through the purge boundary first, so there it is the only check. |
...
- [`README.md`](../README.md): curated public GitHub repository overview. It is not the running implementation-status record; issue acceptance criteria, implementation plans, review evidence, and delivery records own that truth.
