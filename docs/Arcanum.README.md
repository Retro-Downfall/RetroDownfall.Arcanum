...
**Covenant status.** Issue #79 supplies the pure-Core protocol foundation tracked by umbrella issue #74. It has no public endpoint, CLI command, configuration key, persistence schema, or provider-call wiring in this slice. Issue #84 owns provider-specific token measurement and runtime context-pressure selection.
