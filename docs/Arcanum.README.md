...
- In-process file/dir tools enforce **path containment + symlink resolution** and **handle-based revalidation** (pre-open path identity vs opened fd dev/ino) for read/write tools; MCP `RequestTimeoutSeconds` must be ≥ `ExecuteCommandTimeoutSeconds`; workspace `mcp.json` servers are registered only after operator trust; `execute_command` uses `ArgumentList` (no shell); **tool children** (`execute_command` / `run_spell_script`) use an MVP **OS filesystem jail** under **macOS-ARM beta posture**: deprecated `sandbox-exec` Seatbelt on macOS (active); Linux Landlock code present but **inactive** (fail-closed unless escape hatch); Windows has **no FS jail** (Job Objects only; Sanctum `Enabled`+`EnforcePathBoundary` denies those tools) — **filesystem-only**, not network isolation — with fail-closed denials unless `Arcanum:Security:AllowUnsandboxedToolChildren=true` (does not bypass Windows Sanctum denial); outbound URLs pass an SSRF guard with **DNS-rebind IP pinning** on untrusted egress (`CommLinkWebhook`, `McpHttp`); errors return **sanitized public envelopes** (detail stays in logs).
