...
- In-process file/dir tools enforce **path containment + symlink resolution** and **handle-based revalidation** (pre-open path identity vs opened fd dev/ino) for read/write tools; MCP `RequestTimeoutSeconds` must be ≥ `ExecuteCommandTimeoutSeconds`; workspace `mcp.json` servers are registered only after operator trust; `execute_command` uses `ArgumentList` (no shell); **tool children** (`execute_command` / `run_spell_script`) run under an MVP **OS filesystem jail** (Landlock on Linux, deprecated `sandbox-exec` on macOS; Windows denies those tools when Sanctum `Enabled`+`EnforcePathBoundary`) — **filesystem-only**, not network isolation — with fail-closed denials unless `Arcanum:Security:AllowUnsandboxedToolChildren=true`; outbound URLs pass an SSRF guard with **DNS-rebind IP pinning** on untrusted egress (`CommLinkWebhook`, `McpHttp`); errors return **sanitized public envelopes** (detail stays in logs).
