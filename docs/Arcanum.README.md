...
**Wire contracts:** `ApiResponse<T>` for `/api` JSON; **NDJSON** for `ping-stream`, spell/prompt `execute-stream`, and `llama/models/pull`; **SSE** for `/api/events/*`, `/api/sessions/{id}/stream`, and Chronicle; **OpenAI JSON/SSE** for `/v1/*`. See [DESIGN.md §8](Arcanum.DESIGN.md#8-http-json-and-minimal-api-design-api-project).
