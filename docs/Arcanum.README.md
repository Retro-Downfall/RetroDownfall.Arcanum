...
Inference flows through one hub behind a single `IChatClient` abstraction. See [DESIGN.md §10](Arcanum.DESIGN.md#10-intelligence-pipeline).
...
- **Providers (`AiProviderKind`):** `OpenAICompatible` (`Microsoft.Extensions.AI.OpenAI` — DeepSeek, Groq, GitHub Models, LM Studio, Ollama via its `/v1` endpoint, …).
...
- In-process file/dir tools enforce **path containment + symlink resolution** and **handle-based revalidation** (pre-open path identity vs opened fd dev/ino) for read/write tools; MCP `RequestTimeoutSeconds` must be ≥ `ExecuteCommandTimeoutSeconds`; workspace `mcp.json` servers are registered only after operator trust; `execute_command` uses `ArgumentList` (no shell); **tool children** (`execute_command` / `run_spell_script`) use an MVP **OS filesystem jail** under **macOS-ARM beta posture**: deprecated `/usr/bin/sandbox-exec` Seatbelt on macOS (active); Linux Landlock / internal `__sandbox-exec` helper present but **inactive** (fail-closed: *"Linux filesystem jail is not active in this beta…"* unless escape hatch); Windows has **no FS jail** (Job Objects only; health **Degraded**; Sanctum `Enabled`+`EnforcePathBoundary` denies those tools) — **filesystem-only**, not network isolation — with fail-closed denials unless `Arcanum:Security:AllowUnsandboxedToolChildren=true` (does not bypass Windows Sanctum denial). Inspect with `arcanum doctor` (Tool Child Sandbox) or `GET /api/health` `ToolChildSandbox`; outbound URLs pass an SSRF guard with **DNS-rebind IP pinning** on untrusted egress (`CommLinkWebhook`, `McpHttp`); errors return **sanitized public envelopes** (detail stays in logs).
...
docs/                                    # all project documentation lives here
  ...
  chat-loop.md                           # chat loop workflow (mermaid + walkthrough)
...
Default base `http://localhost:5001`. **All `/api` and `/v1` routes require the API key** (`X-Arcanum-Key: <KEY>` or `Authorization: Bearer <KEY>`). This is a grouped overview — the exhaustive per-endpoint table (verbs, status codes, payload DTOs) lives in [DESIGN.md §4.3](Arcanum.DESIGN.md#43-retrodownfallarcanumapi-class-library-not-executable).

| Area | Routes | Notes |
|------|--------|-------|
| Metrics | `GET /metrics` | Prometheus text format (`text/plain; version=0.0.4`). Canonical path outside `/api`/`/v1`; **API key required by default** (`X-Arcanum-Key` or `Authorization: Bearer <key>`). Set `Arcanum:Metrics:RequireApiKey=false` only for unauthenticated scrapes on a loopback bind — forced on automatically when the host binds all interfaces. See [DESIGN.md §8.22](Arcanum.DESIGN.md#822-metrics-endpoint-get-metrics). |
| Health & meta | `/api/health`, `/api/meta`, `/api/grimoire/stats`, `/api/budget` | `health` returns `ApiResponse<HealthReportDto>` with Grimoire/MCP/provider components (HTTP 200 for healthy/degraded). `meta` adds `uptime` and `nativeAot`. `grimoire/stats` returns db/WAL sizes and row counts. `budget` returns `ApiResponse<BudgetSummaryDto>` with enabled flag, daily limit, today's spend, remaining, spent percent, and alert threshold. |
...
| OpenAI compat surface | `POST /v1/moderations`, `POST /v1/images/*`, `POST /v1/audio/*` | `/moderations` is a disabled-by-default (`Arcanum:Moderations:Enabled`) pass-through stub — always unflagged when enabled, `404 feature_disabled` when not. `/images/*` and `/audio/*` always return `501 not_supported` — not implemented yet, no config toggle. See [DESIGN.md §11.18](Arcanum.DESIGN.md#1118-openai-moderations-post-v1moderations) / [§11.19](Arcanum.DESIGN.md#1119-openai-images-and-audio-stubs-post-v1images-post-v1audio). |
| Files | `POST/GET /v1/files`, `GET/DELETE /v1/files/{id}`, `GET /v1/files/{id}/content` | Standalone upload storage (multipart), feeding `/v1/batches`. Bytes on disk under a fresh GUID name (never the client filename); download always `Content-Disposition: attachment`. See [DESIGN.md §11.20](Arcanum.DESIGN.md#1120-openai-files-v1files). |
| Batches | `POST/GET /v1/batches`, `GET /v1/batches/{id}`, `POST /v1/batches/{id}/cancel`, `POST /v1/batches/{id}/reset` | Async bulk chat-completion over an uploaded JSONL file (Phase 1: `/v1/chat/completions` only). Processed out-of-band by `BatchProcessingService`; startup `StartAsync` reconciles stranded `in_progress` batches via `IBatchRecoveryService` (shared with `/reset`). See [DESIGN.md §11.21](Arcanum.DESIGN.md#1121-openai-batches-v1batches). |
| Sessions (Grimoire) | `/api/sessions/*` (CRUD, `/entries`, `/export`, `/rest` (202 queued / 503 `Session.RestQueueFull`), `/stream`, `/attachments`, `/analytics`, `/divine`, `/fork`), plus memory-management routes: `DELETE /entries/{entryId}`, `POST/DELETE /entries/{entryId}/pin`, `POST /compact` | Single source of truth for threads; FTS5 search; SSE live stream. `GET /entries` returns entry history with keyset pagination and optional `?countOnly=true` for an efficient count. `GET /attachments` lists **bound** session attachments (`SessionAttachmentDto[]`, incl. `RelativePath`) for Command Center list/Reveal/re-attach — bytes live under `~/.config/arcanum/attachments/` (see [DESIGN.md §10.2.5](Arcanum.DESIGN.md#1025-session-attachments-disk--grimoire-pointers), [PERSISTENCE.md §13](Arcanum.PERSISTENCE.md#13-session-attachments-attachments--sessionattachments)). Memory-management endpoints are gated by `Arcanum:Sessions:AllowMemoryManagement` (default `false`); pinning is bounded by `Arcanum:Sessions:MaxPinnedEntries`; pinned entries survive read-time context compression. `POST /divine` is RAG Phase 2 semantic search over embedded entries (disabled by default; see [DESIGN.md §21.6](Arcanum.DESIGN.md#216-phase-2--session-divination)). `POST /fork` branches a session (optionally truncated at an entry) into a new independent session — **Bound attachments copy with the fork** (remapped ids; cutoff excludes `EntryId`-null) — tracked via `ForkedFromSessionId` and bounded by `Arcanum:Sessions:MaxForkDepth` (see [DESIGN.md §11.16.1](Arcanum.DESIGN.md#11161-session-forking-post-apisessionsidfork)). |
| Lore | `/api/lore/*` | **Legacy / operator-only** key-value memory (`MageSettings`). The Lore MCP tools are removed; agent memory is The Lexicon (see [DESIGN.md §10.6](Arcanum.DESIGN.md#106-the-lexicon--agent-directed-entity-memory)). |
| Saga | `/api/saga/*` (list, `/divine`, delete single/all, `/stats`) | RAG Phase 4 — auto-extracted long-term associative memory (disabled by default; see [DESIGN.md §21.8](Arcanum.DESIGN.md#218-phase-4--saga-long-term-associative-memory)). `GET`/`DELETE`/`/stats` are not gated on `SagaEnabled`; `POST /divine` requires `Arcanum:Embeddings:Enabled` + `SagaEnabled`. |
| Spells | `/api/spells/*` (CRUD, `/search`, `/validate`, `/export`, `/import`, `/execute(-stream)`, `/versions` (list/get/create/update/activate), `/clone`, `/cast`) | Built-in spells are read-only (`source: builtin`). Structured sidecar is canonical **`SPELL.json`** (legacy **`SKILL.json`** is still **read** when the canonical file is absent; writes always create/update `SPELL.json`). `SPELL.json` `dependencies` and `declaredTools` affect **execution** (Arcane Resonance + Artifact Attunement), not just validation. List responses include optional `isValid` and `unresolvedDependencies` when Arcane Resonance deps are missing from the catalog. `/versions` uses **string** labels (`SPELL.v{label}.md`); `GET .../versions/{version}` returns the editable body only (use `(active)` for `SPELL.md`); `/clone` copies a spell to a new name; `/cast` is a dry-run system-prompt preview (no inference). |
| The Forge — campaigns | `/api/campaigns/*` (+ `/codex`, `/export`, `/import`, `/spells`, `/prompts`, `/sessions`), `/api/codex` | Registers workspace roots; creates `.arcanum/`. `/spells`, `/prompts`, `/sessions` list resources scoped to the campaign. Delete unregisters the Grimoire row only (disk files remain). Campaign export spell payloads use wire `spellJson` (legacy `skillJson` accepted on import). The Forge Atelier UI can create/edit/delete/export/import campaigns via these routes. |
...
| The Conclave — A2A | `/api/conclave/a2a/*` (JSON-RPC + `/agent-card`) · `dispatch_sending` tool | A2A (Agent-to-Agent) protocol surface, additive to The Conclave. **Disabled by default** — requires `Arcanum:Conclave:Enabled` **and** `Arcanum:Conclave:A2A:Enabled`, plus `ServerEnabled`/`ClientEnabled` respectively. Inbound A2A messages spawn headless Apprentices (server side); `dispatch_sending` delegates outward to a remote A2A agent (client side, blocking). See [DESIGN.md §5.7.1](Arcanum.DESIGN.md#571-a2a-and-the-conclave). |
| Wards & Sanctum | `/api/wards/*`, `/api/campaigns/{campaignId}/sanctum(/breaches)` | Forbidden Arts gating + per-campaign sandbox: path/network/tool policy plus OS-enforced CPU time, memory, and open-file-descriptor limits (setrlimit on macOS, cgroups v2 with setrlimit fallback on Linux, Windows Job Objects for CPU/memory/`MaxProcessCount`) on the child processes spawned by `execute_command` and `run_spell_script`. Those children also use an MVP **filesystem jail** under macOS-ARM beta posture: deprecated `sandbox-exec` on macOS (active); Linux Landlock **inactive** (fail-closed); Windows **no FS jail** (denies when Sanctum `Enabled`+`EnforcePathBoundary`) — **not** network isolation. Escape hatch: `Arcanum:Security:AllowUnsandboxedToolChildren` (default false; does not bypass Windows Sanctum denial). Breach history — including resource-limit breaches — is Grimoire-backed (`SanctumBreaches` table, survives host restart) with paginated `GET .../breaches` (`limit`, `before`, `tool`) and per-campaign retention (`SanctumConfig.MaxBreachCount`). |
| MCP | `/api/mcp` (list), `/api/mcp/{name}` (status), `/api/mcp/*` (`/start`, `/stop`, `/restart`, `/reload`, `/trust-workspace`), `/api/mcp/tools/invoke` | Manage external + in-process MCP servers, plus **Diagnostic MCP Invocation** (`POST /api/mcp/tools/invoke`): policy-constrained direct invoke of an **external** MCP tool by an operator. External servers only; the internal `arcanum-internal` server and all Forbidden Arts are blocked. Not model execution; not unauthenticated (inherits `X-Arcanum-Key`). See [DESIGN.md §11.28](Arcanum.DESIGN.md#1128-diagnostic-mcp-invocation-post-apimcptoolsinvoke). |
| Workspaces | `/api/workspaces/*` (+ `/files`, `/files/info`, `/files/contents`, `/files/directory`, `/files/divine`, `/files/index`, `/files/index/status`, `/files/chunks`) | Registry + file browser (read) plus write/modify/delete (`PUT`/`PATCH .../files/contents`, `DELETE .../files`, `POST .../files/directory`) gated by `Arcanum:Workspaces:EnableFileWrite` (default `false` — **403** `Workspace.FileWriteDisabled` when off). `POST .../files/divine` and `.../files/index` are RAG Phase 3 semantic codebase retrieval and manual re-index (disabled by default; see [DESIGN.md §21.7](Arcanum.DESIGN.md#217-phase-3--semantic-codebase-retrieval)). `GET .../files/index/status` and `.../files/chunks` are RAG Phase 7 read-only inspection of the indexed substrate (status counts/dimensions + paginated 500-char-capped chunk previews; registry-only, never trigger indexing). `HEAD .../files/contents` returns `Content-Length` and `Last-Modified` with an empty body for size/freshness checks. |
...
| The Proving Grounds | `POST /api/proving-grounds/trials/run` | Ephemeral **Trial** runner: targets a Spell, Prompt, or Apprentice Goal and adjudicates output with **Inquisitors** (`regex`, `jsonSchema`, `semantic`). Returns `ApiResponse<TrialResult>`. The Forge exposes a singleton Workbench UI over this route (no Trial persistence). |
...
- **RAG and Semantic Retrieval (disabled by default, all five phases implemented):** `Arcanum:Embeddings:Enabled` gates **The Weave**, Arcanum's embedding and vector substrate, and **Divination**, semantic search over it. `IWeaveService` imprints text into vectors via a per-provider `IEmbeddingGeneratorFactory` (OpenAI-compatible providers, including Ollama via `/v1`); `IDivinationService` runs cosine-similarity KNN search, accelerating to a sqlite-vec `vec0` index when available and otherwise falling back to a managed cosine scan (SIMD-vectorized via `Vector<float>`, streaming top-K heap, hardcoded internal row budget of 50,000 — not an `Arcanum:*` setting; zero functional difference from the `vec0` path within that budget). **Phase 2** (`Arcanum:Embeddings:SessionSearchEnabled`) imprints Grimoire entries in the background (`EntryWeavingService`) for `POST /api/sessions/divine` / `arcanum session divine`. **Phase 3** (`Arcanum:Embeddings:CodebaseRetrievalEnabled`) indexes workspace files in the background (`WorkspaceIndexingService`) and, when enabled, `WizardIntelligenceProvider` retrieves semantically relevant chunks for the current prompt and injects them into the system prompt as a `### Semantic Context (Retrieved Codebase)` DATA section before every inference turn with a non-empty working directory. **Phase 4 — Saga** (`Arcanum:Embeddings:SagaEnabled`) auto-extracts long-term associative memories from inference conversations in the background (`SagaExtractionService`, event-driven) and, when enabled, injects semantically relevant memories as a `### Saga (Associative Memory)` DATA section; browse/search/delete via `/api/saga/*`, `read_saga` (MCP, read-only), and `arcanum saga`. **Phase 5 — semantic spell routing** (`Arcanum:Embeddings:SemanticSpellRoutingEnabled`) adds an embedding-based pre-filter in front of the existing LLM-based spell router: pure mode picks the highest-similarity spell with no LLM call, hybrid mode (`SpellRoutingHybridMode`) narrows the catalog to the top `SpellRoutingHybridTopK` candidates before the LLM router picks. Every phase has zero behavior change when disabled (the default). See [DESIGN.md §21](Arcanum.DESIGN.md#21-the-weave-divination-and-saga-rag).
- **The Lexicon — agent-directed entity memory (enabled by default):** replaces the legacy key-value Lore MCP tools for model use. The `scribe_lexicon` / `delete_lexicon` MCP tools write typed entities (Name + Type + Facts) to a raw-SQL `lexicon_entries` table with an FTS5 `lexicon_fts` index (no EF migration; bootstrapped alongside The Weave; FTS is rebuilt on schema ensure). On every user-facing inference turn, `SemanticRouter` extracts `entities` alongside `spellName`; when routing is bypassed (`OverrideSpellName`, pure embedding routing, no-spell turns), a low-token `LexiconEntityExtractor` preflight runs so memory retrieval stays available (Unseen Servant override-spell ticks skip that extractor because Previous State is already in the kickoff). Matched entities (exact-name then column-weighted `bm25`) are injected into the system prompt under DATA as `### Lexicon (Known Context)`, hardened so facts cannot create headings or break DCI structure — including Unseen Servant Previous State kickoff text. Read-time context compression rebuilds the system prompt **with** Lexicon, Semantic Context, and Saga still attached so those DATA sections are not dropped at the compression threshold. **Wire/config behavior change (Option A):** the Lore MCP tools are removed; `EnableLoreSystem` no longer gates any MCP tool. Lexicon is gated by **`EnableLexiconSystem`** (default **true**). On bind, Arcanum auto-migrates in memory: `delete_lore` → `delete_lexicon` in ForbiddenArts, and `EnableLoreSystem: false` forces `EnableLexiconSystem: false` (startup warning; persist `arcanum.json` to keep the change). Operators may also set **`EnableLexiconSystem: false`** explicitly. `delete_lexicon` cannot remove `daemon_state:*` Unseen Servant entries. See [DESIGN.md §10.6](Arcanum.DESIGN.md#106-the-lexicon--agent-directed-entity-memory).

---

## Configuration

Settings bind under the `Arcanum` object in **`arcanum.json`**, living in the per-user config dir (created on first run): `~/.config/arcanum/` on macOS/Linux, `%USERPROFILE%\.config\arcanum\` on Windows. Override any key with env vars using the **`ARCANUM_`** prefix and `__` for nesting (use env vars for secrets — e.g. `ARCANUM_Arcanum__Providers__1__ApiKey`). Every numeric setting has a runtime clamp in `ArcanumSettingClamps`. On `arcanum serve` startup the configuration is validated **before serving**, and the host aborts with a clear logged message (not a crash) when settings are semantically invalid — an unknown default/fast model, an MCP timeout / JSON-RPC ordering conflict, or a missing/relative allow-list root. Obsolete removed managed-local and prompt-cache keys are also hard-rejected so they cannot be silently ignored after unbinding.

> **Compendium** — a .NET 10 Avalonia desktop editor for `arcanum.json` is available at `src/RetroDownfall.Compendium.Ux`. It reuses Core models, supports System Light/Dark modes, and interoperates with Arcanum's DataProtection-encrypted provider keys. See [`docs/Compendium.README.md`](Compendium.README.md).

**The full key reference (types, defaults, clamps) is [DESIGN.md §3.4](Arcanum.DESIGN.md#34-configuration-reference-arcanumsettings).** Sections at a glance:

| Section | Controls |
|---------|----------|
| `Arcanum:Host` | Kestrel HTTP port, optional HTTPS (`Host:Https`), CORS, body cap, rate limiter, Scalar UI toggle, system fingerprint, default workspace, loopback vs `ListenAny`. |
...
| `Arcanum:Intelligence` | Tool timeouts/caps, **`InferenceTimeoutSeconds`** (default 600; wall-clock cap per inference turn), agentic round cap, **Lexicon** gates/bounds (`EnableLexiconSystem` default true, `LexiconMaxMatchedEntries`, `LexiconMaxInjectedBytes`; see [DESIGN.md §10.6](Arcanum.DESIGN.md#106-the-lexicon--agent-directed-entity-memory)) and legacy `EnableLoreSystem` (operator-only, no longer gates MCP tools), archive gate, context compression (`ManaPreflight` LRU), `SemanticRouterMaxTokens` (default 128 — bumped from 50 to cover extracted `entities`), optional `UseFastModelForSpellRouting`, tokenizer encoding, token tracking. **Injection bounds (enforced):** `MaxPingPromptChars`, `MaxStatelessMessages`, `MaxOpenApiMessages`, `MaxPlanSteps`, `ArchiveSearchMaxQueryLength`. |
...
| `Arcanum:Ward` / Sanctum / `Arcanum:Security:AllowUnsandboxedToolChildren` | Forbidden Arts list, ward timeout, `MaxActiveWards` (default 50), unattended auto-deny; per-campaign Sanctum config, including OS-enforced CPU/memory/file-descriptor limits (`SanctumConfig.ResourceLimits`, not bound from `Arcanum:*` — set per campaign via the Sanctum API); tool-child FS jail escape hatch (default false; macOS Seatbelt active / Linux Landlock inactive for beta / Windows no FS jail; filesystem-only, not network). |
...
| `Arcanum:Metrics` | `GET /metrics` Prometheus endpoint. `Enabled` (default `true`) toggles the route (`404` when `false`). `RequireApiKey` (default `true`) attaches `ApiKeyEndpointFilter` to the same `/metrics` path (`X-Arcanum-Key` or `Authorization: Bearer`); set `false` only for unauthenticated scrapes on loopback — forced to effectively `true` whenever the host binds all interfaces. |
...
| `Arcanum:Attachments` | Session attachment persistence (Command Center + host; DESIGN.md §10.2.5). `Enabled` (default `true`) master switch; `MaxReferencesPerTurn` (default 8, clamp 1–32) is a **combined** user `AttachmentReferences` + model `attach_session_file` turn budget; `MaxVersionsPerLogicalKey` (default 20, clamp 1–100); `MaxBytesPerSession` (default 256 MiB, clamp 1 MiB–10 GiB); `PendingRetentionHours` (default 24, clamp 1–168) for startup GC of stale pending rows/`_pending` dirs; `MaxIndexItemsInPrompt` (default 40) / `MaxIndexBytesInPrompt` (default 4096) bound the metadata-only Session Attachments Index; `EnableModelAttachTool` (default `true`) advertises internal MCP `attach_session_file`. Fork carries Bound attachments. Metadata is SQLCipher-encrypted; bytes under `~/.config/arcanum/attachments` are owner-perm only (copy that tree with the DB for full continuity). |
...
| `Arcanum:StructuredOutput` | JSON Schema response validation, retry, and provider-side constrained decoding. `Enabled` (default `true`) is the master toggle; `MaxValidationRetries` (default `2`, clamp 0–10); `UseProviderConstrainedDecoding` (default `true`) injects `strict: true` for OpenAI-compatible providers; `StrictMode` (default `false`) flips best-effort to a hard `400 StructuredOutput.ValidationFailed`; `SchemaMaxDepth` (default `10`, clamp 1–50). See [DESIGN.md §22.1](Arcanum.DESIGN.md#221-structured-output-enforcement-arcanumstructuredoutput). |
...
HTTP remains the default on **loopback**. When `Arcanum:Host:Https:Enabled` is `true` without ListenAny, Kestrel adds a second TLS listener alongside HTTP. When `Arcanum:Host:ListenAny` (or `ARCANUM_HOST_ANY`) is effective, HTTPS is **required** and is the **only** listener (`https://0.0.0.0:{HttpsPort}` — no plaintext any-IP HTTP). API key auth, rate limiting, and CORS apply on HTTPS the same as HTTP. Certificate password is encrypted at rest (`dp:v1:`) and redacted on `GET /api/config`. Clients do not bypass TLS validation — trust the cert in the OS store (Compendium self-signed certs are loopback-SAN only).

**PFX** (password optional; omit `PrivateKeyPath`):
  ...
  "Arcanum": {
    "Host": {
      ...
      "Https": {
        "Enabled": true,
        "Port": 5443,
        "CertificatePath": "/home/me/.config/arcanum/certs/arcanum-localhost.pfx",
        "PrivateKeyPath": null,
        "CertificatePassword": "dp:v1:..."
      }
    }
  }
}
...
**PEM** (unencrypted key only; `CertificatePassword` is ignored):
  ...
  "Arcanum": {
    "Host": {
      "Https": {
        "Enabled": true,
        "Port": 5443,
        "CertificatePath": "/etc/ssl/certs/arcanum.crt",
        "PrivateKeyPath": "/etc/ssl/private/arcanum.key"
      }
    }
  }
}
...
Compendium can generate a local self-signed PFX under `~/.config/arcanum/certs/` (loopback SANs only; not auto-trusted). Encrypted PEM private keys are deferred.
...
**Command Center:** continuous chat/workbench (Terminal.Gui): header (model · session title/short id · API · generating), left **Sessions** sidebar (UpdatedAt desc; filterable; collapses &lt;100 cols → **Ctrl+O** picker), transcript with follow-tail, multiline **composer** (grows **1–10** content rows into the transcript/sessions body then scrolls; **Enter** newline; **Ctrl+Enter** send), focus-aware footer. Resume loads up to **200** recent entries (chronological; older-history paging deferred). Startup restores `cli-session.txt` when valid; otherwise New Session. Shortcuts: **F1** help, **Ctrl+K** palette, **Ctrl+O** sessions, **Ctrl+N** new, **Ctrl+R**/F5 refresh, **Tab** focus, **Enter** / **Ctrl+Enter** (composer), **Ctrl+C** cancel turn / clear input / quit hint, **Ctrl+Q** quit. **HITL:** streamed `ask_human` opens a hard-modal operator question (Enter = newline, **Ctrl+Enter** = submit answer via `POST /api/intelligence/human-response`; closes on timeout/ToolError/terminal frames). Ward approvals remain a separate overlay (pending Wards are denied before Help/Palette/SessionPicker steal focus). Slash allowlist: `/help`, `/keys`, `/status`, `/doctor` (compact), `/clear`, `/mana`, `/tools`, `/model list`, `/provider list`, `/mcp`, `/arsenal`, `/campaign list`, `/session list|new|resume <id>`, `/attach <path>`, `/attachments` (list), `/attachments add <logicalName> [vN]`, `/attachments reveal <logicalName> [vN]`, `/spell list`, `/ward list`, `/exit`/`/quit`; inline `@path` stages a local text file or Scrying image. Denied: `/serve`, all `/daemon …`, `/key …`. Host persists staged attachments when `Arcanum:Attachments:Enabled` (see above); model re-attach via MCP `attach_session_file`. Size gate after TG Init (floor ≥80×12; prefer ≥80×24); too small or init failure → exit **1**. `NO_COLOR` / `ARCANUM_NO_COLOR` theme only. Auto-serve honors `ARCANUM_NO_AUTO_SERVE=1`.
...
**Auto-start serve:** interactive Command Center / `chat` / `ask` ensure a local API host is reachable before the first request. If authenticated `GET /api/health` fails with a definite no-listener signal (connection refused / network unreachable / DNS failure), the CLI spawns `arcanum serve` in the background with `ARCANUM_AUTO_LAUNCHED=1` and waits until authenticated health succeeds (up to ~20s). It does **not** auto-start on TLS/cert failures, probe timeouts, HTTP 503, or 401/403 (auth mismatch). Disable with `ARCANUM_NO_AUTO_SERVE=1`. Auto-start never auto-acks `ListenAny` — run `arcanum serve` manually once to acknowledge, or set `ARCANUM_LISTEN_ANY_ACK=1` if intentional. `NO_COLOR` / `ARCANUM_NO_COLOR` disable color and the live chat layout (and Command Center color theme) only; they do **not** disable auto-serve or the Command Center. Auto-launched servers write an owner-only bootstrap log at `~/.config/arcanum/logs/auto-serve-bootstrap.log` (typically `%USERPROFILE%\.config\arcanum\logs\auto-serve-bootstrap.log` on Windows) and do **not** print the raw first-run API key — retrieve it with `arcanum key show`.

| Command | Purpose |
|---------|---------|
| *(bare)* | Open Command Center (interactive TTY). Non-interactive / `ARCANUM_NO_COMMAND_CENTER=1` → usage, exit 0. |
| `serve` | Run the host on `localhost:5001` by default (writes a PID file). After `StartAsync` and Grimoire readiness (`WaitUntilReadyAsync`), prints and logs the bound address (`http://` on loopback; `https://` on ListenAny). Optional HTTPS is an **additional** loopback listener when `Arcanum:Host:Https:Enabled` is true without ListenAny. Binding all interfaces (`Arcanum:Host:ListenAny` / `ARCANUM_HOST_ANY`) is **HTTPS-only** on `Host:Https:Port` and requires a loadable cert plus **first-run interactive acknowledgement** (or `ARCANUM_LISTEN_ANY_ACK=1` / `ARCANUM_HOST_ANY` for automation). When spawned via auto-start (`ARCANUM_AUTO_LAUNCHED=1`), suppresses the Listening line and the raw key print. |
...
When `serve` is started via interactive auto-launch (`ARCANUM_AUTO_LAUNCHED=1` from `chat`/`ask`), the raw key is **not** printed — retrieve it with `arcanum key show`. The key is still persisted normally.

```bash
dotnet run --project src/RetroDownfall.Arcanum.Cli/RetroDownfall.Arcanum.Cli.csproj -- serve
```

Or simply open the Command Center (auto-starts serve when needed):

```bash
arcanum
# interactive TTY → Command Center
# non-interactive / ARCANUM_NO_COMMAND_CENTER=1 → usage, exit 0
# frameless chat REPL: arcanum chat
```
...
> **Key rotation.** New Grimoire databases use a dedicated encryption secret, so rotating the master API key only invalidates API authentication; the Grimoire `.db` and `.kdf` files can stay in place. Legacy databases encrypted directly from the API key remain destructive to rotate. To retrieve the key later (same machine), run **`arcanum key show`**; to set/rotate it, run **`arcanum key set`**. The key lives in the OS credential store (`arcanum` / `master-api-key`) with a `security.dat` mirror/fallback. See [DESIGN.md §16.3](Arcanum.DESIGN.md#163-security-and-identity).
>
> **Lost Grimoire encryption secret / missing Data Protection key.** If launch FailFasts with “encryption secret cannot be decrypted” or “key verification failed,” `grimoire-key.dat` was sealed with a DP key that is no longer under `~/.config/arcanum/keys/`. Restore the matching `key-*.xml` from backup, **or** reset (destructive — loses session data): delete `~/.config/arcanum/arcanum.db`, `arcanum.db.kdf`, and `~/Library/Application Support/arcanum/grimoire-key.dat`, then run `arcanum` again.
...
API host integration tests (`[Collection("ApiHost")]`) spin up `ArcanumWebApplicationFactory` against the DevHost entry point with a seeded Grimoire database, authenticated `HttpClient`, and `[SkippableFact]` when SQLCipher is unavailable. They cover lore, wards, sessions, apprentices, spell search, workspaces, meta, MCP status, logs, and OpenAI `/v1/models`. The factory sets `ASPNETCORE_ENVIRONMENT=Testing` before the entry point runs; Serilog uses a deferred ring-buffer sink so host `Build()` does not deadlock under WebApplicationFactory (see `docs/tests.README.md`).
...
> **Purely additive tables** that are accessed only via raw SQL (not added to `ArcanumDbContext`'s `DbSet`s) skip the `dotnet ef migrations add` / `dotnet ef dbcontext optimize` workflow below entirely — hand-author the `.sql` file, register it in `GrimoireSqlSchemaMigrator`'s `MigrationOrder`, and stop there. `SanctumBreaches` and `UnseenServantWatermarks` are both folded into the current `InitialCreate.sql` baseline this way: new table, no backfill, no config element changes, no Compendium (`arcanum.json` editor) updates needed — see [`persistence.md`](Arcanum.PERSISTENCE.md) §9. The migration applies on first start with zero risk to existing data.
...
8. **Requires docs in the same change** — update `docs/Arcanum.DESIGN.md` (and this README for operator-visible changes).
...
- **[`DESIGN.md`](Arcanum.DESIGN.md)** — the authoritative deep reference. Quick links: [§3.4 Configuration](Arcanum.DESIGN.md#34-configuration-reference-arcanumsettings) · [§4 Projects](Arcanum.DESIGN.md#4-project-model-and-dependency-graph) · [§8 HTTP/JSON design](Arcanum.DESIGN.md#8-http-json-and-minimal-api-design-api-project) · [§9 Native AOT](Arcanum.DESIGN.md#9-native-aot-and-trimming) · [§10 Intelligence pipeline](Arcanum.DESIGN.md#10-intelligence-pipeline) · [§11 Security](Arcanum.DESIGN.md#11-local-api-security) · [§17 Glossary](Arcanum.DESIGN.md#17-glossary) · [§19 The Forge](Arcanum.DESIGN.md#19-the-forge--campaign-spell-metadata-and-prompt-registry)
- **[`persistence.md`](Arcanum.PERSISTENCE.md)** — which operational state lives in the Grimoire vs. in memory, serialization/migration/retention conventions, and what is intentionally never persisted (Wards, SSE subscriber state, live token streams)
- **[`chat-loop.md`](chat-loop.md)** — the inference chat loop workflow: mermaid diagrams and a step-by-step walkthrough of pre-flight gates, context assembly, the iterative tool-call loop (Ward → Sanctum → invoke → re-inference), streaming wire contract, and loop points
- **[`COMPENDIUM.md`](Compendium.README.md)** — desktop `arcanum.json` editor: project layout, theming, secret interop, and build/run instructions
