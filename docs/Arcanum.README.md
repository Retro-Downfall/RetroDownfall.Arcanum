...
HTTP remains the default. When `Arcanum:Host:Https:Enabled` is `true`, Kestrel adds a second TLS listener (same loopback / ListenAny mode as HTTP). API key auth, rate limiting, and CORS apply on HTTPS the same as HTTP. Certificate password is encrypted at rest (`dp:v1:`) and redacted on `GET /api/config`.
...
| Command | Purpose |
|---------|---------|
| `serve` | Run the HTTP host on `localhost:5001` (writes a PID file). Prints and logs the bound `http://` address before accepting traffic. Optional HTTPS is an **additional** listener when `Arcanum:Host:Https:Enabled` is true (same loopback / ListenAny mode). Binding all interfaces (`Arcanum:Host:ListenAny`) requires a **first-run interactive acknowledgement** (or `ARCANUM_LISTEN_ANY_ACK=1` / `ARCANUM_HOST_ANY` for automation). |
