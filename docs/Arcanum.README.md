...
| Command | Purpose |
|---------|---------|
| `serve` | Run the host on `localhost:5001` by default (writes a PID file). Prints and logs the bound address before accepting traffic (`http://` on loopback; `https://` on ListenAny). Optional HTTPS is an **additional** loopback listener when `Arcanum:Host:Https:Enabled` is true without ListenAny. Binding all interfaces (`Arcanum:Host:ListenAny` / `ARCANUM_HOST_ANY`) is **HTTPS-only** on `Host:Https:Port` and requires a loadable cert plus **first-run interactive acknowledgement** (or `ARCANUM_LISTEN_ANY_ACK=1` / `ARCANUM_HOST_ANY` for automation). |
