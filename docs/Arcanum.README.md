...
- **Effective package pins:** ASP.NET Core / Extensions / EF Core **10.0.10**, `Microsoft.Extensions.AI*` **10.8.1**, `Microsoft.Bcl.Memory` **10.0.10** after per-project overrides, `Microsoft.OpenApi` **2.7.5**, tokenizers **2.0.0**
...
docs/                                    # all project documentation lives here
  ...
  Arcanum.DESIGN.md                      # authoritative architecture reference
  Arcanum.CHAT-LOOP.md                   # chat loop workflow (mermaid + walkthrough)
  Arcanum.PERSISTENCE.md                 # storage, durability, and migration boundaries
  Arcanum.Design.Human.md                # verified non-authoritative consolidated mirror
...
The token-accounting and current persistence/security hardening changes add no Grimoire table or compiled-model change and require no migration or reinstall for a normal upgrade. The reliable editing loop also uses existing `Entries` rows for mandatory `apply_patch` call/result receipts. The notice below applies only to a developer database created before the pre-existing reasoning-accounting install-script change.
