...
**Default command:** bare `arcanum` (no arguments) opens the interactive **`chat`** REPL — the primary UX. Explicit commands (`serve`, `ask`, `--help`, …) behave as before.

**Auto-start serve:** interactive `chat` / `ask` ensure a local API host is reachable before the first request. If authenticated `GET /api/health` fails with a definite no-listener signal (connection refused / network unreachable / DNS failure), the CLI spawns `arcanum serve` in the background with `ARCANUM_AUTO_LAUNCHED=1` and waits until authenticated health succeeds (up to ~20s). It does **not** auto-start on TLS/cert failures, probe timeouts, HTTP 503, or 401/403 (auth mismatch). Disable with `ARCANUM_NO_AUTO_SERVE=1`. Auto-start never auto-acks `ListenAny` — run `arcanum serve` manually once to acknowledge, or set `ARCANUM_LISTEN_ANY_ACK=1` if intentional. `NO_COLOR` / `ARCANUM_NO_COLOR` disable color and the live chat layout only; they do **not** disable auto-serve. Auto-launched servers write an owner-only bootstrap log at `~/.config/arcanum/logs/auto-serve-bootstrap.log` (typically `%USERPROFILE%\.config\arcanum\logs\auto-serve-bootstrap.log` on Windows) and do **not** print the raw first-run API key — retrieve it with `arcanum key show`.
...
Or simply open chat (auto-starts serve when needed):

```bash
arcanum
# equivalent to: arcanum chat
```
