...
> **This document is a dated snapshot, not a live defect list.** It records what the review found on 2026-08-10. A remediation pass has since run against the confirmed list above and fixed the large majority of it, so **no individual entry below should be read as current open state** — the code it quotes has moved, and several entries quote line numbers that no longer exist. Verify against the source before acting on any of them. Entries are deliberately *not* being struck through one at a time: annotating a handful of the 131 would imply the unannotated remainder is still open, which would be more misleading than the uniform snapshot this is. Where an entry's *reasoning* was found to be wrong on its own terms — as opposed to merely fixed since — that is corrected inline and marked as a correction, because a review record that misstates how the code worked is defective as a record regardless of what has since changed.
...
*Failure:* `ArcanumInternalToolServer.ExecuteSearchArchivesAsync` clamps `maxResults` to 1–100 (`ArcanumSettingClamps.ArchiveSearchMaxResults`, ArcanumSettingClamps.cs:79) and `GrimoireLimits`/`SessionSettings.MaxEntryContentBytes` allows entries up to 1 MiB by default and 16 MiB when configured (ArcanumSettingClamps.cs:311). A model-issued `search_archives` on a Grimoire containing large tool-result or pasted-document entries builds a ~100 MB string (≈200 MB as UTF-16) plus StringBuilder growth copies, and the model can re-issue the tool each loop iteration. `ExecuteSearchArchivesAsync` (ArcanumInternalToolServer.LexiconTools.cs:220-228) constructs `McpToolContentTextWire` directly rather than calling `CapToolTextResult` (ArcanumInternalToolServer.Helpers.cs:20-44). *(Correction, recorded during remediation: the finder's original wording — "so `ToolOutputCapBytes` never applies — the allocation happens and the oversized payload is handed to the JSON-RPC writer" — was wrong on the day it was written. `EnforceInProcessToolOutputCap` (ArcanumInternalToolServer.cs:1340, called unconditionally at :1333) measures every tool result against the same effective cap before serialization, so the oversized payload never reaches the writer. The real cost of bypassing `CapToolTextResult` is the other half: the allocation still happens inside the repository, and the response is then replaced wholesale by a generic "Tool output too large" error that names no parameter the caller could change — `search_archives` exposes only `query` — turning every large match into a dead end instead of a truncated-but-useful result.)*
