...
| **In flight** | Retention, reset, and full installation erasure, now split into eight tracked steps. Five are done: governed memory is counted rather than swept, every way to delete something goes through one door that knows which rows are protected, the ten steps a wipe takes now have permanent names so an interrupted wipe leaves a record of how far it got and is dealt with before anything else is allowed to write, the piece that actually runs those ten steps and decides whether the door may reopen now exists and is tested against every point it could crash, and the single step that actually removes the governed rows is written — one transaction that will not begin until it has proved the database will overwrite what it frees, and that leaves the record of what was disclosed, the markers proving which folders are Arcanum's, and the evidence of an earlier escape exactly where it found them. The part that scrubs the file itself is still to come, so nothing is connected to any of it yet. Then the release qualification gate: performance, Native AOT, coverage, security review, and documentation. |
