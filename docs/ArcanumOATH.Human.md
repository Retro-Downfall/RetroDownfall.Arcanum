...
| **In flight** | Retention, reset, and full installation erasure, now split into eight tracked steps. Four are done: governed memory is counted rather than swept, every way to delete something goes through one door that knows which rows are protected, the ten steps a wipe takes now have permanent names so an interrupted wipe leaves a record of how far it got and is dealt with before anything else is allowed to write, and the piece that actually runs those ten steps and decides whether the door may reopen now exists and is tested against every point it could crash — though the part that scrubs the file is still to come, so nothing is connected to it. Then the release qualification gate: performance, Native AOT, coverage, security review, and documentation. |
