...
| **Restore-credential proof** | Arcanum can prove a profile's restore history is over and remove its three restore credentials in a fixed order, one at a time, checking each still holds what the proof said. Nothing reaches that step yet, because it is specified to run after the database is deleted and nothing deletes it. |
