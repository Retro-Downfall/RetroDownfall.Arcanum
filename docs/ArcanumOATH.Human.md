...
Housekeeping is the fourth word, and it is the one that does *not* apply here. Arcanum can be told to throw away old sessions, old attachments, old logs — anything with an age worth forgetting. Governed memory has no such setting, and asking for one is refused rather than accepted and ignored: the agreement between an operator and an agent is not the kind of thing that should quietly expire on a Tuesday, and the records of what has already been deleted are exactly the records that prove the deletion happened. What an operator can now do is *look*: the housekeeping report says how much governed memory exists, how many files it owns, how many sessions it touched, and how many times something has already left the machine — counts only, never the contents. An installation that has never switched the feature on reports nothing at all rather than a row of zeroes, because zero is an answer and "nobody looked" is a different one. Destroying it is still a deliberate, separate act, and this build will not do it yet: asked to, it says so plainly and changes nothing, rather than deleting some of it and leaving the rest. What has been settled is the bookkeeping that destruction will need, and now the part that decides. The machine has the piece that runs a wipe end to end — it closes the door, waits for everyone still reading to finish, stops the one background writer that could still add to the file, hands each thing to the single tool that knows how to remove that kind of thing, and decides at the end whether it is safe to open the door again. It now also has the step that actually removes the rows. That step is one transaction on one connection nothing else is using, and it will not begin until the database has told it, in that moment, that it overwrites what it frees — a promise to make something unrecoverable is worth nothing from a connection that only tidies the index. Everything the family owns goes in that one transaction, so a crash leaves either all of it or none of it, and one fresh identity is stamped on what remains. Four things are deliberately left standing. The record of what has already left the machine, because a disclosure that happened is not undone by deleting the local copy of it. The markers that prove which folders on disk are Arcanum's, because destroying them would strand the folders rather than clean them. The evidence that this installation was once started in a way that could have exposed its keys, because that is the one thing a wipe must never be able to launder. And the schema itself, so what comes back is an empty family rather than a missing one. All four are checked byte for byte, in one place that every path promising to keep them is measured against. What it still does not have is the part that scrubs the file itself, so none of it is connected to anything yet. The rule it follows is worth stating plainly: the door reopens only if nothing was destroyed at all. The moment anything has been, a failure leaves the door shut and the job resumable, because reopening on a wipe nobody finished would hand live work a half-erased file. And it now has the part that scrubs the file afterwards, which is the part that decides whether a wipe may say the bytes are gone. It does not take the database's word for anything: it asks the engine to tidy up and then reads the answer, refusing if the tidy-up was blocked or left work behind. It shrinks the file and then measures whether what is left really is only the pages the file claims to hold, and where it cannot prove that, it writes a whole fresh copy, proves the copy opens and is intact and complete, and only then puts it in place — and if the copy cannot be proved, the original stays exactly where it was. It then looks for every kind of leftover file that a database can leave beside itself, by name, and if one survives it says which kind it was and never where it lives. Last, it opens the result again read-only through a handle that is incapable of creating any of those leftovers itself, reads back the identity the wipe stamped on it, and counts the emptied rows one more time. Only when all of that passes does a wipe get to claim local erasure is complete. Wiping governed memory happens in ten named steps, and the machine now has a permanent, written-down name for each one, so that a crash partway through leaves a record of exactly how far it got instead of a family nobody can tell apart from one that was never touched. A record that claims a step out of order, or a step this build has never heard of, is refused rather than guessed at. Two rules matter more than the list itself: nothing may start wiping anything until that record has actually been written down — the code is arranged so there is no way to skip ahead — and on the next start, a half-finished wipe is dealt with before anything else is allowed to write, so no ordinary work lands in a place that is about to be erased.
...
**Status as of 2026-08-18**, on the `long-term-memory` branch.
...
| Stage | Human summary |
|---|---|
| **Built** | The pure Core Covenant language, normalization, canonical encoding, evidence and sensitivity rules, linking, and admission contracts. The hermetic SQLCipher runtime and connection-authorization foundation. The three-tier database catalogs and their health publication. Canonical storage, the operation gate that closes and drains work before anything destructive, the single mutation kernel, and the degradable search accelerator. Invocation authority and canonical Campaign binding. Prompt placement, token attribution, admission, disclosure-before-dispatch, and one-transaction publication. The two agent tools and their single-use capability protocol. Derived-output labelling and the host-process-tools taint gate. The frozen public and recovery contracts. Maintenance and protected-erasure recovery. The feature gate and the pre-binding authority boundary. Disclosure-aware backup, and a restore that authenticates its own crash-recovery evidence, recovers topology and authority before readiness, reconciles protected state in staging, and refuses to reinstate someone else's memory without an explicit, separately confirmed answer. And the refusal of plaintext export: a session that holds protected material cannot be written out as a readable JSON or Markdown file at all, and a campaign bundle now says in numbers what it left behind instead of leaving it out in silence. The restore command also finally asks the operator for those choices in words they can type, instead of leaving them reachable only from code. And governed memory now has a name in the housekeeping settings — one an operator can look up, and one that deliberately cannot be given a schedule. |
| **In flight** | Retention, reset, and full installation erasure, now split into eight tracked steps. Six are done: governed memory is counted rather than swept, every way to delete something goes through one door that knows which rows are protected, the ten steps a wipe takes now have permanent names so an interrupted wipe leaves a record of how far it got and is dealt with before anything else is allowed to write, the piece that actually runs those ten steps and decides whether the door may reopen now exists and is tested against every point it could crash, the single step that actually removes the governed rows is written — one transaction that will not begin until it has proved the database will overwrite what it frees, and that leaves the record of what was disclosed, the markers proving which folders are Arcanum's, and the evidence of an earlier escape exactly where it found them — and now the part that scrubs the file itself, which checks the answers the database gives it instead of taking them: it refuses if a cleanup was blocked or left anything behind, shrinks the file and then measures whether it really is only the pages it claims, writes a fresh copy and proves that copy opens and is whole before it puts it in place, looks for every kind of leftover file by name and reports which kind survived without ever printing where it lives, and finally reopens the result read-only through a handle that cannot create any of those leftovers itself. Only when all of that passes is the wipe allowed to say the bytes are gone. Connecting it to anything an operator can run is still to come, so none of it is reachable yet. Then the release qualification gate: performance, Native AOT, coverage, security review, and documentation. |
