...
| **In flight** | Retention, reset, and full installation erasure, now split into eight tracked steps. Three are done: governed memory is counted rather than swept, every way to delete something goes through one door that knows which rows are protected, and the ten steps a wipe takes now have permanent names, so an interrupted wipe leaves a record of how far it got and is dealt with before anything else is allowed to write. Then the release qualification gate: performance, Native AOT, coverage, security review, and documentation. |
