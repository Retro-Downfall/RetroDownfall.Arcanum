...
| **In flight** | Retention, reset, and full installation erasure, now split into eight tracked steps. Two are done: governed memory is counted rather than swept, and every way to delete something now goes through one door that knows which rows are protected. Then the release qualification gate: performance, Native AOT, coverage, security review, and documentation. |
