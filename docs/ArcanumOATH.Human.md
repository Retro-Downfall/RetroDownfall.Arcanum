...
For a global or all installation reset, nothing is shown or asked until the authenticated host's current protected inventory has been bound into the local plan. If the host, key, inventory, or exact match is missing, the operation stops before active state or shutdown; workspace reset remains offline. After confirmation, Arcanum writes an owner-only prepared handoff, sends its operation identity to the host only as a replay name, and receives a different server operation identity. It stores content-free proof of that exact completion before shutdown, then takes the maintenance lock and continues offline: prepare, host apply or replay, proof, shutdown, lock, continuation. An uncertain outcome keeps the replay record. Only that prepared record without proof may briefly admit `serve` to finish recovery; every proof-bearing or later record blocks startup again.
...
**Status as of 2026-08-21**, on the `long-term-memory` branch.
