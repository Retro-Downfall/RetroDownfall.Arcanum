...
Even acceptance is deliberately modest. Arcanum stores only encrypted one-way digests proving that this exact operation was authorized; it does not store or print the statement, signature, issuer, nonce, file path, trust root, or key. A new statement must be within its time window. Once the exact claim has been authenticated and stored, that same operation may resume after expiry only if the complete statement and current marker evidence still match; expiry never turns a different statement into valid evidence. Arcanum then stays closed. The next slice owns exact marker and Campaign cleanup, and the following slice owns the remaining files, retained restore credentials, and installation-identity change. Until those complete, a successful authorization is not a successful full deletion, and no output may describe it as one.
