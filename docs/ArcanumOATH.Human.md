...
The production graph and its same-process and fresh-process proofs are built, but public Covenant reset and healthy-catalog factory-erasure entry points still refuse and change nothing. Issue #128 owns connecting them.
...
**Status as of 2026-08-20**, on the `long-term-memory` branch.

OATH combines built foundations with approved work that remains unreachable from public Covenant surfaces. The feature is still off by default.

| Stage | Human summary |
|---|---|
| **Built** | The Covenant language, SQLCipher catalogs, canonical storage, generation-bound authority, disclosure controls, erasure kernels, retention identity, durable ten-phase reset, canonical and local storage erasure, bounded inventory, catalog guard, warm-writer lifecycle, atomic runtime publication, one-shot disposition, startup adoption, recovery handlers, and the shared production composition. Real SQLCipher acceptance proves same-process revocation/reopen and fresh-process adoption/resume. |
| **Still refused publicly** | No Covenant management command or route is registered. `MemoryResetScope.Covenant` and healthy-catalog factory erasure change nothing and retain their safe refusal. Issue #128 owns activation. |
| **Remaining roadmap** | Full installation reset and remediation, release qualification, and later retrieval, rollup, consolidation, curation, evaluation, scoped-recall, caching, delegation, and bitemporal work. |
...
The formal [OATH architecture](Arcanum.OATH.md) carries the precise status. The internal reset/recovery machinery exists and is composed; no route or command turns it on.
