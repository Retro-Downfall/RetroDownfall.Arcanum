...
**Status as of 2026-09-05**, written on `long-term-memory` and since merged into `main`.
...
| **A rebuild inside a reset** | A full reset that has to rebuild the database now writes down that it is starting one before it starts it, under a name the rebuild itself takes up, and writes down what came back when it ends. The rebuild works out that it belongs to that reset by reading the reset's own record, so a machine that restarts halfway through reaches the same conclusion it reached the first time. At startup the two records are read as one answer: agreement carries on, and any disagreement refuses in the same words every time. |
| **Finishing a rebuild the power cut off** | A machine that lost power in the middle of a rebuild used to be a machine that could not start at all until somebody intervened. It now finishes the rebuild on the next start, before it opens the database for anything else, and carries on into a normal start once the rebuild has ended. It refuses only when it genuinely cannot finish — the database will not open with the key this installation holds, what the database says does not agree with what the rebuild's own record says, or the rebuild stops part way and needs a person. A rebuild that was written down but never actually began is finished too, just after the database is opened and before the host says it is ready, rather than by the ordinary catch-up pass that used to run once the host was already answering requests. |
