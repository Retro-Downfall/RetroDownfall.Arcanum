...
| `providers.models.supportsTools` | `bool?`, `null` (undeclared) | — | Declares tool-calling capability. Undeclared is not the same as `false`: only an explicit `false` is authoritative for the streaming no-tools-restart gate, and leaving it unset falls back to a substring match against the provider's own error text. |
