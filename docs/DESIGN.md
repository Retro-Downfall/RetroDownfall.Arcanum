...
- **`SessionRepository`** — implements **`ISessionRepository`** for Forge session CRUD, entry append, export, and analytics. **`AddEntryAsync`** returns **`Result<Entry>`** for expected domain outcomes (not found, archived, entry limits); **`GrimoireLimits.EnforceEntryLimits`** returns **`Error?`** instead of throwing prefixed exception text.
