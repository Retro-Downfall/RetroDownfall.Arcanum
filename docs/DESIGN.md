...
- **`Mcp/ArcanumInternalToolServer.cs`** — **`internal`**: MCP server loop over the opposite channel ends; handles **`initialize`**, ignores **`notifications/initialized`**, stub **`tools/list`** (empty **`tools`**), stub **`tools/call`** (text **`content`**, **`isError: false`**), JSON-RPC **`-32601`** for unknown methods.
...
- **`Mcp/Protocol/McpWireDtos.cs`** — MCP session DTOs serialized only through **`McpJsonSerializerContext`**: **`McpInitializeParams`** / **`McpClientCapabilities`** / **`McpClientInfo`**, **`McpToolsListParams`** (optional **`cursor`**), **`McpToolsCallParams`** (**`name`** + **`arguments`**), **`McpEmptyJsonObject`**, plus in-process server result shapes (**`McpInitializeServerResult`**, **`McpServerCapabilitiesWire`**, **`McpServerInfoWire`**, **`McpToolsListResultWire`**, **`McpToolsCallResultWire`**, **`McpToolContentTextWire`**) used by **`ArcanumInternalToolServer`**.
...
| **`ArcanumInternalToolServer`** | **`internal`**: in-process MCP JSON-RPC loop (**`initialize`**, stub **`tools/list`** / **`tools/call`**, **`notifications/initialized`** ignored). |
