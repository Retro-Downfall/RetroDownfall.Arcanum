...
|---------|---------|--------------------|
| `Microsoft.Extensions.AI.Abstractions` | 10.5.2 | Core |
| `Microsoft.Extensions.AI` | 10.5.2 | Api, Infrastructure |
...
| `Scalar.AspNetCore` | 2.14.11 | Api |
| `Spectre.Console.Cli` | 0.55.0 | Cli |
...
| **`Arcanum:Ollama:ContextWindowLimit`** | `int` | `8192` | Passed to Ollama as **`num_ctx`** via **`ChatOptions.AdditionalProperties`** on each inference **`ChatClient`** call; **`arcanum chat`** uses the same bound for the Mana bar denominator (§4.4). |
| **`Arcanum:Bureau:Enabled`** | `bool` | `false` | Enables Bureau integration when the feature is wired. |
| **`Arcanum:Intelligence:ExecuteCommandTimeoutSeconds`** | `int` | `30` | Hard timeout (seconds) for in-process MCP **`execute_command`** and for native **`run_spell_script`** (**`ArcanumSpellScriptTool`**); runtime clamp **`1`–`600`**. Exposed in the MCP **`execute_command`** **`tools/list`** schema text for the LLM. |
...
| **`Arcanum:Intelligence:EnableLoreSystem`** | `bool` | `true` | When **`false`**, in-process MCP omits **`read_lore`**, **`scribe_lore`**, and **`delete_lore`** from **`tools/list`** and returns a **`ToolError`** if **`tools/call`** targets them anyway. |
| **`Arcanum:Intelligence:EnableArchiveSearch`** | `bool` | `true` | When **`false`**, omits **`search_archives`** from **`tools/list`** and rejects **`tools/call`** for that name with a **`ToolError`**. |
| **`Arcanum:Intelligence:ArchiveSearchMaxResults`** | `int` | `5` | Maximum rows returned per **`search_archives`** call; passed into **`IGrimoireRepository.SearchArchivesAsync`** as **`LIMIT @limit`** after **`Math.Max(1, …)`** in the tool server; **`GrimoireRepository`** clamps **`1`–`500`**. |
| **`Arcanum:Intelligence:CampaignLogThreshold`** | `int` | `25` | Passed to **`IGrimoireRepository.GetConversationsNeedingSummarizationAsync`** as the message-count **safety valve**: conversations enqueue when the count of related **`ChatMessage`** rows with **`Timestamp > (LastSummarizedMessageAt ?? DateTime.MinValue)`** exceeds this value (Campaign Log summarization pipeline; LLM step is a later phase). Also see idle timeout and sweep interval below. |
| **`Arcanum:Intelligence:CampaignLogIdleTimeoutMinutes`** | `int` | `240` | **Natural break:** minutes of UTC idle before **`DateTime.UtcNow.AddMinutes(-this)`** is passed as **`idleCutoff`** into **`GetConversationsNeedingSummarizationAsync`**. Conversations with at least one un-summarized message and **`Max(Timestamp)`** before that cutoff are enqueued for consolidation. |
| **`Arcanum:Intelligence:CampaignLogSweepIntervalMinutes`** | `int` | `15` | **`CampaignLoggerBackgroundService`** **`PeriodicTimer`** period between hybrid repository sweeps that enqueue conversation ids. |
...
When adding a property to **`ArcanumSettings`** or a nested settings type in **`RetroDownfall.Arcanum.Core`**, extend this table in the same change set. **Every feature plan or pull request that changes runtime behavior, contracts, or MCP surfaces must update [`docs/DESIGN.md`](DESIGN.md) in the same change set** — not README or code alone.
...
flowchart TB
  subgraph cli [RetroDownfall.Arcanum.Cli]
    ...
    Chat[ChatCommand]
    Look[LookCommand]
    LoreCommands[LoreList/Get/Set/Delete]
    DaemonCommands[DaemonInstall/Uninstall/Status]
  ...
  subgraph infra [RetroDownfall.Arcanum.Infrastructure]
    ...
    CampQueue[CampaignLoggerQueue]
    CampHost[CampaignLoggerBackgroundService]
  end
  subgraph core [RetroDownfall.Arcanum.Core]
    ...
    Storage["Storage: IGrimoireRepository, ICampaignLoggerQueue, entities"]
  ...
  Chat --> ApiClient
  Chat --> CliSession
  LoreCommands --> ApiClient
  DaemonCommands --> infra
  Look --> Pattern
  EyeSvc --> Pattern
  EyeDi --> EyeSvc
  Boot --> InfraDi
...
- **`OllamaSettings`** — `Endpoint` (default `http://localhost:11434`), `DefaultModel` (default `llama3.2`), and **`ContextWindowLimit`** (default **`8192`**, **`num_ctx`** + Mana bar cap).
...
- **`DependencyInjection/ServiceCollectionExtensions`** — **`AddArcanumInfrastructure`** calls **`Configure<ArcanumSettings>(configuration.GetSection("Arcanum"))`** first, then **`AddArcanumEyeOfTheWorld`** (**`IEyeOfTheWorld`** → **`EyeOfTheWorldService`**, singleton), then Serilog (**`LoggingBootstrapper`**, which resolves **`IOptions<ArcanumSettings>`** for **`Host.RetainedLogFileCount`**), then Data Protection, **`ISecretStore`** → **`DataProtectionSecretStore`**, Grimoire passphrase source + hosted service + **`DbContext`** + **`IGrimoireRepository`**, singleton **`CampaignLoggerQueue`** + hosted **`CampaignLoggerBackgroundService`** (Campaign Log worker — bounded channel, **`IServiceScopeFactory`** for scoped repository access, hybrid consolidation settings from **`IOptions<ArcanumSettings>.Value.Intelligence`** — **`CampaignLogThreshold`**, **`CampaignLogIdleTimeoutMinutes`**, **`CampaignLogSweepIntervalMinutes`**), **`IWorkspaceScanner`** → **`PhysicalWorkspaceScanner`**, singleton **`McpConnectionManager`**. **`AddArcanumEyeOfTheWorld`** alone registers perception without Serilog/Grimoire/MCP (CLI **`look`** path); callers must also register **`Configure<ArcanumSettings>`** when options-backed limits are required.
...
- **`Data/ArcanumDbContext`**, **`Data/Migrations/`** (EF Core migrations), **`Repositories/GrimoireRepository`**, **`Hosting/GrimoireDatabaseHostedService`**, **`Hosting/CampaignLoggerQueue`**, **`Hosting/CampaignLoggerBackgroundService`**, **`Data/ArcanumDbContextFactory`** (design-time), **`Generated/`** — compiled model for `UseModel(...)`.
...
- **`Mcp/InProcessMcpTransport.cs`** — **`internal`**, **`IMcpTransport`**: bounded **`Channel<string>`** from server to client demultiplexed into **`McpInboundEnvelope`** (same parser as stdio); writes JSON-RPC lines to the paired client→server channel under **`SemaphoreSlim`**; **`CreatePair(..., IntelligenceSettings intelligenceSettings, …)`** returns transport + **`ArcanumInternalToolServer`** (settings snapshot gates **`tools/list`** / disabled-tool **`tools/call`** errors).
...
**Native AOT and JSON:** No reflection-based serialization for MCP wire shapes in this path. Each tool’s **`inputSchema`** on **`tools/list`** is a **static `readonly JsonElement`** produced once via **`Utf8JsonWriter`** inside **`BuildSchema`** (zero per-request schema allocation). Every deserialization of tool **`arguments`** uses **`JsonSerializer.Deserialize(..., _json.<T>)`** with **`McpJsonSerializerContext`** only (**`ReadFileChunkParams`**, **`ReplaceTextBlockParams`**, **`WriteFileParams`**, **`ListDirectoryParams`**, **`ExecuteCommandParams`**, **`AskHumanParams`**, **`ReadLoreParams`**, **`ScribeLoreParams`**, **`DeleteLoreParams`**, **`SearchArchivesParams`** — defined in **`McpWireDtos.cs`** and registered on the context in **`JsonRpcModels.cs`**).

**Lore / archive tools and DI:** **`ArcanumInternalToolServer`** is constructed with **`IServiceScopeFactory`** and a snapshot of **`IntelligenceSettings`** (from **`IOptions<ArcanumSettings>`** via **`McpConnectionManager`** → **`InProcessMcpTransport.CreatePair`**). **`EnableLoreSystem`** controls whether **`read_lore`**, **`scribe_lore`**, and **`delete_lore`** appear on **`tools/list`**; **`EnableArchiveSearch`** controls **`search_archives`**. Each Lore or **`search_archives`** invocation creates **`await using var scope = _scopeFactory.CreateAsyncScope()`** and resolves scoped **`IGrimoireRepository`** so the singleton MCP host never captures a scoped **`ArcanumDbContext`**. **`search_archives`** calls **`SearchArchivesAsync`** with **`ArchiveSearchMaxResults`** (see §3.5); the repository runs **parameterized** raw SQL (**`@query`**, **`@limit`** parameters) against SQLite **FTS5** **`ChatMessages_fts`** (see §5.4.1) joined to **`ChatMessages`** — not EF-mapped — for AOT safety.
...
| **`read_lore`** | Read **`MageSettings`** value by string **`key`**. | Omitted from **`tools/list`** when **`Arcanum:Intelligence:EnableLoreSystem`** is **`false`**; disabled **`tools/call`** → configuration **`ToolError`**. When enabled: **`IServiceScopeFactory`** → scoped **`IGrimoireRepository.ReadLoreAsync`**; missing key returns plain text **`Key not found.`** with **`isError: false`**; failures → **`isError: true`**. |
| **`scribe_lore`** | Upsert **`key`** / **`value`** in **`MageSettings`**. | Same gating as **`read_lore`**. When enabled: same scope pattern → **`ScribeLoreAsync`**; success confirmation text; **`IOException`** / EF failures → **`isError: true`**. |
| **`delete_lore`** | Delete lore row by **`key`**. | Same gating as **`read_lore`**. When enabled: **`DeleteLoreAsync`** (**`ExecuteDeleteAsync`**); text indicates whether a row was removed; failures → **`isError: true`**. |
| **`search_archives`** | Keyword / FTS5 **`MATCH`** over archived **`ChatMessage`** rows. | Omitted when **`Arcanum:Intelligence:EnableArchiveSearch`** is **`false`**; disabled **`tools/call`** → **`ToolError`**. When enabled: scoped **`IGrimoireRepository.SearchArchivesAsync`** with **`query`** + configured max results; formatted text or **`No matching archives found.`**; **`isError: false`** on empty hits; SQL / IO failures → **`isError: true`**. |
...
- **`ApiBootstrapper`** — `AddArcanumApiServices(this IServiceCollection, IConfiguration)` calls **`AddArcanumInfrastructure`** (Serilog, options, Data Protection, secrets, Grimoire, workspace scanner, **`IEyeOfTheWorld`** via `AddArcanumEyeOfTheWorld`), then registers `ApiKeyEndpointFilter`, OpenAPI, JSON options, a named **`AddHttpClient("Ollama", …)`** registration (**`BaseAddress`** from **`ArcanumSettings.Ollama.Endpoint`**, **`HttpClient.Timeout = InfiniteTimeSpan`**), scoped **`OllamaApiClient`** constructed with **`IHttpClientFactory.CreateClient("Ollama")`** and **`ArcanumSettings.Ollama.DefaultModel`** (no raw **`new HttpClient()`** per request), `IOllamaApiClient`, `IChatClient`, and `IArcanumIntelligenceProvider`. `MapArcanumEndpoints(this WebApplication)` wires OpenAPI/Scalar + the `/api` route group with API-key filter + **`GET /api/health`**, **`GET /api/conversations`** (optional **`take`** query, clamped; **`ApiResponse<List<ConversationSummaryDto>>`** via **`IGrimoireRepository.ListRecentConversationsAsync`**), **`GET /api/conversations/{id:guid}`** (**`ApiResponse<ConversationDetailDto>`** via **`IGrimoireRepository.GetConversationDetailAsync`**; **`404`** when the id is missing), **`GET /api/conversations/{id:guid}/messages`** (**`ApiResponse<List<ConversationMessageDto>>`** — **`IGrimoireRepository.GetConversationAsync`** loads ordered **`ChatMessage`** rows; the handler maps them to DTOs only, never serializing EF entities), **`DELETE /api/conversations/{id:guid}`** (**`204 No Content`** or **`404`** with **`ApiResponse<bool>`**), **`POST /api/conversations/{id:guid}/rest`** (**`202 Accepted`** — enqueues the conversation for Campaign Log processing via **`CampaignLoggerQueue`**), **`GET /api/lore`** (**`ApiResponse<List<LoreDto>>`** via **`ListLoreAsync`**), **`GET /api/lore/{key}`** (**`ApiResponse<LoreDto>`** via **`GetLoreAsync`**; **`404`** when the key is missing), **`POST /api/lore`** (**`UpsertLoreRequest`** → **`ScribeLoreAsync`** + re-fetch; **`ApiResponse<LoreDto>`**; not gated by **`EnableLoreSystem`**), **`DELETE /api/lore/{key}`** (**`ApiResponse<bool>`** — whether a row was removed), **`POST /api/intelligence/ping`**, **`POST /api/intelligence/ping-stream`**, **`POST /api/intelligence/human-response`**, **`POST /api/mcp/reload`** (**`PingRequest`** → **`McpConnectionManager.ReloadAsync`**, **`ApiResponse<string>`**), **`POST /api/intelligence/arsenal`** (**`SpellScanner.ScanAsync`** + **`GetServerStatusesAsync`**, **`ApiResponse<WorkspaceArsenalDto>`** via **`ArcanumJsonContext`**).
- **`Intelligence/OllamaIntelligenceProvider`** — implements `IArcanumIntelligenceProvider` using OllamaSharp and `IChatClient`. Handles model-exists checks, on-demand model pull with progress, buffered inference, and streaming inference. Model name matching is **case-insensitive** and handles Ollama's `:latest` tag convention via a shared `ModelNameMatches` helper. After **`CodexReader.ReadCodexAsync`**, it **`await`s `SpellScanner.ScanAsync`** with the normalized workspace string when **`Infrastructure/Mcp/ToolHelpers.TryNormalizeWorkspace`** succeeds, or **`null`** when it does not (global spells under **`~/.config/arcanum/spells/`** are still discovered), then **`SemanticRouter.DetermineActiveSpellAsync`** (same scoped **`IChatClient`** as main inference; timeout from **`Arcanum:Intelligence:SemanticRouterPreflightTimeoutSeconds`**; **`MaxOutputTokens`** / **`Temperature`** from **`Arcanum:Intelligence:SemanticRouterMaxTokens`** / **`SemanticRouterTemperature`**, clamped via **`ArcanumSettingClamps`**). It prepends a **dynamic `ChatRole.System`** message from **`SystemPromptBuilder`** (base persona, optional **`ContextSnapshot`**, optional cascaded **`CODEX.md`**, optional **`### Active Operational Spell`** and optional **`### Available Spell Scripts`** — see §10.2.2). That system turn exists **only in memory** for Ollama — it is **not** persisted to Grimoire. **`BuildToolSetWithMcpAsync`** always starts from **`ArcanumLocalTimeTool`**, adds **`ArcanumSpellScriptTool`** when the active spell’s **`AvailableScripts`** is non-empty (even if **`PingRequest.DisableMcpTools`** is **`true`** — spell scripts are **not** MCP), then merges MCP tools when MCP is not disabled. When tools are enabled, **`CreateInferenceChatOptions(bool includeTools, List<AITool>? tools)`** attaches **`ChatOptions.Tools`** from the pre-built tool set (see §10.2.1). **Operator-safe errors:** inference failures, model listing/pull failures, streaming **`IntelligenceEventType.Error`** payloads, **`Result`/`Error`** messages returned to HTTP clients, and **Grimoire-persisted tool result text** on tool-invocation exceptions use **fixed generic strings**; **`ILogger`** records the full exception (including **`OperationCanceledException`** rethrow on the buffered path where applicable). **`LooksLikeModelDoesNotSupportTools`** still inspects the raw provider message internally to retry once without tools — that substring check is **not** echoed to clients or Grimoire as an exception dump.
...
- **`Intelligence/Tools/`** — sealed **`ArcanumLocalTimeTool`** (**`GetLocalSystemTime`**) and **`ArcanumSpellScriptTool`** (**`run_spell_script`**) with **`static readonly JsonDocument`** schemas (**`JsonSchema`** overrides); invocation is **`InvokeCoreAsync`** only. **`run_spell_script`** runs **`Process`** with **`UseShellExecute = false`**, **`WorkingDirectory`** set to the active spell’s **`scripts/`** folder (full path under **`Path.Combine(ParsedSpell.DirectoryPath, "scripts")`**), **`script_name`** accepted only as a **bare file name** (prefix containment after **`Path.GetFullPath`** — no **`../`** escapes), extension-based runner map (**.py** / **.js** / **.sh** / **.ps1** / direct), optional **`arguments`** tokenized into **`ProcessStartInfo.ArgumentList`** (no shell), stdout/stderr capture, and the same hard timeout + **`Kill(entireProcessTree: true)`** behavior as **`execute_command`** (**`Arcanum:Intelligence:ExecuteCommandTimeoutSeconds`**, clamped **1–600**). Workspace file reads, writes, and MCP command execution remain **`McpBridgeTool`** entries from the in-process server (**`read_file_chunk`**, **`replace_text_block`**, **`write_file`**, **`list_directory`**, **`execute_command`** — relative paths, **`Infrastructure/Mcp/ToolHelpers`** workspace sandbox). Spell script execution is **structurally separate** from that sandbox (spell roots may be outside **`PingRequest.WorkingDirectory`**).
...
- **`Services/CliSessionManager`** — internal static helper: **`GetLastConversationId`**, **`SaveConversationId`**, **`ClearSession`** against **`Path.Combine(ArcanumPaths.GrimoireDirectory, "cli-session.txt")`** (same base directory as **`arcanum.db`**). Plain text file I/O only (Native AOT–friendly). **`IOException`** and **`UnauthorizedAccessException`** on that path print a **dim yellow** Spectre warning (**could not save/load session state**) instead of failing silently.
...
- **`Commands/Lore/*`** — `AsyncCommand` types (**`LoreListCommand`**, **`LoreGetCommand`**, **`LoreSetCommand`**, **`LoreDeleteCommand`**) that resolve **`ArcanumApiClient`** and call the **`/api/lore`** CRUD surface; table / panel output uses **`Markup.Escape`** on Grimoire-sourced strings so hostile lore text cannot break Spectre markup.
...
- **`Services/ArcanumApiClient`** — wraps `IHttpClientFactory` + `ISecretStore`; provides **`AskAsync(PingRequest, CancellationToken)`** (buffered **`/api/intelligence/ping`**) and **`AskStreamAsync(PingRequest, CancellationToken)`** (`IAsyncEnumerable<IntelligenceEvent>` by reading the response stream line-by-line and **`JsonSerializer.Deserialize<IntelligenceEvent>(line, ArcanumJsonContext.Default.IntelligenceEvent)`** for each NDJSON line, matching the server’s **`application/x-ndjson`** writer; request bodies still use **`ArcanumJsonContext`** for nested **`PatternSnapshot`** / **`DomainType`** metadata). Conversation management: **`GetConversationsAsync`**, **`GetConversationAsync(Guid)`** (**`GET /api/conversations/{id}`** → **`ConversationDetailDto`**, including **`Summary`** for the REPL **`/log`** command), **`GetConversationMessagesAsync(Guid)`** (**`GET /api/conversations/{id}/messages`** → **`List<ConversationMessageDto>`**, ordered message history for external clients), **`DeleteConversationAsync`**, **`RestAsync(Guid)`** (**`POST /api/conversations/{id}/rest`**, treats **`202 Accepted`** as success), plus **`ReloadMcpAsync`**, **`GetWorkspaceArsenalAsync`**, **`SubmitHumanResponseAsync`**. Lore (**`MageSettings`** via HTTP): **`ListLoreAsync`** (**`GET /api/lore`**), **`GetLoreAsync`** (**`GET /api/lore/{key}`**, path segment uses **`Uri.EscapeDataString`**), **`UpsertLoreAsync`** (**`POST /api/lore`** with **`UpsertLoreRequest`**), **`DeleteLoreAsync`** (**`DELETE /api/lore/{key}`** → **`ApiResponse<bool>`**; **`false`** means no row removed). All JSON read/write uses **`ArcanumJsonContext`** (`SerializeToUtf8Bytes` / **`Deserialize`** on response bytes), not reflection-based **`JsonSerializer`** overloads.
...
| **Chat** | `chat` | Multi-turn REPL with Mana bar, `/attach`, slash commands, swap-at-the-end Markdig rendering, per-turn cancellation. Sends **`cliTerminalFormatting: true`**. |
...
| **Lore** | `lore list\|get\|set\|delete` | Direct CRUD on **`MageSettings`** via **`/api/lore`** (requires **`serve`** running). |
| **Daemon** | `daemon install\|uninstall\|status` | OS-specific background agent lifecycle (Windows SC, macOS launchd, Linux systemd user). |
...
**Version pinning:** `Spectre.Console.Cli` is pinned to **0.55.0** in the project file for reproducible restores; upgrades should be deliberate and re-run `dotnet publish` with AOT analysis. From **0.55** onward, `AsyncCommand` **`ExecuteAsync`** overrides must use **`protected override`** (the base member is no longer `public`).
...
- **`GrimoireDatabaseHostedService`** (`IHostedService`) — runs **`SQLitePCL.Batteries_V2.Init()`**, verifies the master API key is present, derives the DB passphrase with **`GrimoireKeyDerivation`** (HKDF from the UTF-8 API key), probes an **existing** DB with `SELECT 1` or **`Database.MigrateAsync`** when the file is missing, and **`FailFast`** on fatal key mismatch (see README for operator-facing semantics).
- **`CampaignLoggerQueue`** / **`CampaignLoggerBackgroundService`** — singleton bounded **`Channel<Guid>`** (**`BoundedChannelFullMode.DropOldest`**, capacity **100**) plus a **`BackgroundService`** that **`Task.Yield()`**s, then runs a **hybrid sweep** (**`IServiceScopeFactory`** → **`IGrimoireRepository.GetConversationsNeedingSummarizationAsync(threshold, idleCutoff)`** using **`CampaignLogThreshold`**, **`CampaignLogIdleTimeoutMinutes`**, and a **`PeriodicTimer`** from **`CampaignLogSweepIntervalMinutes`**) **concurrently** with **`await foreach`** on **`ReadAllAsync`** to process ids. An **immediate** sweep runs before the first timer tick. Operators may also enqueue via **`POST /api/conversations/{id}/rest`** (phase **1** logs only; LLM summarization and updating **`Conversation.Summary`** / **`LastSummarizedMessageAt`** are deferred). Per-item failures are caught so the host keeps running.
- **`ArcanumDbContext`** — compiled model via **`UseModel(ArcanumDbContextModel.Instance)`**; connection password from **`IGrimoireDbPassphraseSource`** set by the hosted service. **`ChatMessage`** uses a **composite index** on **`(ConversationId, Timestamp)`** (regenerate **`Generated/`** after mapping changes; existing DB files may need the manual **`CREATE INDEX IF NOT EXISTS`** in the README).
- **`GrimoireRepository`** — implements **`IGrimoireRepository`**; streaming inference uses **`ExecuteSqlInterpolatedAsync`** for token append and **`ExecuteUpdateAsync`** for final assistant content where appropriate (SQLite **`AFTER UPDATE`** triggers keep **`ChatMessages_fts`** in sync). **`SearchArchivesAsync`** bypasses EF mapping for the virtual FTS table.

**Api integration:** **`OllamaIntelligenceProvider`** injects **`IGrimoireRepository`**; persistence failures on the non-streaming path are logged as warnings and do not replace the inference result. Streaming behavior matches the README (append tokens, finalize on completion, **`conversationBound`** event). Multi-turn threads use **`PingRequest.ConversationId`**: prior messages are loaded for **`IChatClient`**; **`BeginAssistantReplyAsync`** appends to an existing **`Conversation`** when the id exists. **`WorkingDirectory`**, **`ContextSnapshot`**, and the cascaded global+local **`CODEX.md`** content feed a **dynamic system** message prepended to the chat sent to Ollama (see **§10.5**); Grimoire still stores only **User**, **Assistant**, and tool-bracket transcript rows — not that synthetic system prompt. Tool rows persisted after a failed tool invocation store a **generic** result line (see **`OllamaIntelligenceProvider`** above), not raw exception text.
...
|--------|-------|-------------|--------------------------------|
| **`Conversation`** | `Conversations` | `Id` (Guid) | `Title` (string, max 512, required); optional nullable **`Summary`** (long text); optional nullable **`LastSummarizedMessageAt`** (**`DateTime`**, cursor for Campaign Log summarization); **index on `CreatedAt`**; has-many `ChatMessage` with cascade delete. Migration **`AddCampaignLogFields`** adds the two nullable columns. |
| **`ChatMessage`** | `ChatMessages` | `Id` (Guid) | `ConversationId` (Guid FK); **composite index** on **`(ConversationId, Timestamp)`**; `Role` (enum stored as `int` via `HasConversion<int>()`: `User` = 0, `Assistant` = 1, `System` = 2); `Content` (required); `ModelUsed` (max 256, required); `Timestamp` (DateTime). **FTS5:** migration **`AddChatMessagesFts`** creates virtual table **`ChatMessages_fts`** plus **`AFTER INSERT` / `UPDATE` / `DELETE`** triggers and backfill — not an EF-mapped entity; consumed only by **`IGrimoireRepository.SearchArchivesAsync`** via ADO.NET. |
| **`MageSetting`** | `MageSettings` | `Key` (string, max 256) | `Value` (required); `UpdatedAt` (DateTime). Consumed by **`IGrimoireRepository`** Lore methods and the in-process MCP tools **`read_lore`**, **`scribe_lore`**, **`delete_lore`** when **`Arcanum:Intelligence:EnableLoreSystem`** is **`true`** (§4.2). |
...
- **`ConversationSummaryDto`** (`Intelligence/Models/`) — `sealed record (Guid Id, DateTime CreatedAtUtc, DateTime UpdatedAtUtc, string Snippet)` — wire/API projection for conversation lists; not a mapped entity. Populated by `ListRecentConversationsAsync` after an EF query (no string truncation inside LINQ); `Snippet` is the first chat message by `Timestamp`, truncated to 50 characters with `...` in memory; `UpdatedAtUtc` is the max message timestamp or `CreatedAtUtc` when there are no messages.
- **`ConversationDetailDto`** (`Intelligence/Models/`) — `sealed record (Guid Id, string Title, DateTime CreatedAt, string? Summary)` — wire projection for a single conversation’s identity and Campaign Log field (`Conversation.Summary`) without loading **`ChatMessage`** rows. Populated by **`GetConversationDetailAsync`** via an EF **`Select`** projection only.
- **`ConversationMessageDto`** (`Intelligence/Models/`) — `sealed record (Guid Id, MessageRole Role, string Content, string ModelUsed, DateTime Timestamp)` — wire projection for one persisted **`ChatMessage`**. Returned as an ordered **`List<ConversationMessageDto>`** from **`GET /api/conversations/{id}/messages`** (server-side **`Select`** projection via **`GetConversationMessagesAsync`**; never serializes EF entities).
- **`LoreDto`** (`Intelligence/Models/`) — `sealed record (string Key, string Value, DateTime UpdatedAtUtc)` — wire projection for **`MageSetting`** rows. Populated by **`ListLoreAsync`** / **`GetLoreAsync`** with **`AsNoTracking`** + **`Select`** projection.
- **`UpsertLoreRequest`** (`Intelligence/Models/`) — `sealed record (string Key, string Value)` — request body for **`POST /api/lore`** (source-generated via **`ArcanumJsonContext`**).
...
The contract (`Core`) and implementation (`Infrastructure/Repositories/GrimoireRepository`) expose fifteen methods. The implementation uses **`ArcanumDbContext`** directly for EF paths; **`SearchArchivesAsync`** uses **`Database.GetDbConnection()`** with **`DbCommand`** / **`DbDataReader`** only (parameterized **`MATCH`** + **`LIMIT`**).
...
| **`ListRecentConversationsAsync`** | Returns up to **`take`** conversations ordered by last activity. | `AsNoTracking`, `OrderByDescending` on **`Messages.Max(Timestamp) ?? CreatedAt`**, **`Take(n)`**, projects an intermediate shape (id, createdAt, lastUpdate, first message content by timestamp) with **no string manipulation inside LINQ**; materializes, then builds **`ConversationSummaryDto`** in memory (snippet truncation). Returns empty when `take <= 0`. |
| **`DeleteConversationAsync`** | Deletes one conversation and its messages. | Two-step bulk delete for SQLite safety: **`ChatMessages`** matching **`ConversationId`**, then **`Conversations`** matching **`Id`** — both via **`ExecuteDeleteAsync`**. Returns **`0`** or **`1`** conversation rows removed. |
| **`GetConversationAsync`** | Loads a conversation with its messages ordered by timestamp. | `AsNoTracking`, eager-loads `Messages` via `Include(c => c.Messages.OrderBy(m => m.Timestamp))`. Consumed by **`GET /api/conversations/{id}/messages`**, which projects each **`ChatMessage`** to **`ConversationMessageDto`** for API-first thread reconstruction over the wire (EF entities are never serialized on that route). |
| **`GetConversationDetailAsync`** | Loads **`Id`**, **`Title`**, **`CreatedAt`**, and **`Summary`** for one conversation (no messages). | `AsNoTracking`, **`Where`** by id, **`Select`** into **`ConversationDetailDto`** — avoids **`Include`** payload for HTTP **`GET /api/conversations/{id}`** and CLI **`/log`**. |
| **`GetConversationsNeedingSummarizationAsync`** | Returns **`Conversation.Id`** values where **either** the count of messages with **`Timestamp > (LastSummarizedMessageAt ?? DateTime.MinValue)`** exceeds **`threshold`**, **or** there exists at least one such un-summarized message **and** **`Messages.Max(m => (DateTime?)m.Timestamp) < idleCutoff`** (natural idle break using UTC **`idleCutoff`** from the caller). | Static LINQ over **`Conversations`** / **`Messages`** translated by EF Core (Native AOT–friendly; nullable **`Max`** for SQLite). Used by **`CampaignLoggerBackgroundService`** on each sweep. |
| **`ReadLoreAsync`** | Reads **`MageSettings.Value`** by **`Key`**, or **`null`** when missing. | `AsNoTracking`, `FirstOrDefaultAsync` on **`Key`**. |
| **`ScribeLoreAsync`** | Upserts one **`MageSetting`** row. | Tracked **`FirstOrDefaultAsync`** by **`Key`**; insert or update **`Value`** + **`UpdatedAt`** (`UtcNow`); **`SaveChangesAsync`**. |
| **`DeleteLoreAsync`** | Deletes at most one row by **`Key`**. | **`ExecuteDeleteAsync`**; returns whether any row was removed. |
| **`ListLoreAsync`** | Lists all **`MageSettings`** rows as **`LoreDto`**, ordered by **`Key`**. | `AsNoTracking`, **`OrderBy(Key)`**, **`Select`** into **`LoreDto`** (`UpdatedAt` projected as **`UpdatedAtUtc`**). |
| **`GetLoreAsync`** | Loads one lore entry by **`Key`**, or **`null`** when missing. | `AsNoTracking`, **`Where`** by **`Key`**, **`Select`** into **`LoreDto`**, **`FirstOrDefaultAsync`**. |
| **`SearchArchivesAsync`** | Returns a formatted string of up to **`maxResults`** chat rows matching an FTS5 **`query`**. | Opens **`DbConnection`** if needed; SQL joins **`ChatMessages_fts`** to **`ChatMessages`** with **`WHERE f MATCH @query`** and **`LIMIT @limit`** (**`@query`** / **`@limit`** bound as parameters); **`maxResults`** clamped **`1`–`500`**; empty / whitespace **`query`** → **`No matching archives found.`** without executing **`MATCH`**. |
...
- `ArcanumApiClient.AskStreamAsync(PingRequest, …)` serializes the body with **`ArcanumJsonContext`**, sends the request with **`HttpCompletionOption.ResponseHeadersRead`**, then reads UTF-8 text lines from the response body and deserializes each non-empty line with **`JsonSerializer.Deserialize(line, ArcanumJsonContext.Default.IntelligenceEvent)`** (NDJSON is **not** a JSON array root, so **`DeserializeAsyncEnumerable`** is the wrong primitive for this wire format). If a line fails to parse (**`JsonException`**), the client yields a fabricated **`IntelligenceEvent(Error, "Malformed data received from server. Skipping frame.")`** and **continues** reading the stream — a single corrupted frame does not terminate the session.
- `AskCommand` handles **`ConversationBound`** by saving the GUID from **`Data`** via **`CliSessionManager`** (silent). It prints `status` events to stderr (dim Spectre markup), prints `toolCall` / `toolResult` lines to stderr (grey Spectre markup), writes `token` data directly to stdout, and returns exit code 0 (success), 1 (error), or 130 (Ctrl+C interrupt). The terminal **`result`** line’s **`data`** is **not** assistant text — it is the **total token count** string from the last model completion round; clients must concatenate **`token`** frames for the answer body.
...
**Session-Based Consolidation** is the Arcanum model for when the Campaign Log pipeline should treat a conversation as ready to consolidate (enqueue for summarization; LLM persistence of **`Conversation.Summary`** / **`LastSummarizedMessageAt`** remains a later phase). Three mechanisms work together:

1. **Message-count threshold** (**`Arcanum:Intelligence:CampaignLogThreshold`**) — a **safety valve**: unbounded growth of un-summarized messages triggers consolidation even during active chat.
2. **Idle timeout** (**`Arcanum:Intelligence:CampaignLogIdleTimeoutMinutes`**) — a **natural session boundary**: if there is still un-summarized content but the conversation’s latest **`ChatMessage.Timestamp`** (via **`Max`**) is strictly before **`idleCutoff`** (**`DateTime.UtcNow`** minus configured minutes), the conversation is selected.
3. **Explicit rest** — **`POST /api/conversations/{id}/rest`** enqueues a conversation id immediately without waiting for a sweep.

**Implementation:** **`GrimoireRepository.GetConversationsNeedingSummarizationAsync`** evaluates the hybrid OR predicate server-side (nullable **`(DateTime?)m.Timestamp`** in **`Max`** for EF Core SQLite and Native AOT). **`CampaignLoggerBackgroundService`** performs an immediate sweep, then repeats on a **`PeriodicTimer`** (**`CampaignLogSweepIntervalMinutes`**), **concurrently** with the queue consumer; **`ApiBootstrapper`** registers the rest route on the keyed **`/api`** group.

**Cognitive analogy:** Like human memory, episodic detail from an active session can stay in a high-fidelity “working” form until the session **pauses** (idle), **grows too large** (threshold), or the operator **explicitly ends** the session (**`/rest`**), after which consolidating into a durable summary matches how people compress experience into long-term recall.
...
**Streaming:** `StreamPromptAsync` yields `IntelligenceEvent` objects. Status events report model checks and download progress; **`conversationBound`** carries the canonical conversation id after Grimoire begins the turn; token events carry incremental assistant text; **`toolCall`** / **`toolResult`** surface local tool execution between LLM rounds; a final **`result`** event carries **total token usage** (input + output from **`ChatResponse.Usage`** on the last non-tool completion round) as a decimal string in **`Data`** (the **`StringBuilder`** with capacity 1024 still accumulates assistant text for Grimoire finalization only — it is **not** duplicated on the wire in **`result.data`**); error events describe failures. The `[EnumeratorCancellation]` attribute threads cancellation correctly through the async iterator.
...
| **`run_spell_script`** | **`ArcanumSpellScriptTool`** | Runs a file under **`Path.Combine(activeSpell.DirectoryPath, "scripts")`** only; **`script_name`** must be a bare file name; **`arguments`** optional, tokenized without a shell; same timeout / kill-tree semantics as **`execute_command`** (§4.3 **`Intelligence/Tools/`**). Registered **even when** **`DisableMcpTools`** is **`true`**. |
| **`read_file_chunk`**, **`replace_text_block`**, **`write_file`**, **`list_directory`**, **`execute_command`**, **`ask_human`** | **`McpBridgeTool`** → in-process **`ArcanumInternalToolServer`** | File and directory tools require **`relativePath`** (or **`'.'`** for workspace root for **`list_directory`**) resolved under the partition’s normalized workspace; rooted inputs are rejected; escapes return **`isError: true`**. **`write_file`** supplies full-file **`content`**, creates parent directories when needed, and overwrites an existing file atomically at the resolved path (same sandbox as reads). **`execute_command`** uses optional **`workingDirectory`** relative to the workspace root, stdout/stderr capture, configurable hard timeout (**`Arcanum:Intelligence:ExecuteCommandTimeoutSeconds`**, default **30**, clamped **1–600**), and **`Kill(entireProcessTree: true)`** on timeout. **`ask_human`** is available even when **`WorkingDirectory`** is empty (no-workspace partition); other filesystem internal tools return a fixed **`ToolError`** when no workspace root is configured. |
| **`read_lore`**, **`scribe_lore`**, **`delete_lore`** | **`McpBridgeTool`** → in-process **`ArcanumInternalToolServer`** | Grimoire **`MageSettings`** key-value store via scoped **`IGrimoireRepository`** (**`IServiceScopeFactory`** per §4.2); **no** workspace root required; **`tools/list`** / **`tools/call`** gated by **`Arcanum:Intelligence:EnableLoreSystem`**. |
| **`search_archives`** | **`McpBridgeTool`** → in-process **`ArcanumInternalToolServer`** | FTS5 search over **`ChatMessages`** via **`SearchArchivesAsync`**; **no** workspace root required; gated by **`Arcanum:Intelligence:EnableArchiveSearch`**; max hits from **`Arcanum:Intelligence:ArchiveSearchMaxResults`**. |

**Empty workspace:** When **`PingRequest.WorkingDirectory`** is empty, **`McpConnectionManager`** still starts an internal server for the sentinel partition with **no filesystem root** so **`ask_human`**, Lore tools (when enabled), and **`search_archives`** (when enabled) work; **`read_file_chunk`**, **`replace_text_block`**, **`write_file`**, **`list_directory`**, and **`execute_command`** return **`Workspace not configured. This tool requires a valid workspace.`** The API process **never** substitutes **`Environment.CurrentDirectory`** for tool sandboxing.

**`ask_human` failure surface (CLI):** When **`AskHumanToolCallStreamHandler.TryHandleAskHumanAsync`** submits the user's answer (or unattended stub) via **`ArcanumApiClient.SubmitHumanResponseAsync`** and the API responds with a failure (`Result<bool>.IsFailure`), the handler prints **`[red]Failed to submit response to Daemon. The stream may be disconnected.[/]`** and returns **`AskHumanResult.SubmitFailed`**. Both **`AskCommand`** and **`ChatCommand`** treat this as a terminal condition for the current turn: **`AskCommand`** exits with code **1**; **`ChatCommand`** sets **`errored = true`** and breaks the **`await foreach`** loop, returning the user to the **`Mage >`** prompt without hanging indefinitely.
...
3. **Main inference:** Unchanged **`ChatOptions`** tool registration and multi-round tool loop. **`SystemPromptBuilder.Build`** appends **`### Active Operational Spell ({Name})`** and the spell’s **`FullContent`** when a spell was selected, and **`### Available Spell Scripts`** (listed file names + **`run_spell_script`** instruction) when **`AvailableScripts`** is non-empty.
...
**Streaming path (`StreamPromptAsync`):** Loads history when **`conversationId`** is present, maps Grimoire messages plus the new user prompt to a **`ChatMessage`** list, **prepends the dynamic system prompt (§10.5)**, then **`BeginAssistantReplyAsync`**, yields **`conversationBound`**, streams via **`GetStreamingResponseAsync`** (tokens accumulate in memory for NDJSON **`token`** events), and completes the Grimoire row with **`FinalizeAssistantMessageAsync`** (full assistant text — see §5.4). Failures follow the same non-fatal logging stance as the README.
...
**Same `WorkingDirectory` for tools:** **`McpConnectionManager.GetAvailableToolsAsync`** receives the same **`PingRequest.WorkingDirectory`** string used for **`CODEX.md`** and spell discovery, so the internal MCP partition root matches the operator cwd **`ask`** sends — not the daemon process default directory. Models pass **relative** paths (and optional relative **`workingDirectory`**) to **`execute_command`** under that root. **`run_spell_script`** is **not** constrained to that MCP root: it resolves only under **`Path.Combine(activeSpell.DirectoryPath, "scripts")`** (spell **`DirectoryPath`** from discovery), which may differ from **`WorkingDirectory`** when spells live under **`~/.config/arcanum/spells/`** or another tree.
...
**HTTP:** The same perception engine is exposed on the authenticated Minimal API as **`GET /api/perception/look`** with optional query **`directory`** (requires **`X-Arcanum-Key`** like other **`/api`** routes). When **`directory`** is omitted or whitespace, the host falls back to **`Environment.CurrentDirectory`**. The wire shape is **`ApiResponse<PatternSnapshot>`** (camelCase JSON via **`ArcanumJsonContext`**).

### 15.3 Enumeration and noise control (`EyeOfTheWorldService`)

**Thread-pool safety:** **`PerceivePatternAsync`** offloads all synchronous filesystem I/O and TOC generation to the thread pool via **`await Task.Run(() => { … }, cancellationToken)`**. Early-return guards (empty path, directory-not-found) remain synchronous with no I/O so they do not block. This prevents thread-pool starvation on the caller (ASP.NET request thread or CLI main thread) during large workspace walks.
...
**Cooperative cancellation:** **`cancellationToken.ThrowIfCancellationRequested()`** is injected at three levels: (1) inside the **`Directory.EnumerateFiles`** loop (per file), (2) inside each **`BuildSignatureToc`** bucket iteration and dedup loop, and (3) inside the **`BuildUnknownToc`** sorted-file iteration. This ensures Ctrl+C (or any upstream cancellation) aborts the walk immediately regardless of which phase is active.
...
- **The agentic loop (tool calling).** `OllamaIntelligenceProvider` implements a bounded local loop for **`Microsoft.Extensions.AI`** tools: `FunctionCallContent` from the model is executed on the API host, `FunctionResultContent` is fed back on `ChatRole.Tool`, and NDJSON **`toolCall`** / **`toolResult`** events surface progress. **Two** first-party sealed **`AIFunction`** tools live under **`Api/Intelligence/Tools`** (§10.2.1): **`ArcanumLocalTimeTool`** (`GetLocalSystemTime`) and **`ArcanumSpellScriptTool`** (`run_spell_script`, registered when the active spell lists `scripts/` files). Workspace file reads, writes (**`write_file`**), directory listing, command execution, Grimoire Lore (**`read_lore`**, **`scribe_lore`**, **`delete_lore`** when enabled), and archive search (**`search_archives`** when enabled) are **`McpBridgeTool`** instances from the **partition-scoped** in-process Arcanum MCP server plus optional **`mcp.json`** servers when **`PingRequest.DisableMcpTools`** is **`false`**. **Deferred:** pluggable **tool catalogs** beyond **`mcp.json`**, and **human approval** gates before high-risk actions.
...
- **EF Core migrations.** Grimoire schema is versioned under **`Data/Migrations/`** (baseline **`InitialCreate`**). First-run hosts apply pending migrations via **`MigrateAsync`** when **`arcanum.db`** is missing. **Legacy** files created without **`__EFMigrationsHistory`** cannot apply **`InitialCreate`** as-is; operators back up and recreate the file or manually baseline migration history (see README). Further schema changes add migrations and regenerate **`Generated/`** with **`dotnet ef dbcontext optimize`**.
- **`MageSetting`** is consumed by **`IGrimoireRepository`** Lore methods and the in-process MCP tools **`read_lore`**, **`scribe_lore`**, and **`delete_lore`** (when **`EnableLoreSystem`** is **`true`**). **`ChatMessage`** rows (and migration-maintained **`ChatMessages_fts`**) back **`search_archives`** when **`EnableArchiveSearch`** is **`true`**. Operators may document Lore and archive search in **`~/.config/arcanum/CODEX.md`**; the repo ships **[`docs/CODEX.template.md`](CODEX.template.md)** as merge-ready sample text. **`WorkspaceContext`** remains defined, mapped, and unused — reserved for future workspace indexing.
...
| **`IGrimoireRepository`** | Core contract for Grimoire CRUD — streaming append, finalize, buffered save, list, lightweight **`GetConversationDetailAsync`** (id + **`Summary`**), full **`GetConversationAsync`** (with messages; consumed by **`GET /api/conversations/{id}/messages`** as a projected **`List<ConversationMessageDto>`**), Campaign Log sweep selection, Lore (**`ReadLoreAsync`**, **`ScribeLoreAsync`**, **`DeleteLoreAsync`**, **`ListLoreAsync`**, **`GetLoreAsync`**), and **`SearchArchivesAsync`** (FTS5). Implemented by **`GrimoireRepository`** in Infrastructure. |
| **`ConversationDetailDto`** | Core wire type: **`Guid Id`**, **`string? Summary`** — returned by **`GET /api/conversations/{id}`** and **`ArcanumApiClient.GetConversationAsync`**; built from EF projection only (no message hydration). |
| **`ConversationMessageDto`** | Core wire type for one persisted turn: **`Guid Id`**, **`MessageRole Role`**, **`string Content`**, **`string ModelUsed`**, **`DateTime Timestamp`** — returned as **`List<ConversationMessageDto>`** by **`GET /api/conversations/{id}/messages`** and **`ArcanumApiClient.GetConversationMessagesAsync`**; mapped in the API from **`ChatMessage`** only (never serialized as an EF entity). |
...
| **`McpJsonSerializerContext`** | Source-generated **`JsonSerializerContext`** for JSON-RPC DTOs, **`JsonElement`**, primitive and **`Dictionary<string, JsonElement>`** tool-argument maps, MCP wire DTOs (**`McpInitializeParams`**, **`McpToolsCallParams`**, …), and **`ArcanumInternalToolServer`** tool parameter records (**`ReadFileChunkParams`**, **`ReplaceTextBlockParams`**, **`WriteFileParams`**, **`ListDirectoryParams`**, **`ExecuteCommandParams`**, **`AskHumanParams`**, **`ReadLoreParams`**, **`ScribeLoreParams`**, **`DeleteLoreParams`**, **`SearchArchivesParams`**); uses explicit **`JsonPropertyName`** for spec-correct JSON-RPC member names. |
...
| **`ArcanumInternalToolServer`** | **`internal`**: in-process MCP JSON-RPC loop — **`initialize`**, **`tools/list`**, **`tools/call`** with native tools (**`read_file_chunk`**, **`replace_text_block`**, **`write_file`**, **`list_directory`**, **`execute_command`**, **`ask_human`**, optional Lore trio, optional **`search_archives`** per **`IntelligenceSettings`**); per-instance **`JsonElement`** input schemas built in the constructor (**`execute_command`** embeds the configured timeout); tool args via **`McpJsonSerializerContext`** only; **`IServiceScopeFactory`** resolves **`IGrimoireRepository`** for Grimoire-backed tools (§4.2). |
...
| **`arcanum chat`** | Spectre interactive REPL with per-turn **`CancellationTokenSource`**, slash commands including **`/exit`**, **`/clear`**, **`/rest`** (Campaign Log enqueue), **`/log`** (summarized **`Conversation.Summary`** in a **`Panel`**), **`/attach`** (**`SelectionPrompt`** over **`BrowseItem`**, **1 MiB** staging cap, **`StringComparer.Ordinal`** path set, default prompt when submitting empty with staged files), **`-m` / `-n` / `--no-tools`** options, plain-text streaming, and swap-at-end Markdown render through **`MarkdigSpectreRenderer`** (§4.4, §16.5). |

---

## 18. Document maintenance

**Policy:** Any **implementation plan**, feature work, or pull request that changes **architecture, contracts, configuration, persistence, or MCP behavior** must **update this `DESIGN.md` file in the same change set**. Treat **`DESIGN.md`** as mandatory alongside code; do not close work with only README or **`CODEX.template.md`** updates.
