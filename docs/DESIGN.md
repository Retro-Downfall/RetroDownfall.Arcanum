...
| **`Arcanum:Intelligence:CampaignLogThreshold`** | `int` | `25` | Passed to **`IGrimoireRepository.GetConversationsNeedingSummarizationAsync`** so **`CampaignLoggerBackgroundService`** can enqueue conversations where the count of related **`ChatMessage`** rows with **`Timestamp > (LastSummarizedMessageAt ?? DateTime.MinValue)`** exceeds this value (Campaign Log summarization pipeline; LLM step is a later phase). |
...
- **`CampaignLoggerQueue`** / **`CampaignLoggerBackgroundService`** — singleton bounded **`Channel<Guid>`** (**`BoundedChannelFullMode.DropOldest`**, capacity **100**) plus a **`BackgroundService`** that **`Task.Yield()`**s, runs a **resilience pass** (**`IServiceScopeFactory`** → scoped **`IGrimoireRepository.GetConversationsNeedingSummarizationAsync`** with **`Arcanum:Intelligence:CampaignLogThreshold`**) to enqueue work, then **`await foreach`** on **`ReadAllAsync`** to process ids (phase **1** logs only; LLM summarization and updating **`Conversation.Summary`** / **`LastSummarizedMessageAt`** are deferred). Per-item failures are caught so the host keeps running.
...
The contract (`Core`) and implementation (`Infrastructure/Repositories/GrimoireRepository`) expose twelve methods. The implementation uses **`ArcanumDbContext`** directly for EF paths; **`SearchArchivesAsync`** uses **`Database.GetDbConnection()`** with **`DbCommand`** / **`DbDataReader`** only (parameterized **`MATCH`** + **`LIMIT`**).
...
| **`GetConversationsNeedingSummarizationAsync`** | Returns **`Conversation.Id`** values where the count of related messages with **`Timestamp > (LastSummarizedMessageAt ?? DateTime.MinValue)`** exceeds **`threshold`**. | Static LINQ over **`Conversations`** / **`Messages`** translated by EF Core (Native AOT–friendly; no dynamic LINQ). Used by **`CampaignLoggerBackgroundService`** on startup. |
...
| **`IGrimoireRepository`** | Core contract for Grimoire CRUD — streaming append, finalize, buffered save, list, get, Lore, and **`SearchArchivesAsync`** (FTS5). Implemented by **`GrimoireRepository`** in Infrastructure. |
