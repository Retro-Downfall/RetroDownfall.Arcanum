...
**Idempotency-Key replay:** an optional `Idempotency-Key` request header on `POST /api/intelligence/ping(-stream)`, `POST /v1/chat/completions`, and `POST /v1/embeddings` caches the full response (buffered or streamed) and replays it verbatim on a retry with the same key + body, instead of re-executing inference. See [DESIGN.md §11.17](DESIGN.md#1117-idempotency-key-request-replay).
...
| `Arcanum:Grimoire` / `Sessions` | Load/query caps, snapshot retention, page sizes, SSE replay caps, `MaxEntriesPerSession` / `MaxEntryContentBytes` entry bounds (also caps stateless `/v1` and ping message content), `MaxForkDepth` lineage cap for `POST /api/sessions/{id}/fork`. |
