...
| Inference (OpenAI) | `POST /v1/chat/completions`, `GET /v1/models` | OpenAI-shaped JSON/SSE; **not** envelope-wrapped. Images (`content[].type: "image_url"`) are gated by **Scrying** — sent to a model whose `models` entry lacks `supportsVision: true` return `400 invalid_request_error` (`code: "vision_not_supported"`); disabled Scrying returns `403` (`code: "feature_disabled"`). |
| Sessions (Grimoire) | `/api/sessions/*` (CRUD, `/entries`, `/export`, `/rest`, `/stream`, `/analytics`, `/divine`) | Single source of truth for threads; FTS5 search; SSE live stream. `POST /divine` is RAG Phase 2 semantic search over embedded entries (disabled by default; see [DESIGN.md §21.6](DESIGN.md#216-phase-2--session-divination)). |
