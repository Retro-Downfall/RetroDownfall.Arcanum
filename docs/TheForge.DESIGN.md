...
- **`IntelligenceEvent`:** token text is in **`Data`**, not `Message`. `IntelligenceEventType` is camelCase string enum (`token`, `toolCall`, `toolResult`, `toolError`, `warded`, `wardResolved`, `status`, `sessionBound`, `conversationBound`, `result`, `error`). Ignore deprecated `conversationBound`. `ChatCompletionUsage` uses OpenAI snake_case field names inside an otherwise camelCase envelope.
