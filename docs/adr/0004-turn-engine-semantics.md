...
ModelCallStarted, TextDelta, ModelCallCompleted, ModelCallFailed,
...
Terminal events carry `TurnTerminationReason`, optional stable `Error`, accumulated usage, warnings, and interruption metadata. `RunCompleted` is authoritative for buffered projection (final text, usage, tool calls, finish reason, warnings, session id, structured-output warning state).
...
- First provider text/content delta (even if guardrail-buffered)
...
After commit, connectivity failure terminates the run — no provider switch. Never discard emitted provider text for another provider.

### Mode policies (one loop, different flags)

1. **Structured output:** no corrective retries after any client-visible `TextDelta`. Streaming validation is post-hoc; StrictMode → `RunFailed`. Buffered retries use the executor + budget; only the accepted candidate reaches buffered final text.
...
3. **Guardrails streaming:** provider commitment occurs on provider content; `TextDelta` may be withheld until the filter passes; successful buffered mode may project one consolidated token; rejection → `RunFailed` with no client-visible content under buffered mode.
...
6. **OpenAI `/v1`:** projection filter omits toolResult/toolError/ward/status; not an engine mode.
...
`IModelCallExecutor` lives in Core and returns `ModelCallPurpose`-tagged `ModelCallUpdate` / `ModelCallResult`. It does not emit Api `TurnEvent`s. TurnEngine decides whether to emit client-visible `TextDelta` (SpellRouting / LexiconExtraction never become client deltas; structured-output retries are buffered candidates).
