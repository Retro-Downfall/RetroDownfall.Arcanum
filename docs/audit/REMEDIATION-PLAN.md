...
- [x] **W2.4 — Apprentice engine reliability** ✅ *done (3 P1s) — slot-before-`Running` (revert to `Escalated` on capacity fail); recovery-queue-full logs a warning; `OnExited` disposes the `Process` (idempotent vs `StopAsync`). **Deferred:** pending-queue P2s (queued-start-returns-failure, `CancelAsync`-ignores-pending, dedup) and Simulacrum P2s (fan-out, single-branch fault).*
...
- [x] **W2.5 — llama-server lifecycle & cache integrity** ✅ *done (P1 port clamp + P2 portOverride validation + P2 atomic manifest finalize). **Deferred:** LRU-over-cap, manifest-less/legacy integrity bypass, raw cache-key trust, manifest-touch RMW race.*
