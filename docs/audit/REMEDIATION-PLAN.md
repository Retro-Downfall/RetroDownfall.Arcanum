...
- [x] **W2.1 — Harden `SpellScanner` + workspace scanning + atomic spell writes** ✅ *done — cycle/depth/step caps + canonical-path visited set (both BFS walks), pre-open handle revalidation, configured `Spells:*` bounds threaded into scans, bounded `PhysicalWorkspaceScanner`, atomic `SPELL.md`/`SKILL.json` writes (`SpellAtomicFile`), scan single-flight (`SingleFlight.CoalesceAsync`), arsenal TTL bypass fixed, export byte caps enforced.*
...
- [x] **W2.4 — Apprentice engine reliability** ✅ *done — slot-before-`Running` (revert to `Escalated` on capacity fail); recovery-queue-full logs a warning; `OnExited` disposes the `Process` (idempotent vs `StopAsync`); pending-queue dedup + cancellable + `CancelAsync` drains it + queued start returns success; Simulacrum single-branch fault isolation; shutdown cancel persists `Paused` (no step re-exec); Planning recovery emits `ApprenticeResumed`.*
...
- [x] **W2.5 — llama-server lifecycle & cache integrity** ✅ *done — computed port + `--port` override clamped to 1..65535; `portOverride` validated; manifest finalize + manifest touch atomic (`SpellAtomicFile`); LRU-over-cap operator warning; `RequireModelHash` rejects unverified cache entries; `GetEntryDirectory` normalizes the cache key at the storage boundary; `McpProcessTransport` child-exit disposes the `Process` (idempotent vs `DisposeAsync`).*
...
- [x] **W3.1 — Self-evicting keyed-lock / keyed-cache utility (systemic S3)** ✅ *done — `KeyedLock<TKey>` (self-evicting per-key `SemaphoreSlim` with reference-equality `TryRemove` cleanup); migrated `SessionWriteLock`, `SpellRepository._workspaceLocks`, `TheReliquary._downloadLocks`; `SanctumBreachStore` → bounded LRU; `HumanPromptRegistry` left (wrong-shaped for bounded store); MCP registry out of scope. Repaired `SpellScannerTests` parallelization (static-cache sharing) via a `SpellScanner` `DisableParallelization` collection.*
...
- [x] **W3.2 — Surface suppressed failures (systemic S4)** ✅ *done — CommLink returns `Failure("CommLink.Suppressed")`; `SecureFilePermissions` logs chmod/ACL failures + self-check/hardening cover both secret files + temp-file perms set before write; in-process MCP emits a JSON-RPC error (`-32600`, `id: null`) for oversized lines; `SessionEventHub` logs an `EventsDropped`-equivalent warning.*
...
- [x] **W3.3 — Honor config reload & make soft caps atomic (systemic S5 + S6)** ✅ *done — `WardGate`/`MCP MaxServers` atomic increment-then-compare-then-rollback (mirrors `SseConnectionGate`); `ChatClientFactory` soft-cap over-cap warning; daemon single-running atomic `TryStartAsync` + id-matched removal; `ApiKeyDigestCache` atomic `(digest,expiry)` snapshot record; `ManaPreflight` LRU reload-aware via `OnChange`; hubs/ring-buffer documented startup-only. Repaired `WardGateTests` (removed fragile Barrier) + serialized `ApprenticeServiceReliabilityTests`.*
...
- [x] **W3.4 — Atomic durable writes, streaming disconnect, MCP robustness (systemic S9 + S10)** ✅ *done — `ClientDisconnect` helper + all three writers catch broken-pipe IOException; ward `JsonDocument` disposed on all terminal paths; MCP outbound line cap (`McpOutboundLineGuard`), wire-cancel on timeout token, bridge fallback restricted to `McpTransportUnavailableException` only, write-path handle revalidation; `SearchArchivesAsync` opens connection; WAL checkpoint on shutdown; export stream-serialized. **AOT gate passed.** Wire contracts preserved.*
...
- [x] **W3.5 — Api correctness & consistency (P2)** ✅ *done — codex write + prompt-test now use `EffectiveCodexMaxSizeBytes`; config PUT rejects residual `"***"` for new providers / model-map URLs (`ConfigurationRedactor.ValidateNoResidualMask`); `run_spell_script` allow-lists `.py/.js/.sh/.ps1` (unknown extensions rejected, fall-through fails closed); Sanctum preflight validates the script path under **every** candidate root (active spell + resonant deps) via `TurnContext.SpellScriptRoots`; `/intelligence/ping` (resolve + turn failures) routes through `InferenceErrorMapper` incl. `Campaign.NotFound`→404; rate-limit 429s emit the `ApiResponse` envelope (`OnRejected`); provider-test returns a sanitized failure (detail to logs); `ManaPreflight` accumulates token counts in `long` (saturating) and counts non-text content; `/v1` rejects unsupported/over-cap content parts (`MaxContentPartsPerMessage`) `400 invalid_value` and interleaves SSE keep-alives during idle gaps; streaming + sync interrupt cleanup uses `CancellationToken.None` (no orphan rows, terminal Error event reliably emitted); `BuildInferenceFailureMessage` no longer leaks the provider endpoint URL.*
...
- [x] **W3.6 — Core / Proving Grounds correctness (P2)** ✅ *done — `PingRequestBoundsValidator` measures UTF-8 bytes (was UTF-16 chars), counts tool-call args + content parts (no empty-`Content` bypass), bounds `AdditionalSystemPrompt`, and null-coalesces `Intelligence`; ProvingGrounds clamps `output` before parse/judge, checks cancellation between inquisitors, and fails closed on unknown schema type; `ConclaveLineage` paginates the full set into an in-memory parent map (fixes the cap-bypass and the O(items×depth) DB walks; `FindRootAsync` cycle guard already present); `ApprenticePlanParser` caps input size + maps `JsonException`→`InvalidOperationException`; `SanctumConfig` returns non-downcastable read-only collections; `Result<T>.Value`/`IsFailure` are `[JsonIgnore]`; `ApiResponse<T>.Data` omits on default via `WhenWritingDefault` (value-type-safe; omits the misleading defaulted `data` on failure); `PatternSnapshot.Threads` normalizes null→empty.*
...
- [x] **W4.1 — Code-quality P3s (cross-module)** ✅ *done — removed dead `StripAddColumnStatements`; `IChronosyncEngine.AnalyzeAndSyncAsync` takes a `CancellationToken` (threaded from CLI callers + impl); `ListPageResult.Items` and `PatternSnapshot.Threads` null-normalize; `Error.Details` is a non-downcastable read-only copy + dropped the unused `using`; `Session.Clone`→`CloneHeader` (+doc, caller updated); `ArcanumEvent`/`LlamaServerEvent` docs corrected (the latter marked internal-only — no SSE subscriber); semantic judge requires an exact YES/NO first token; `AdjudicateAsync` over-limit returns a failed verdict (no throw); `ProviderResolver` model matching is symmetric on `:tag`; `DoctorCommand.Settings` rooted via `[DynamicDependency]`; `daemon initiative` + `llama start` flags validated client-side; pre-stream error events surface the error `Code` (`FormatApiError`); `ask_human` submit failures + silent cancel/empty answer now print a message; `MarkdigSpectreRenderer` caps render input at 256 KiB; `ask` returns 130 only on real cancellation; llama start/stop/status use `HttpCompletionOption.ResponseHeadersRead`; `key show` help corrected to "stderr". The refuted oversized-`@path` item was confirmed not a bug.*
...
- [x] **W4.2 — Documentation accuracy (coordinate, don't duplicate)** ✅ *done — synced DESIGN §3.4 for the now-symmetric `ProviderResolver` `:tag` matching (the audit's flagged gap). The other items need no change: the `/v1` `tool_calls` omission is intentional + already documented (§8.8.1, asserted by `OpenAiV1ParityTests`), and the CommLink wording / Sanctum route-param naming are already accurate in the tree.*
...
- [~] **W4.3 — (Optional) repo-wide blank-line normalization (systemic S11)** ⏭️ *intentionally skipped — optional by design; new/edited code already conforms to the one-blank-line house style, and a repo-wide `align-csharp-blanklines.sh` reformat would be a large diff that the plan says to keep as a separate isolated commit (not mixed with behavioral waves). Run standalone if/when desired.*
...
The documented gate is line ≥ 85% / branch ≥ 75% / security types 100% branch. **As of the Wave 4 close this is fully met** — `scripts/coverage.sh --threshold` reports **88.66% line / 77.91% branch** with **all security types at 100% branch**, exactly as the W5.1 prediction anticipated (the branch tests landed alongside the Waves 2–3 fixes that created those edge paths).

- [x] **W5.1 — Security types to 100% branch** ✅ *done — `McpSecurityLimits`, `SandboxedFileIo`, `WardGate`, `ApiKeyDigestCache`, `GrimoireKeyDerivation`, and `OutboundUrlGuard` are all at 100% branch coverage. As predicted, the gaps closed "for free" as the error/edge-path tests were written alongside the Waves 2–3 fixes (e.g. W3.4 ward `JsonDocument` disposal + MCP line caps, W3.6 `McpSecurityLimits` inherit-env branches).*
- [x] **W5.2 — Overall line/branch to target** ✅ *done — 88.66% line / 77.91% branch against the 85/75 gate, with the W0.1 runsettings exclusions (generated EF model, source-gen contexts, `[ExcludeFromCodeCoverage]` types, DTO contracts) applied. No coverage was chased on generated/excluded code.*
...
- [x] **DX4 — [P3][tests] `InferenceFlagBinderTests` global `AnsiConsole.Console` parallelization flake** ✅ *done — `InferenceFlagBinderTests` + `CliSessionManagerTests` moved to a `GlobalConsole` `DisableParallelization` collection.*
...
- [x] **DX5 — [P1][DI] `CliApplicationFactory.ConfigureCliServices` does not register `IApiKeyDigestCache`** ✅ *done — `IApiKeyDigestCache` registered (mirrors the Infrastructure wiring); new regression test resolves it AND constructs `AskCommand` via the real DI container.*
