...
The `UnseenServantWatermarks` migration (`20260703150000_AddUnseenServantWatermarks.sql`) is a single `CREATE TABLE` with no foreign keys to existing tables and no changes to existing tables or columns.
...
The `UnseenServantWatermarks` table is purely additive — a new table with no foreign keys to existing tables, no column changes on existing tables, no data backfill. Arcanum has no production Grimoire databases in the wild (see `docs/README.md` "Database migrations" section), so the migration applies on first start via `GrimoireDatabaseBootstrapper` with zero risk to existing data. No data migration step is needed.
