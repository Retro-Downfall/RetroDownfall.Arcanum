...
held list carrying the exact request and the generation observed before its refusal; a top-of-loop
generation comparison that writes held requests straight to `_channel.Writer` with `TryWrite`; the
reconciliation scope given a lease of its own.
...
- Never link `IGrimoireWorkLease.MaintenanceRevocation` into a token passed to the provider, to a
  ...
  fails review.
- Register no next-open-generation waiter. §1.3 of the spec records why, and why the hazard is worse
  for this worker than for Entry weaving.
...
- [ ] RED: a test asserting an admitted request returns `SessionAttachmentIndexDisposition.Concluded`
      does not compile / fails.
- [ ] GREEN: add the enum; make it the first positional member of `SessionAttachmentIndexOutcome`;
      add the single `DeferredForMaintenance` static; update every construction site.
- [ ] Every existing attachment-indexing case still passes unchanged in meaning.

### Task 2 — Sequential effect groups are real

- [ ] RED: a gate test that disposes one effect group and begins a second on the same lease while
      ordinary fails.
- [ ] GREEN: none needed — the gate already supports it. The test pins the behaviour §3.4 depends on.

### Task 3 — One work lease per dequeued request

- [ ] RED: a dequeued request against a closed gate creates no scope, opens no connection, calls no
      provider, writes nothing, and returns `DeferredForMaintenance`.
- [ ] RED: an admitted request takes exactly one lease of kind `SessionAttachmentIndexing`.
- [ ] RED: a closure begun from inside the request's scope disposal does not conclude its drain
      synchronously.
- [ ] RED: a genuine failure's `MarkFailedAsync` scope is created while the lease is still held.
- [ ] GREEN: inject `IGrimoireConnectionAdmissionGate`; take the lease before the `try`; declare it
      so reverse-order disposal releases every scope first.

### Task 4 — One sequential effect group per batch

- [ ] RED: revocation winning the frontier makes zero provider calls, writes nothing past the pending
      mark, and returns `DeferredForMaintenance`.
- [ ] RED: effect start winning makes the closure wait through the provider call and its append.
- [ ] RED: a close landing between batch N and batch N+1 keeps batch N's chunks durable, makes no
      further provider call, and defers.
- [ ] GREEN: pass the lease into `ProcessAsync`; open the group at the top of `FlushBatchAsync` and
      close it after whichever durable exit that batch takes.

### Task 5 — The identity, the attempt, and the re-signal

- [ ] RED: after a deferral the attachment is still in `_pending`, the durable `AttemptCount` is
      unchanged, the durable status is not `Failed`, and the staged generation still exists.
- [ ] RED: a `TryEnqueue` for that attachment during the deferral is still deduplicated.
- [ ] RED: after reopen the exact request instance reaches the channel once, and a second iteration
      at the same generation does not write it again.
- [ ] GREEN: read the generation before the lease attempt; hold the exact request; release `_pending`
      only on `Concluded`; re-signal with `TryWrite` at the top of the loop.

### Task 6 — The loop arms and the reconciliation scope

- [ ] RED: a repeatedly deferred worker logs nothing at `Error` or `Warning`, creates no scopes, and
      calls no provider.
- [ ] RED: a deferred reconciliation opens no scope.
- [ ] GREEN: add the `Debug` arms; stop the batch loop on a deferral and skip its trailing
      reconciliation; give `ReconcileAndEnqueueAsync` its own lease.
- [ ] The existing genuine-failure and host-cancellation paths still pass, pinning all three apart.

### Task 7 — The missing coverage reason

- [ ] GREEN: `[ExcludeFromCodeCoverage]` on `SessionAttachmentIndexingService` gains the inline
      reason `Arcanum.DESIGN.md` §13.8 requires and every sibling already carries.

### Task 8 — Documentation

- [ ] `docs/Arcanum.DESIGN.md` §21.8, §10.20.3, §13.7.
- [ ] `docs/Arcanum.Engineering.md` per-issue paragraph after #253.
