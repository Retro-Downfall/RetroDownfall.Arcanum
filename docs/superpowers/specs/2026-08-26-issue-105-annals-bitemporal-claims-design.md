...
**Revisited.** Saga curation gave the store an operator write path after this slice shipped, and the sentences above are the record of what was true when it did. Both still hold of the insert path they describe: a memory arrives there from extraction alone, and its opening claim is still `AgentExtracted` at revision one. What changed is that a Saga claim now does reach revision two — correction, retirement, and reinstatement append `OperatorStated` versions over a memory that already exists. Correction and retirement open the claim with this same assertion first when none exists yet, so the history reads as extraction asserting and the operator acting rather than crediting the operator with the original assertion. Reinstatement does not, and needs no such arm: the only memories it can act on are ones a retirement already claimed. `ValidFromUtc` and `RecordedAtUtc` are both the memory's own `CreatedAt`. `SensitivityCode` is `None`; a labelled Saga artifact is refused deletion by the existing guard rather than carried here.
