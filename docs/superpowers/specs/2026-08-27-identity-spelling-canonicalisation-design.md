...
**Also left in the minority form on purpose, and recorded here because an undocumented divergence is how this family reached fourteen sites.** `saga_memories.SessionId` is written by the Saga memory store with a bare `ToString()` and read back the same way by every one of its own readers, so it is internally consistent and nothing compares it across a component boundary — except the version-two scope-classification sweep, which joins it to `session_campaign_bindings.SessionId`. That one join is **normalised rather than either column being converted**, because the two sit on opposite sides of a governance boundary: the binding column is forced canonical by a foreign key while this one is free. Converting it would have meant a second data rewrite with its own writers, readers and guards. `session_campaign_bindings.CampaignId` is the second: it carries no foreign key *by design*, because it is the historical authority identity and a Campaign deletion must be able to clear its own row without rewriting the durable fact, and the repository writer and both of its readers render it the same way.
