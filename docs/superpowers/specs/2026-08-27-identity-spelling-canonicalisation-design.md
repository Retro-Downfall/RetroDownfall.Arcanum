...
**`session_campaign_bindings.CampaignId` was listed here and is not a divergence.** It carries no foreign key *by design* — it is the historical authority identity, and a Campaign deletion must be able to clear its own row without rewriting the durable fact that a Session was bound to that Campaign — and nothing therefore forced its **two** writers to agree. They did not: the core data initializer canonicalised what it backfilled and the turn-begin repository bound a bare `ToString()`, so one column held two spellings of one Campaign. `SagaMemoryScopeClassifier` returns that value verbatim and both the live write path and the version-two sweep copy it into `saga_memories.CampaignId`, so Campaign-scoped recall — which binds one rendering against that column on every turn — returned only about half of a Campaign's memories, silently. Both columns are converted, guarded and repaired below.
