...
| Line | ≥ 85% |
| Branch | ≥ 75% |
| Security-critical types | 100% branch (`ApiKeyEndpointFilter`, `ApiKeyDigestCache`, `DataProtectionSecretStore`, `GrimoireKeyDerivation`, `McpSecurityLimits`, `TrustedMcpWorkspaceStore`, `SandboxedFileIo`, `SecureFileReader`, `IdentityOwnedFileSystemCleanup`, `SanctumGuard`, `OutboundUrlGuard`, `HostProcessToolPolicy`, `IdempotencyClaimStore`, `BudgetReservationService`, `WardGate`) |
...
`scripts/coverage_threshold.py` and the Windows fallback `scripts/coverage_threshold.ps1` carry the same required security-type set. Parser tests compare those sets and exercise both missing-type failures; either threshold implementation fails when a required class is absent from Cobertura, so exclusion/rename cannot silently appear as 100% coverage.
...
- **Mutable scenarios** (workspace trees, Grimoire DB copies, CODEX/config/session/MCP writes) use `Support/TempWorkspace` or fixture helpers. They retain and delete only their exact uniquely named owned root; cleanup must never infer an ancestor by walking parent directories. Before the first `ArcanumPaths` access, path-owning suites set `DOTNET_ENVIRONMENT=Testing`, `ASPNETCORE_ENVIRONMENT=Testing`, and guarded `ARCANUM_TEST_HOME=<unique root>`, assert the resolved path is beneath that root, then restore all variables. They never inspect, back up, modify, migrate, or delete real user config/files. This is required on Windows because changing `HOME`, `USERPROFILE`, or `APPDATA` does not redirect .NET known-folder paths after process start.
...
`ProcessEnvironment` is the canonical collection for suites that mutate environment/path globals. A class that already requires another single xUnit collection, such as `CliSessionManagerTests` under the also-nonparallel `GlobalConsole` collection, must still apply the same Testing + `ARCANUM_TEST_HOME` guard and serialization guarantee; xUnit permits one collection per class.
...
Windows process-boundary coverage lives in `ProcessResourceLimiterWindowsBehaviorTests`, `WindowsJobObjectSessionTests`, and `ChildProcessBoundaryBehaviorTests`: Job Object create/configure/assign errors use hand-written API fakes, stream setup/read failures use custom `Encoding`/`Decoder` implementations, filesystem cleanup uses uniquely owned temp paths, and process-tree termination uses immediate-exit or 30-second bounded children with prompt-termination assertions and unconditional cleanup. `SecureFileReaderTests` covers no-follow/nonblocking opens, regular/single-link enforcement, FIFO/device/hardlink rejection, capped pooled reads, and post-open metadata failures. `IdentityOwnedFileSystemCleanupTests`, `AtomicFileTests`, and child-jail tests cover owner-only quarantine, replacement retention, backup-content verification, and bounded temp cleanup. `PhysicalFileSystemBrowserTests` / `SandboxedFileIoTests` cover post-open containment and identity. `SpellVersionPathPolicyTests` covers the complete label and sidecar-filename policy without touching the filesystem. `WorkspacePathPolicyTests` / `WorkspacePathPolicySymlinkTests` cover the primary lexical, symlink-component, and handle-revalidation boundary independently of campaign Sanctum.
...
- `TrustedMcpWorkspaceStoreTests` and `McpConnectionManagerTrustGateTests` prove the exact single-read parse/digest binding, current-file cache/lifecycle freshness, source-digest-carrying entries, stale retirement, bounded trust-document validation, and owner-only atomic persistence.
- `SecureFileReaderTests`, `SandboxedFileIoTests`, `PhysicalFileSystemBrowserTests`, `IdentityOwnedFileSystemCleanupTests`, `AtomicFileTests`, and child-process boundary tests prove no-follow regular single-link reads, strict caps/UTF-8, post-open containment, private identity-owned quarantine, verified backup restore, and bounded child temp cleanup.
