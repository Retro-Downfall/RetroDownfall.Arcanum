...
Grimoire DB tests use `[SkippableFact]` and skip when `e_sqlcipher` is unavailable (`GrimoireFixture.SqlCipherAvailable`). The availability probe disables SQLite pooling and requires its temporary database to be deletable before reporting SQLCipher available. Cached-template validation, remediation, and main-database/sidecar copying share one process-wide lifecycle lock; template connections are non-pooled so parallel fixture construction cannot delete or expose a partial template while another fixture copies it.
