    ...
    TimeProvider timeProvider) : IProtectedArtifactTransferStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using (SqliteCommand write = destination.Connection.CreateCommand())
        {
            ...
            _ = UtcInstantSql.AddStoredParameter(write, "$createdAt", graph.Session.CreatedAt);

            _ = UtcInstantSql.AddStoredParameter(write, "$updatedAt", graph.Session.UpdatedAt);
            ...
            _ = UtcInstantSql.AddStoredParameter(
                write,
                "$lastSummarizedAt",
                graph.Session.LastSummarizedMessageAt);

            _ = write.Parameters.AddWithValue("$tokens", graph.Session.TotalTokensUsed);

            _ = ExactUsdText.AddParameter(write, "$cost", graph.Session.TotalCostUsd);
        ...
        foreach (EntryRow entry in graph.Entries)
        {
            ...
            _ = UtcInstantSql.AddStoredParameter(write, "$createdAt", entry.CreatedAt);
        ...
        foreach (AttachmentRow attachment in graph.Attachments)
        {
            ...
            _ = UtcInstantSql.AddStoredParameter(write, "$createdAt", attachment.Values[11]);
            ...
            _ = UtcInstantSql.AddStoredParameter(write, "$sourceLastWriteAt", attachment.Values[18]);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await reader.ReadAsync(cancellationToken).ConfigureAwait(false)
            ? new SessionRow(
                ...
                UtcInstantText.Normalize(reader.GetString(4)),
                Value(reader, 5),
                reader.IsDBNull(6) ? null : UtcInstantText.Normalize(reader.GetString(6)),
                Value(reader, 7),
                ExactUsdText.Read(reader, 8),
                Value(reader, 9))
            : null;
    }
    ...
    private sealed record SessionRow(
        ...
        string UpdatedAt,
        object Summary,
        string? LastSummarizedMessageAt,
