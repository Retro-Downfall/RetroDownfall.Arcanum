...
public static class SettingDescriptors
{
    ...
    [
        ...
        new("providers.models.supportsTools", ConfigSection.Providers, "Can call tools", "Declared tool-calling capability for this model. Leave unset when unknown — only an explicit off means this model cannot call tools.", SettingKind.Bool, AllowUnset: true),
        ...
        new("features.annals", ConfigSection.Features, "Remember how memory changed (The Annals)", "On by default, this records automatic Saga and Lexicon history without changing what the AI recalls. Turning it off stops only future automatic records, while existing history stays readable. Re-enabling it does not backfill the opted-out period or create earlier claims; later writes follow the setting in effect then. Corrections, retirements, and reinstatements that change Saga state still record required evidence, while pin and unpin never write Annals.", SettingKind.Bool),
