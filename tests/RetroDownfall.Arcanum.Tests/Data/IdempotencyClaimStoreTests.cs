...
public sealed class IdempotencyClaimStoreTests : IAsyncLifetime
{
    ...
    [SkippableFact]
    public async Task TryAcquireAsync_WhenReclaimedRowIsDeleted_ReturnsOriginalLeaseSnapshot()
    {
        RequireSqlCipher();
        ...
        Assert.False(result.Conflict);
        Assert.False(result.Acquired);
        Assert.Equal(acquired, result.Claim);
        Assert.Null(await store.TryGetAsync("deleted-after-reclaim"));
    }
    ...
    public async Task HeartbeatAndLinkRun_UpdateOnlyCurrentRunningOwner()
    {
        ...
        await store.HeartbeatAsync(acquired.Id, "wrong-owner", wrongOwnerLease);
        ...
        await store.HeartbeatAsync(acquired.Id, "owner-1", correctOwnerLease);
